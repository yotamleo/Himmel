...
3. **Cross-model finding passes (critic panel, codex), then adjudication (HIMMEL-178, HIMMEL-270, HIMMEL-415, HIMMEL-926).**
   ...
   **The App's contract, which step 7 enforces after the PR exists.** Three things, none of which this step can do:
   - **Re-trigger on every push.** `@coderabbitai review` after each head move — a new head DISCARDS the in-flight review. `scripts/hooks/trigger-cr-on-pr-create.sh` / `trigger-cr-on-push.sh` do this; a manual push still owes the comment.
   ...
   - **A concluded status, not merely an absent one.** `cr-signal.sh` reads CodeRabbit's **commit status** (it posts no check-run); `absent`, `pending`, `paged` and `skipped` all fail CLOSED. Armed per-clone with `git config --local himmel.coderabbit true`.
