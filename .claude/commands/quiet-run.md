...
Lifetime (HIMMEL-2221): a quiet-run started in the background is a **live child of your session** — count it (and any `tail -f` / `Monitor` you pointed at its log) when you decide "zero live children" before closing or wrapping, not just Agent-tool subagents. Killing quiet-run with TERM/INT/HUP reaps the whole command tree it wraps (exit 128+signal) when its stdin is not a terminal (every agent/CI run); with a terminal on stdin it keeps the foreground shape so the command can still read the tty (^C reaches the whole group there, but a bare `kill <pid>` of the wrapper does not). Two more exceptions: SIGKILL cannot be trapped, and a quiet-run started as a non-interactive `&` job inherits SIGINT already ignored, which bash cannot trap, so INT is not reaped there (TERM/HUP still are). A `tail -f` on the log is yours to stop — quiet-run starts none.
