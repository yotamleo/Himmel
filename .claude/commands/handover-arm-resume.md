...
argument-hint: --time <HH:MM|smart|auto> --handover <path> [--force] [--dedup-any] [--dry-run] [--model <name>] [--fable-ok <reason>] [--context <1m|standard>]
---
...
- **Context mode is an arming-time choice, never inherited (HIMMEL-2658):** `--context 1m|standard` appends/strips the `[1m]` model-id suffix (silently a no-op on a Fable-family model — the CLI strips it there) and passes `--autocompact auto|200000`, the lever that actually moves cost. Defaults to `1m` on a `*-console.md` handover, `standard` on every other arm; the resolved mode, its source, and the effective `--autocompact` are printed at guard time, so `--dry-run` shows them too.
