...
# The 4 skills himmel actually invokes:
#   brainstorming, writing-plans, systematic-debugging,
#   verification-before-completion
# plus the 7 they hand off to, pulled in to close the `superpowers:<name>`
# reference closure (vendoring only the 4 leaves dangling cross-references):
#   executing-plans, finishing-a-development-branch, requesting-code-review,
#   subagent-driven-development, test-driven-development, using-git-worktrees,
#   writing-skills
# DELIBERATELY NOT vendored (3 of 14): using-superpowers (its SessionStart
# injection is the ~700 tok this change reclaims), receiving-code-review,
# dispatching-parallel-agents — nothing vendored references them.

vendored_from=mattpocock/skills@1.2.3 path=skills/productivity/grilling
# grilling only. 0 of the plugin's 11 exposed skills were ever invoked in
# 1628 local transcripts; grilling reaches us through the minerva hook's
# namespace-agnostic `*:grilling|grilling` matcher, which keeps routing the
# bare vendored name. The upstream skill ships an agents/openai.yaml
# (Codex interface metadata) that is dropped here as harness-specific.
