...
// [telegram-himmel fork] Only the designated owner session polls getUpdates.
// Telegram allows one poller per token; ungated polling lets any claude
// session steal the slot. Set TELEGRAM_OWN_POLLER=1 in the owner launch.
const OWN_POLLER = process.env.TELEGRAM_OWN_POLLER === '1'
...
// [telegram-himmel fork] Non-owners must neither reap the owner's poller nor
// clobber bot.pid — both are owner-only acts.
if (OWN_POLLER) {
  try {
...
// [telegram-himmel fork] Poll only as the owner. Non-owner sessions keep
// every outbound tool (reply/react via bot.api) but never start getUpdates,
// so they cannot take the single poll slot.
if (!OWN_POLLER) {
  process.stderr.write('telegram channel: poller disabled (not owner; set TELEGRAM_OWN_POLLER=1)\n')
} else void (async () => {
