...
| `handover-resume #N` (op; or `/handover:handover-resume`) | Resume a tracked item — surface its brief, decisions, and stop-point. Read-only (no worktree gate). Forms: **no ID** → picker over active items; **`#N`/`HIMMEL-N`/bare number** → that item; append **`overnight`** → overnight-mode trigger. Invoke via "load #N", the namespaced `/handover:handover-resume`, or (in a himmel checkout) the bare `/handover-resume` convenience command. Distinct from `/handover-resume-armed`, which recovers an interrupted/armed session. |
