...
2. **Preflight — validate before mutating** (retry-aware, compatible with the step-3 re-runnable skips). **Branch on suggestion type first — the move-state rules below apply in full only to suggestions whose destination is already known** ("fold X under Y", where the suggestion itself names `Y`).

   **`promote A+B+C to new epic` has no destination yet at preflight** — the epic name is chosen by the `AskUserQuestion` in step 3, so the `<JIRA-KEY>-<slug>` path does not exist to validate. Requiring "destination absent + ancestors identity-check" here is unevaluatable, not strict. Split it in two: validate the **sources** now (each source dir exists and holds its expected `<ID>-<slug>` item, and each Jira key still resolves), then **compute the destination immediately after the name is chosen and re-validate it there** — destination absent, ancestors identity-checked — **before the first move**, so a bad destination still stops the apply before it mutates anything. A **ghost follow-up** ("X referenced but never filed") moves nothing and has no source dir or destination, so requiring a move state would reject every valid one. For it: validate the **closed source item that references the ghost**, and confirm the referenced ID is **still unfiled** — do NOT require the ghost's own ID to resolve, since "never filed" is precisely its definition and a resolving ID means the suggestion is stale, not valid. Then let it proceed to step 3's "file as standalone now?" action. For a known-destination move suggestion — and, for `promote`, at the post-prompt re-validation point above — accept EXACTLY ONE of two valid states — each pins **both** ends of the move, source *and* destination: (a) **not yet moved** — the recorded source dir exists **and holds the expected `<ID>-<slug>` item** (identity-check the source too, not merely its presence — a path that exists but holds an unrelated item is a stale suggestion, not a pending move) AND the expected destination is **absent** AND every **existing ancestor of that destination identity-checks** — for a "fold X under Y" the destination parent `Y/tasks/` must actually belong to the expected `Y`; an ancestor holding an **unrelated** item means the suggestion is stale or collides, so **stop and report** rather than moving X underneath the wrong parent; or (b) **move already completed** on an earlier run — the source is **absent** AND the expected destination already holds the **same** `<ID>-<slug>` item (identity match). Treat (b) as VALID and let step 3's re-runnable skips carry the retry forward; do NOT reject it as stale. Confirm every Jira key still resolves. **Stop and report** for every other state: source **and** destination both present (an ambiguous half-state — step 3's identity-match skip would skip the `git mv` and leave the source behind as a **duplicate** directory while refs and Jira advance past it, so it must never reach step 3); a source missing with **no** identity-matched destination (the stored suggestion is stale — re-run `/handover hygiene consolidate`); or the expected destination occupied by an **unrelated** item (a real directory collision). Do not begin a partial apply on a genuinely stale suggestion.
3. Execute its proposed action. Do each sub-step so it is **individually re-runnable** — but a re-run must never mask a real conflict:
   - **Skip a move only on an identity match with the source already gone.** Skip a `git mv` only when the destination already exists **and** holds the expected source item (same `<ID>-<slug>`) **and** the source is **absent** — that trio is what "the move already completed" means. If the source **still exists** while the destination holds the expected item, **stop and report**: skipping there would leave the source behind as a duplicate directory. Step 2's preflight already rejects that state, but do not rely on it — re-assert the invariant here so the skip cannot silently duplicate an item if it is ever reached by another path. If the destination exists but holds an **unrelated** item (a directory collision), **stop and report** — never treat an unrelated collision as a completed move.
   ...
   - **Create the epic idempotently.** When "promote A+B+C to new epic" creates the epic, follow **Idempotent Remote Create** in `references/resolution.md` — write the intent record BEFORE the create, look up the marker, then create (or adopt the key the lookup found). The intent record lives in `<state-root>/.pending-consolidate/<canonical-identity>.json`, **NOT** in `.last-consolidate.json`: that file is the suggestion cache this flow reads back in step 1 to resolve `<N>`, and multiplexing both through it lets either write destroy the other (lose the suggestion → `apply <N>` cannot resolve; lose the nonce → the retry files a second epic). Key the record on the **canonical sorted source IDs + `promote`**; on a retry after partial failure the lookup adopts that key instead of creating a second epic. The epic NAME must NOT be part of the identity — the name is chosen by the step-3 `AskUserQuestion`, so a retry where the operator types a different name would key a different record, miss the nonce, and file the duplicate.
