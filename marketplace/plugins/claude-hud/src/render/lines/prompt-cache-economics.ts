...
import {
  ...
  computeCacheNetUsd,
  formatCacheUsd,
  type CacheEconomicsTotals,
} from '../../cache-economics.js';

const EMPTY_TOKENS: SessionTokenUsage = {
  inputTokens: 0,
  outputTokens: 0,
...
function formatNet(reads: number, writes: number, ctx: RenderContext): string {
...
function formatRow(rowLabel: string, reads: number, writes: number, inputs: number, ctx: RenderContext): string {
...
export function renderPromptCacheEconomicsLine(ctx: RenderContext): string | null {
  ...
  const all: CacheEconomicsTotals | null | undefined = ctx.allSessionsCacheEconomics;
  if (!all) {
    return sessionLine;
  }
