...
5. **Dispatch only on confirmation** — one `Agent` call per item, passing the
   ...
   Every brief carries:
   - **Context / why / done-looks-like** — the child starts blank. State the
     whole arc the item actually wants (implement → run it → inspect → fix what
     fails), not just the first deliverable: an implicit "done" gets a first
     pass handed back with work still on the table, and on the codex/hermes
     lanes that is the documented Astra failure mode
   ...
   - **Long-output note at `xhigh`/`max`** — when an item's effort is
     `xhigh`/`max` and the deliverable is long (multi-section doc, large diff,
     full rewrite), append the guide's note: leave reasoning space to reason
     and output space to write, so the child doesn't draft the deliverable
     twice.
   - **The provider pin on any codex/hermes dispatch** — a codex-family model
     needs `--provider openai-codex` passed alongside whatever `--model` the
     plan selected; the pin is on the provider, never on a particular model id
     (`scripts/cr/hermes-critic.sh` keys it off the model's family). Omitting it
     fails as `No LLM provider configured` from a live lane, not as an
     unknown-model error (`docs/internals/retask-channel.md` §3).
