...
0. **Queue lock (HIMMEL-856)** — before Phase 1, run
   ...
   `$XDG_RUNTIME_DIR/himmel-queue-lock/`), so a Phase 11 release with no
   token on argv reads it back and names the file on stderr — an argv token
   still wins. Keep capturing it anyway: the file is a safety net for a
   mid-run autocompact eating the transcript line, not a replacement for
   carrying the token forward. **rc=2** means the queue is owned by a LIVE session elsewhere
