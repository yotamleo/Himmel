...
`scripts/machine-setup/uninstall-plugins.{sh,ps1}`, at the scope recorded in
the install profile (HIMMEL-2694; **user-scope by default — affects every
repo on the machine** — but a project/local-scope install affects only this
repo), (5) `pre-commit uninstall` ×3 hook types,
