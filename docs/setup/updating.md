...
> **Not on `himmelctl` yet?** This page assumes the current installer is
> already what wired your machine. If himmel got there by an older `adopt.sh`
> run, by hand, or by a `himmelctl` older than this one, converge first:
> [migrating.md](migrating.md) has the four from-states, how to tell which one
> you are in, and the converge path for each.
...
their own scope, uninstalls the repo's git hooks (and restores a hook that
install displaced to `<hook>.himmel-backup` when its target is free; otherwise
