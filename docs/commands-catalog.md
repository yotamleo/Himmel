...
| /graph-publish | Publish a freshly-refreshed graphify graph — commits + opens/refreshes a PR against the tracked graphify-out/ artifacts (HIMMEL-1129). |
| /graph-refresh | One-shot operator refresh of the luna and/or himmel graphify graphs — fires refresh-graph-map.sh per corpus serially with the cadence's argument sets (HIMMEL-1644). |
