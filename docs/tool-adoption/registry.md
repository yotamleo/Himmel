...
| obsidian-second-brain research toolkit (`/x-read`, `/x-pulse`, `/research`, `/research-deep`, `/youtube`) | D (runtime / external data-fetch) | 2026-07-29 skill-hygiene audit (retroactive — installed 2026-05-16, no ADR filed until now); `docs/tooling-catalog.md` | provisional (pending operator sign-off) | (Grok/Perplexity-backed research commands) — provisional 2026-07-29, pending operator sign-off on the ADR: no active XAI/Grok or Perplexity subscription behind the `~/.config/obsidian-second-brain/.env` credentials file (presence-without-validity — file existing since 2026-06-12 is not evidence of a working key, the exact gap HIMMEL-1393's readiness gate now checks for). Separately redundant: the vault-first capture pipeline (`/telegram-clip` + Web Clipper → `harvest-clips` → `luna-ingest`/clip-body) already covers X and YouTube content without any external API (`harvest-clips.md`'s own LUNA-26 comment: these external-fetch paths were already deferred/unused by the MVP); `read-link` (LUNA-78) explicitly rules "never Grok". `/notebooklm` (Gemini File Search, different key) is unaffected | `source-read-mandatory` (external API, secret + network egress — never independently evaluated at install time, HIMMEL-199/200 gap) | `docs/tool-adoption/adr-obsidian-second-brain-research-toolkit.md` (DRAFT, pending operator sign-off) |
| unslop (`cursor/plugins` pstack) | A (prompt / response shape) | HIMMEL-3115 (scoped-research leg + operator pilot decision, 2026-09-17) | pilot-pending | AI-tell removal in human-facing documents (PR bodies, ADRs, handovers, tickets) → fewer operator reword-passes per document. Rubric §1 does NOT pass on evidence (no CR finding/stuck loop/operator complaint about AI-sounding prose exists) — operator chose to pilot anyway given near-zero cost/blast-radius; close on operator-verdict outcome across 3-5 documents, never on rule-count applied | `community-active → validate-before-adopt` (33 numbered style rules, injection scan clean, Layer-A prompt content only — no hook/secret/network surface, reversible by deleting one file) | `docs/tool-adoption/adr-unslop-skill.md` |
| test-audit (`openclaw/openclaw` `.agents/skills/test-audit`) | A (prompt / response shape) | HIMMEL-3580 (operator request, 2026-09-24; folds the HIMMEL-2708 GPT-6 test-audit findings HIMMEL-2725/2726/2728) | adopted | Fewer tests that stay green when their subject breaks: an authoring gate at write time, plus a candidate-evidence record before any test is removed. Measured by the first audit pass shipped in the same PR (RED-at-base excerpt per repaired suite) | `community-active → validate-before-adopt` (read in full, no hook/secret/network surface, Layer-A prompt content only; adapted with deviations listed in the SKILL.md header, reversible by deleting one directory) | — (provenance header in `.claude/skills/test-audit/SKILL.md`) |
