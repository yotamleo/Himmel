...
| Op          | Plugin                                                                      | MCP                                                              |
|-------------|-----------------------------------------------------------------------------|------------------------------------------------------------------|
| Get         | `node scripts/jira/dist/index.js get HIMMEL-N` (default includes description body and a `Labels:` line when the issue has any, HIMMEL-3610; add `--short` for header-only) | `getJiraIssue`                                                   |
...
| Edit        | `... edit HIMMEL-N --title ... --desc ... [--labels a,b]` (`--labels` is FULL-REPLACE: the set becomes the complete label list — no MCP `editJiraIssue` fallback needed for labels since HIMMEL-243) or `... edit HIMMEL-N --add-labels a,b` (APPENDS without reading/replacing the existing set, via Jira's atomic `update` operation; mutually exclusive with `--labels`, HIMMEL-3610) | `editJiraIssue`                                                  |
