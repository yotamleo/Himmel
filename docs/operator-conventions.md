...
- Operator habits with no structural home, inlined:
  ...
  - **Loop CodeRabbit to convergence — and the unguarded gap is public propagation.** The private side is structurally guarded: `check-ci.sh` exit 3 = checks green but unresolved review threads / changes-requested block the merge — lean on that backstop, not on memory. After any fix+push, a prior clean round is STALE for the new head; re-converge. The propagated public delta (`propagate-public.sh`) is a DIFFERENT diff surface (debranded/transformed, no private fixtures), so CodeRabbit there can surface real items the private rounds never saw — and nothing gates that loop: re-run CodeRabbit on the public PR and BACKPORT any real finding to PRIVATE first (fix private → re-propagate), never fix public-only, else the surfaces drift. Also: the `Mergeable` check run lints ONLY the commit message + PR title — never cite it as evidence of CI health, review convergence, or merge readiness.
