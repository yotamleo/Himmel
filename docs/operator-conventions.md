...
- Operator habits with no structural home, inlined:
  ...
  - **The CR marker is rewritten on nearly EVERY push** (`check-cr-before-push.sh` writes `<git-common-dir>/cr-pending/<branch>` — it resolves `git rev-parse --git-common-dir`, not a literal `.git/`, so the lookup also works from a linked worktree). It is NOT non-docs-only: a code push writes lane `full`, a **docs-only push still writes one** (lane `docs-audit` → the one-reviewer docs charter); only a **handover-state-only** push (everything under `handovers/`) writes no marker at all (HIMMEL-142). Clear it AFTER the final push, immediately before `gh pr create`, or a follow-up commit re-creates it and re-blocks. And clear it ONLY via `scripts/cr/clear-cr-marker.sh` (next bullet) — never a manual `rm` of the marker file, even after you genuinely ran the panel clean on that exact HEAD. The auto-mode classifier reads a bare marker `rm` + self-declared "CR clean" as **[CI Bypass]**, and can then DENY unrelated downstream writes in the same session as collateral (observed: a following `jira transition` denied, citing the rm). If a downstream write was already denied, record it as an operator action rather than retrying via another path.
  - **The CR-marker clearing path is `scripts/cr/clear-cr-marker.sh <branch>` — `/pr-check` step 5 calls it for you** (HIMMEL-1064). So re-running `/pr-check` IS sufficient; what is never acceptable is a bare `rm` of the marker, by hand or from any script. History worth keeping: step 5 used to do that `rm` itself, and the classifier denied it identically to a manual one — proven live with the gate clean from three independent sources on one HEAD (panel 0/0/0, codex-adv `approve`, thread gate rc=0). Every clean run hit it, and the stale marker then blocked `gh pr create` on a branch whose CR was actually clean. The script is the sanctioned chokepoint (same self-gating shape as `merge-on-green.sh`): it re-derives the verdict from the CR ledger at the marker's exact SHA — a critic that actually RESPONDED plus zero blocking findings — and, once a PR exists, additionally requires `check-ci.sh` green. It clears only on its own reading, strictly stronger than the `rm` it replaced (which asserted nothing). Zero responders is a MISSING signal, not a clean one (the CodeRabbit-CLI rate-limit shape) → exit 14; a commit after the review → exit 13. Non-zero = the marker STAYS; never fall back to `rm`. Needs the standing allow-rule `Bash(bash scripts/cr/clear-cr-marker.sh:*)`.
  - **An unread CodeRabbit App review is a MISSING signal, never a clean one (HIMMEL-2704).** The CLI round this bullet used to describe is gone; the App is the only surface. Its two failure shapes both LOOK clean: a `success` commit status whose description says `Review skipped: automatic reviews are disabled` (a refusal to review, not a clean review — `scripts/lib/cr-signal.sh` reads it as its own state and callers fail CLOSED on it), and a green check-run rollup sitting over unresolved review THREADS (six of them on PR #2209, 2026-09-06). Gate each round on `check-ci.sh` exit 0 — a CONCLUDED status at the head AND zero unresolved threads — never on a finding count, a green tick, or the absence of a comment.
  - **Budget the free CodeRabbit tier — rounds are scarce.** The free tier rate-limits after ~4 rounds in quick succession, and a rate-limited ATTEMPT appears to EXTEND the window (observed: honouring the reported `waitTime` exactly still bounced and pushed the wait OUT) — wait approximately 1.5× the reported `waitTime`, never poll-retry. Batch ALL known fixes into one push before spending a round; iterate on the free signals (critic panel, `check-ci.sh`, local suites) and spend CodeRabbit rounds on confirmation; EXPLICIT App triggers (`@coderabbitai review` / `@coderabbitai full review`) all draw on the SAME account bank, so every re-trigger after a push spends from it. This project stays on the free tier by policy — budget rounds instead of proposing the usage-based upgrade the rate-limit payload advertises.
  - **Loop CodeRabbit to convergence.** This is structurally guarded: `check-ci.sh` exit 3 = checks green but unresolved review threads / changes-requested block the merge — lean on that backstop, not on memory. After any fix+push, a prior clean round is STALE for the new head; re-converge. Also: the `Mergeable` check run lints ONLY the commit message + PR title — never cite it as evidence of CI health, review convergence, or merge readiness.
  - **Adversarial review never converges to zero — gate on the standard panel.** An adversarially-prompted critic is prompted to maximally find no-ship issues, so it may keep surfacing increasingly narrow or pre-existing findings across rounds — 13 rounds observed on one diff, floored at one [medium]/round of ever-finer edges while the STANDARD critic panel returned 0/0/0 on the same diff. Run the adversarial pass ~3–5 rounds for depth and fix every CONFIRMED finding, but gate CONVERGENCE on the standard panel clean + a clean CodeRabbit App review on the PR (check-ci exit 0), not adversarial-zero; once severity floors at narrow [medium] refinements, stop, record the residual mentions as "further hardening", and ship.
  - **CodeRabbit quiet incrementals post NO review object.** The GitHub App re-reviews every push incrementally, but a clean incremental (zero new actionable findings) posts no new review object — the only signals are CodeRabbit's **commit STATUS** on the head SHA and the updated summary-comment timestamp. **It is a status, not a check-run** (verified live on five consecutive PRs — see `scripts/lib/cr-signal.sh`, which matches it by creator id on `/commits/<sha>/statuses` for exactly this reason); `gh pr checks` shows it only because `statusCheckRollup` merges statuses and check-runs, so a `select(.name=="CodeRabbit")` over `.check_runs[]` matches NOTHING. Do not wait for a check run that never appears. `@coderabbitai review` on an already-reviewed commit replies "Review finished" and does nothing (it exists for paused automatic reviews). Never poll for a new review object after a fix push. And do NOT read that status as the verdict: it says a review concluded, not that its findings were dealt with — the THREAD QUERY is the truth, and a `pass` rollup over unresolved threads is exactly what shipped six of them on PR #2209 (HIMMEL-2704). The re-review-clean gate = (a) a CONCLUDED CodeRabbit commit status on the head SHA (`skipped`/`absent`/`pending` are not clean), (b) zero unresolved review threads (GraphQL `reviewThreads.isResolved`), (c) the actionable findings from the latest non-empty review cycle addressed (a clean incremental creates no new review object to check). When fixing any CR finding, RESOLVE its review thread (GraphQL `resolveReviewThread`) as part of the fix — a fixed-but-unresolved thread still fails gate (b) and stalls the merge. Structural backstop: the `block-unresolved-cr-merge` hook enforces the thread portion of this gate at `gh pr merge` time — it blocks on unresolved threads and on a head SHA whose checks are still in flight; reading CodeRabbit's own commit status per (a) and the (c) re-review check stay session habits.
...
- **Linux + macOS are Supported (Linux CI-gated per-PR, macOS nightly); Windows is alpha (HIMMEL-3125).** Shell scripts must be **bash 3.2-safe** (macOS ships bash 3.2 — no `mapfile`, associative arrays, `${var^^}`, GNU-only `sed -i`/`date` flags) and shellcheck-clean — that requirement stays, it is for macOS, with the documented exceptions in [`docs/setup/new-machine.md`](setup/new-machine.md#scripts-requiring-bash-4) (scripts that intentionally require bash 4+ and tell macOS users to `brew install bash`). Hooks that run in a **PowerShell context** (e.g. SessionEnd) still need a `.ps1` twin changed in **lockstep** with the `.sh` — those twins are functionally required, not optional, since Windows dispatches that hook lifecycle via PowerShell. For an ordinary new `scripts/**/*.sh` that is *not* one of those PowerShell-dispatched hooks, a `.ps1` twin is now **optional** — add one only when you're actually working the Windows path; `scripts/hooks/check-new-shell-platform-guard.sh` warns rather than blocks. Handle portable paths/env (`HOME` vs `USERPROFILE`/`LOCALAPPDATA`, forward-slash normalization, never hardcode `/tmp` vs `%TEMP%`). Test on the platforms the diff touches and attest (`Platforms tested: <os>` pre-push gate); when you can only test one, say so. A single-platform script breaks adopters silently on the platforms that matter — Linux and macOS.
...
- **Implementor tier floor — the smallest lane never ships behavioral code.** Even a transcription-grade, fully-specified plan can come back subtly wrong from the cheapest model tier (a dead or misplaced assertion that review must then catch — costing more than the tier delta saved). Reserve the smallest tier for verbatim single-block patch application and bulk mechanical non-code work; the next tier up is the real floor for anything that creates or edits code with behavioral intent.
