...
- Operator habits with no structural home, inlined:
  ...
  - **A failed CodeRabbit CLI round is a MISSING signal, never a clean one.** `scripts/cr/coderabbit-review.sh` fails OPEN, and every failure shape emits ZERO finding rows — so a naive "0 findings ⇒ clean" reads a dead review as convergence. Gate each round on `{"type":"complete","status":"review_completed"}` in the output AND a `panel-availability: coderabbit ok` line, never on the finding count. Failure shapes seen live: rate limit → `unavailable (rc=1)` with a recoverable `waitTime` in stdout; timeout → `unavailable (timeout 900s)` on large diffs (roughly 800 lines can exceed the default — re-run with `CODERABBIT_TIMEOUT_SECS=2400`). Treat `unavailable` as UNKNOWN and re-establish the signal; the GitHub App's AUTOMATIC reviews run on a separate rate-limit surface and keep working while the CLI is limited (observed live), and the App is the authoritative gate surface (it is what `check-ci.sh` reads, and it auto-resolves its own threads once it sees the fix commit).
  - **Budget the free CodeRabbit tier — rounds are scarce.** The free tier rate-limits after ~4 rounds in quick succession, and a rate-limited ATTEMPT appears to EXTEND the window (observed: honouring the reported `waitTime` exactly still bounced and pushed the wait OUT) — wait approximately 1.5× the reported `waitTime`, never poll-retry. Batch ALL known fixes into one push before spending a round; iterate on the free signals (critic panel, `check-ci.sh`, local suites) and spend CodeRabbit rounds on confirmation; EXPLICIT App triggers (`@coderabbitai full review`) and CLI rounds draw on the same org-level allowance — don't spend both on one head (the App's automatic incremental reviews are the surface that keeps working when the CLI rate-limits); and leave quota headroom for the public-propagation loop. This project stays on the free tier by policy — budget rounds instead of proposing the usage-based upgrade the rate-limit payload advertises.
  ...
  - **Adversarial review never converges to zero — gate on the standard panel.** An adversarially-prompted critic is prompted to maximally find no-ship issues, so it may keep surfacing increasingly narrow or pre-existing findings across rounds — 13 rounds observed on one diff, floored at one [medium]/round of ever-finer edges while the STANDARD critic panel returned 0/0/0 on the same diff. Run the adversarial pass ~3–5 rounds for depth and fix every CONFIRMED finding, but gate CONVERGENCE on the standard panel clean + CodeRabbit clean, not adversarial-zero; once severity floors at narrow [medium] refinements, stop, record the residual mentions as "further hardening", and ship.
  - **CodeRabbit quiet incrementals post NO review object.** The GitHub App re-reviews every push incrementally, but a clean incremental (zero new actionable findings) posts no new review object — the only signals are the `CodeRabbit` check run on the head SHA (`completed`/`success`) and the updated summary-comment timestamp; `@coderabbitai review` on an already-reviewed commit replies "Review finished" and does nothing (it exists for paused automatic reviews). Never poll for a new review object after a fix push. The re-review-clean gate = (a) check run green on the head SHA, (b) zero unresolved review threads (GraphQL `reviewThreads.isResolved`), (c) the actionable findings from the latest non-empty review cycle addressed (a clean incremental creates no new review object to check). When fixing any CR finding, RESOLVE its review thread (GraphQL `resolveReviewThread`) as part of the fix — a fixed-but-unresolved thread still fails gate (b) and stalls the merge. Structural backstop: the `block-unresolved-cr-merge` hook enforces the thread portion of this gate at `gh pr merge` time — it blocks on unresolved threads and on a check run still in-flight on the head SHA; verifying the check run's `success` conclusion and the (a)/(c) re-review reading stay session habits.
