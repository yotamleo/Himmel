...
- **Linux + macOS are Supported (CI-gated); Windows is alpha (HIMMEL-3125).** Shell scripts must be **bash 3.2-safe** (macOS ships bash 3.2 — no `mapfile`, associative arrays, `${var^^}`, GNU-only `sed -i`/`date` flags) and shellcheck-clean — that requirement stays, it is for macOS. Hooks that run in a **PowerShell context** (e.g. SessionEnd) still need a `.ps1` twin changed in **lockstep** with the `.sh` — those twins are functionally required, not optional, since Windows dispatches that hook lifecycle via PowerShell. For an ordinary new `scripts/**/*.sh` that is *not* one of those PowerShell-dispatched hooks, a `.ps1` twin is now **optional** — add one only when you're actually working the Windows path; `scripts/hooks/check-new-shell-platform-guard.sh` warns rather than blocks. Handle portable paths/env (`HOME` vs `USERPROFILE`/`LOCALAPPDATA`, forward-slash normalization, never hardcode `/tmp` vs `%TEMP%`). Test on the platforms the diff touches and attest (`Platforms tested: <os>` pre-push gate); when you can only test one, say so. A single-platform script breaks adopters silently on the platforms that matter — Linux and macOS.
