...
- Operator habits with no structural home, inlined:
  ...
  - **CodeRabbit is best effort (HIMMEL-3360).** It is one CI-only reviewer beside the local critic panel, never a bottleneck. The merge gate is CI green + zero unresolved review threads (any author) + the `/pr-check` panel. A CodeRabbit finding that EXISTS — an unresolved thread, an undisposed outside-diff body finding — still blocks; CodeRabbit absent, pending, skipped, stale or rate-limited never blocks, and nothing sequences, holds or re-runs work on its account. Never run it locally. This is structural, not prose: `scripts/hooks/check-cr-best-effort.sh` (pre-commit) rejects tracked text that schedules or holds work on CodeRabbit, and `check-ci.sh` / `cr-merge-gate.sh` print its missing signal as a NOTE instead of failing.
  - **A green rollup is NOT a clean review (HIMMEL-2704).** The App is the only surface (the CLI round is gone). A green check-run rollup sat over six unresolved review THREADS on PR #2209 (2026-09-06), and a `success` commit status whose description says `Review skipped` reviewed nothing. Gate each round on `check-ci.sh` exit 0 — zero unresolved threads at the head — never on a finding count, a green tick, or the absence of a comment.
  - **Loop review threads to convergence.** This is structurally guarded: `check-ci.sh` exit 3 = checks green but unresolved review threads / changes-requested block the merge — lean on that backstop, not on memory. After any fix+push, a prior clean round is STALE for the new head; re-converge. Also: the `Mergeable` check run lints ONLY the commit message + PR title — never cite it as evidence of CI health, review convergence, or merge readiness.
  - **Adversarial review never converges to zero — gate on the standard panel.** An adversarially-prompted critic is prompted to maximally find no-ship issues, so it may keep surfacing increasingly narrow or pre-existing findings across rounds — 13 rounds observed on one diff, floored at one [medium]/round of ever-finer edges while the STANDARD critic panel returned 0/0/0 on the same diff. Run the adversarial pass ~3–5 rounds for depth and fix every CONFIRMED finding, but gate CONVERGENCE on the standard panel clean + zero unresolved review threads on the PR (check-ci exit 0), not adversarial-zero; once severity floors at narrow [medium] refinements, stop, record the residual mentions as "further hardening", and ship.
  - **CodeRabbit quiet incrementals post NO review object.** The GitHub App re-reviews every push incrementally, but a clean incremental (zero new actionable findings) posts no new review object — the only signals are CodeRabbit's **commit STATUS** on the head SHA and the updated summary-comment timestamp. **It is a status, not a check-run** (verified live on five consecutive PRs — see `scripts/lib/cr-signal.sh`, which matches it by creator id on `/commits/<sha>/statuses` for exactly this reason); `gh pr checks` shows it only because `statusCheckRollup` merges statuses and check-runs, so a `select(.name=="CodeRabbit")` over `.check_runs[]` matches NOTHING. Do not poll for a check run or a new review object — the status is informational (HIMMEL-3360), never a gate input. And do NOT read that status as the verdict: it says a review concluded, not that its findings were dealt with — the THREAD QUERY is the truth, and a `pass` rollup over unresolved threads is exactly what shipped six of them on PR #2209 (HIMMEL-2704). The re-review-clean gate = (a) zero unresolved review threads (GraphQL `reviewThreads.isResolved`), (b) the actionable findings from the latest non-empty review cycle addressed (a clean incremental creates no new review object to check). When fixing any CR finding, RESOLVE its review thread (GraphQL `resolveReviewThread`) as part of the fix — a fixed-but-unresolved thread still fails gate (a) and stalls the merge. Structural backstop: the `block-unresolved-cr-merge` hook enforces the thread portion of this gate at `gh pr merge` time — it blocks on unresolved threads and on a head SHA whose checks are still in flight; the (b) re-review check stays a session habit.
  ...
  - **Machine-generated PRs (dependabot dep-bumps, regenerated-artifact
    ...
    review` there. An absent review blocks nothing on any PR (HIMMEL-3360),
    so the ordinary merge gate — CI green + zero unresolved threads + the
    panel — is the whole gate for them too.
