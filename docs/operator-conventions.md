...
- **File deferred items as real tickets too.** Every work item — including DEFERRED / "Won't Do" / timeboxed eval items — gets a real Jira running number (`HIMMEL-N` / `LUNA-N`), never an internal `#N` placeholder. File-then-close beats file-never-and-forget.
