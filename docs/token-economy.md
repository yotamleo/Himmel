...
| Conversational / verbose output | **Caveman plugin** (never code/precise; vendor-claimed savings vary by level) | incumbent, grandfathered (invariant 3). **No model-tier carve-out (HIMMEL-699 audit):** the SessionStart/UserPromptSubmit hooks (`JuliusBrussee/caveman`, external — not vendored, so himmel can't patch its source) apply uniformly to whatever model is running the session; nothing in the hook checks for Fable. Fable-tier dispatches — including overnight/armed relaunches, which `arm-resume.sh` launches as a plain `claude $prompt` with no `CAVEMAN_DEFAULT_MODE` override — are exposed to compression if caveman is ever toggled on (currently mitigated only by the operator's own `~/.config`/`%APPDATA%\caveman\config.json` `defaultMode: "off"`, not by a structural exemption). Compression cutting hedging/detail is the wrong direction for the judgment-tier model — do not flip caveman on globally, and do not add a repo-local `.caveman.json`/`.caveman/config.json` to himmel, without first adding a model-tier exemption (e.g. an `arm-resume.sh`-side `CAVEMAN_DEFAULT_MODE=off` export for relaunched sessions) |
