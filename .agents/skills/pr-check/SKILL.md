---
name: pr-check
description: Panel-only CR gate for Codex — run the critic panel, clear the CR marker when clean. /pr-check.
---
...
- `scripts/cr/pr-check-external.sh` (the Claude-FREE ship lane) → **"codex
  ...
  longer reach (CodeRabbit is App-only since HIMMEL-2704 and the panel is
  codex-only), so it REFUSES such diffs outright — they go through the
  interactive Claude `/pr-check`.
...
into the Claude runbook's step-4 cross-check and orphan reconciliation. Both the printed context and
...
every consumer of this range must review the SAME range, so the base
...
Claude backstop, so it leaves zero
...
the bypass.
    ...
    # stopping — so the run continued on inputs already known to be stale.
...
continue.** The checkout is no longer at the SHA step 0 captured, or the
...
point: every later step would otherwise record a review of inputs already known
to be stale.
...
## 3.5. CodeRabbit — the GitHub App, not a local pass (HIMMEL-2704)

**There is no CodeRabbit step in this lane.** CodeRabbit is the GitHub App
and reviews the PR only after it exists, so nothing here invokes it. The CLI
wrapper (`scripts/cr/coderabbit-review.sh`) and the conserve-or-run gate
(`scripts/cr/coderabbit-gate.sh`) that used to run in this slot are DELETED,
along with `CODERABBIT_BIN`, `CODERABBIT_WSL`, `CODERABBIT_TIMEOUT_SECS` and
`CODERABBIT_CLI_DISABLE` (operator ruling 2026-09-07: the App is the gate,
and every finding is commented on and tracked on the PR in GitHub).
...
2. Normalize every blocking candidate into the panel bullet contract before
   ...
   bullet under the panel's `## Critical Issues` / `## Important Issues`.
   Map the section/severity to `crit` / `imp`, extract the slug from the
...
3. Append **all finding rows before availability rows**. This ordering fails
   ...
   **An `amend` refusal is a real signal — never fall back to `finding`.** It
   ...
   producer's self-write did not happen (an unpinned producer run, or a failed
   ...
   Then, for each parsed panel availability row, run and check:
...
- `panel_rc = 7` (input-pin mismatch, HIMMEL-1175) never reaches this step —
  step 3 stops the run the moment it sees it, so nothing downstream records a
  review of inputs already known to be stale. Restated here only so the
...
- Else require exactly one panel count line matching each expected format:
  ...
  - If `C = 0` AND `I = 0`, invoke the sanctioned chokepoint and inspect its
    exit status:
  ...
  - Otherwise: **retain** the marker and report the panel's Critical/Important
    findings verbatim for the operator to fix and re-run.
