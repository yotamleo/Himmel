...
#!/usr/bin/env bash
# Generated by scripts/hooks/install-nostash-hooks.sh — do not edit by hand.
# No file-list gate here (unlike pre-commit above): commit-msg validation
# checks the MESSAGE, not staged file content, so it must still run on a
# deletion-only commit — a `--diff-filter=ACMR` files array can be empty then.
set -euo pipefail
cd "$(git rev-parse --show-toplevel)"
