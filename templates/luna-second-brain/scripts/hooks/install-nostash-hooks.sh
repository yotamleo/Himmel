...
# Is $1 a hook WE generated? Requires the marker as an EXACT line-2 match,
# not just present somewhere in the file (a foreign hook that merely quotes
# or embeds that sentence elsewhere must still be treated as foreign, backed
# up, and not silently overwritten).
hook_is_ours() {
  local f="$1"
  [ "$(sed -n '2p' -- "$f" 2>/dev/null)" = "# Generated by scripts/hooks/install-nostash-hooks.sh — do not edit by hand." ]
}

# Would backing up $1 clobber an earlier backup? Checked for BOTH hook paths
# BEFORE either is touched (below) -- checking and moving in the same pass
# per-file left the repo with NEITHER hook installed whenever pre-commit's
# backup succeeded but commit-msg's then hit this refusal and exited.
assert_backup_wont_clobber() {
  local f="$1"
  if { [ -e "$f" ] || [ -L "$f" ]; } && ! hook_is_ours "$f"; then
...
backup_foreign_hook() {
  ...
  # A symlink judged "ours" above (line-2 match read THROUGH the link,
  # against whatever it points at) is left untouched by the branch above --
  # but the generation step below writes via `cat >`, which follows any
  # symlink and would overwrite that link's target, inside or outside the
  # vault. Drop the link itself so the write always lands on a fresh regular
  # file at $f.
  if [ -L "$f" ]; then
    rm -f "$f"
  fi
}
assert_backup_wont_clobber "$hooks_dir/pre-commit"
assert_backup_wont_clobber "$hooks_dir/commit-msg"
