...
if ! git -C "$root" rev-parse --show-toplevel >/dev/null 2>&1; then
  echo "error: $root (derived from this script's own location) is not a git repository" >&2
  exit 1
fi
...
# Resolve pre-commit ONCE, here, and bake the verified choice into the
# generated hooks below -- the way pre-commit's own hook bakes INSTALL_PYTHON.
# Re-resolving from PATH at commit time (the previous approach) breaks under
# a GUI/Obsidian-Git commit, which often runs with a minimal PATH.
#
