...
#             a SUITE_LOCK_WAIT budget ran out with the lock still held (or
#             this run still behind an older live waiter in the FIFO queue,
#             HIMMEL-2623) — distinct from the instant refusal above because
#             it means "waited, then gave up," not "declined to wait." 3, 4
#             and 5 are NOT test verdicts: no tally is a result and no
#             after-report is posted, so a caller must not read any of them
#             as "some suites failed" — 1 remains the only code that means
#             that.
#
...
#                      VISIBILITY below. Waiters queue FIFO (HIMMEL-2623): a
#                      ticket under SUITE_LOCK_DIR.q orders waiters by arrival,
#                      so an older waiter is never jumped by a newer one — see
#                      QUEUE FAIRNESS below. A budget that runs out still
#                      queued, or against a still-held lock, exits 5 (not the
#                      instant-refusal 2) with one "LOCK-WAIT EXPIRED after
#                      <N>s — run NOT executed" line on stdout. A NO-WAIT run
#                      (this knob unset or 0) also refuses (rc 2) when the
#                      queue is non-empty, even if the lock itself is free —
#                      an ad-hoc run must yield to whoever already queued
#                      rather than cut in front of them (round 4); it never
#                      takes a ticket of its own, so this never queues it

#   SUITE_LOCK_WAIT_INTERVAL
#                      seconds between those WAITING: lines (default 60)
#   SUITE_QUEUE_TTL    seconds since a queued waiter's ticket was last SEEN
#                      (refreshed on every poll, HIMMEL-2623 round 7) before it
#                      is presumed abandoned — NOT since it first arrived, and
#                      NOT overridden by a confirmed-alive pid: a waiter that
#                      is alive but has stopped refreshing (stopped, wedged,
#                      hung) is reclaimed past this TTL exactly like a dead
#                      one; a HEALTHY waiter that keeps polling is never
#                      reclaimed on age alone, no matter how long the total
#                      wait. (Contrast SUITE_LOCK_TTL above, which DOES
#                      override a confirmed-alive pid on purpose — a wedged
#                      HOLDER blocks everyone and must stay reclaimable; a
#                      ticket holds no resource, so the same override there
#                      would only break fairness for whoever is queued
#                      behind it.) Effectively floored at a few multiples of
#                      SUITE_LOCK_WAIT_INTERVAL so a waiter configured with a
#                      long interval cannot go stale between its OWN polls;
#                      default SUITE_LOCK_TTL, 4h.
...
# Staleness TTL for a FIFO queue ticket (HIMMEL-2623), mirroring SUITE_LOCK_TTL
# for the same reason: a dead waiter's ticket must not wedge every waiter that
# arrived LATER behind it in the queue forever. Defaults to SUITE_LOCK_TTL
# itself (not a second hardcoded 14400) so a site that already tuned the
# lock's own TTL gets a matching queue TTL without having to set both. Goes
# through _suite_num like every other duration knob here — a malformed or
# zero value falls back rather than silently disabling the tickets' own
# dead-waiter reclaim.
SUITE_QUEUE_TTL=$(_suite_num SUITE_QUEUE_TTL "${SUITE_QUEUE_TTL:-$SUITE_LOCK_TTL}" "$SUITE_LOCK_TTL")

# Nanosecond-resolution capability, probed ONCE here (HIMMEL-2623 round 6,
# codex-2) — never inside _suite_lock_now_ns itself. That function is always
# invoked as $(_suite_lock_now_ns): a lazy probe-and-cache INSIDE it would run
# in the command-substitution SUBSHELL and be discarded the instant that
# subshell exits, so every single call would re-probe forever — two forks per
# branded timestamp instead of one. Resolving it here, in this shell, at the
# same place every other lock/queue knob is resolved, makes the cache actually
# persist for the life of this process. See _suite_lock_now_ns for the BSD
# literal-"%N" detection this feeds.
case "$(date +%N 2>/dev/null)" in
  ''|*[!0-9]*) _suite_lock_ts_ns_ok=0 ;;
  *) _suite_lock_ts_ns_ok=1 ;;
esac
...
# SUITE_QUEUE_STALE_AFTER — the EFFECTIVE window judged against a ticket's
# heartbeat (HIMMEL-2623 round 7, codex-1's sanity check on the interaction
# between these two knobs). SUITE_LOCK_WAIT_INTERVAL has no upper bound, so a
# waiter configured with an interval longer than SUITE_QUEUE_TTL would go
# stale BETWEEN ITS OWN POLLS — pruned by a peer, restored on its very next
# poll (the round-4 restore path still catches this), but visibly ABSENT
# from the queue for the gap in between, which is exactly the jump window
# this clamp exists to close. Floored at 3x this process's own
# SUITE_LOCK_WAIT_INTERVAL, comfortably more than the one interval a healthy
# waiter needs to refresh; only ever WIDENS SUITE_QUEUE_TTL, never narrows
# it, so an ordinary (interval well under TTL) configuration is unaffected.
#
# THIS process's own value ONLY (round 8, codex-1): a ticket is branded with
# the value computed here at join time and carries it in its own owner file
# from then on, because the expiry window belongs to the ticket's OWNER
# (who knows its own interval), not to whichever OTHER process happens to
# be pruning it later — a fast-polling reader applying ITS OWN short window
# to a slow-polling owner's ticket would prune a perfectly healthy waiter.
# See _suite_lock_queue_resolve_stale_after, which is what every judgement
# of ANOTHER ticket actually reads.
SUITE_QUEUE_STALE_AFTER=$(( SUITE_LOCK_WAIT_INTERVAL * 3 ))

# SUITE_QUEUE_STALE_AFTER_CEILING — the ABSOLUTE, READER-INDEPENDENT ceiling
# on a ticket's own declared `stale_after` (HIMMEL-2623 round 9, codex-1).
# This must NEVER be computed from the READER's own configuration — round 8
# tried exactly that (100x the reader's own SUITE_QUEUE_STALE_AFTER) and it
# reintroduced the very bug this mechanism exists to prevent: a reader with
# a tiny SUITE_QUEUE_TTL/SUITE_LOCK_WAIT_INTERVAL computes a tiny window for
# ITSELF (e.g. 3s), so even that reader-relative cap (300s) then REJECTS a
# perfectly healthy DEFAULT-configuration ticket's real stale_after=14400,
# falling back to the reader's own 3s and pruning it between its normal
# polls — a waiter jumped, mid-flight, by the very guard meant to protect
# it. A fixed constant has no such failure mode: every reader validates
# against the SAME ceiling regardless of its own knobs, so it can only ever
# reject a value that is absurd on its own terms, never one that is merely
# larger than some OTHER process's unrelated configuration. A few days in
# seconds cannot plausibly shorten any real lease — SUITE_LOCK_TTL's own
# default (4h) is an order of magnitude under this — while still bounding a
# garbage or malicious value to a finite ceiling, unlike a reader-relative
# multiplier, whose effective ceiling shrinks right along with whatever tiny
# window the reader itself happens to be configured with.
SUITE_QUEUE_STALE_AFTER_CEILING=259200  # 3 days
[ "$SUITE_QUEUE_STALE_AFTER" -lt "$SUITE_QUEUE_TTL" ] && SUITE_QUEUE_STALE_AFTER="$SUITE_QUEUE_TTL"
...
# --------------------------------------------------------------------------
# FIFO waiter queue (HIMMEL-2623).
#
# suite_lock_acquire_waiting used to poll suite_lock_acquire on a plain
# interval: whoever's poll happened to land the instant the lock freed won,
# with no memory of who had been waiting longest. A HIMMEL-2583 runner queued
# 1h49m and was jumped by three newer runners in a row, then expired unrun —
# starved by its own younger siblings, not by the holder. The fix orders
# waiters: each takes a TICKET on arrival, and only the ticket with the
...
#
# TICKET: a directory ${SUITE_LOCK_DIR}.q/<pid> — THIS PROCESS'S OWN PID, not
# an allocated sequence number — containing an "owner" file with pid=/host=
# FIELD NAMES the lock itself uses, so _suite_lock_owner_field,
# _suite_lock_same_host and _suite_lock_probe_pid all read a ticket exactly as
# they read a lock, with no second parser to keep in sync, PLUS two ticket-only
# fields the lock does not have: `started` (ORDER, below) and `seen`
# (LIVENESS, below) — deliberately two separate fields, because they answer
# two separate questions and conflating them is exactly the bug HIMMEL-2623
# round 7 fixed (see LIVENESS). The UNIT of both is the highest resolution
# this host's `date` can give — never the lock's whole-seconds `started` unit;
# the two are never compared against each other, only ticket-against-ticket,
# so the differing unit is safe.
#
# ALLOCATION (HIMMEL-2623 round 3, codex-1): an earlier version of this scheme
# derived the ticket name from a SCAN — read every existing number, mkdir
# max+1 — and its own comment claimed "a fresh mkdir always sorts strictly
# after every ticket that existed at scan time." That claim is false the
# moment a LOWER ticket can be freed between the scan and the mkdir: a
# contender scans an empty queue (max=0, so it will try 1), gets descheduled
# — ordinary on a 32-thread box running several suites at once — and in that
# window two other waiters take tickets 1 and 2, the first of them acquires
# the lock and calls suite_lock_queue_leave, freeing ticket 1; the stalled
# contender's delayed `mkdir .../1` now SUCCEEDS. It ends up holding ticket 1
# while a waiter that arrived earlier holds ticket 2 — jumped, by the
# allocator this PR exists to fix. The bug was coupling IDENTITY (which
# directory name we get) to a value (max+1) that can change between reading
# it and acting on it.
#
# The fix decouples the two. Identity is `mkdir "$qdir/$$"` — no scan, no
# max, no retry-on-collision loop, because a live pid is unique on this host
# by construction; the ONLY way that mkdir can fail is a leftover directory
# from a PAST process that happened to reuse our exact pid, which is exactly
# the shape _suite_lock_queue_ticket_dead already judges (see PRUNING) — dead,
# it is pruned and the mkdir retried once; live, that would have to be our
# own still-running self, which cannot happen, so this logs a NOTE and
# proceeds ticketless (the same fail-open posture as every other allocation
# hiccup here).
#
# ORDER comes from `started` in the owner file — written ONCE, at join time,
# and never touched again. The GUARANTEE, stated precisely (HIMMEL-2623 round
# 4, codex-2): exact FIFO at the branded resolution — nanoseconds since epoch
# where the platform's `date` supports it, whole seconds otherwise — with
# ties AT THAT RESOLUTION broken deterministically by pid. Two arrivals
# landing in the same branded tick are not asserted to be "simultaneous" in
# real time (an earlier round of this comment made exactly that unqualified
# claim, which is not a fact this code can know); they are merely ordered
# consistently, the same way _suite_lock_reclaim's takeover race is decided
# by whoever wins an atomic mkdir rather than by any claim about real time.
# See _suite_lock_now_ns / _suite_lock_ts_ns below for the resolution probe
# and the normalisation every comparison runs through — a ticket branded at
# second-resolution (an old process mid-rollout, or a platform without %N)
# and one branded in nanoseconds must never be compared as raw integers, or
# the second-resolution one looks astronomically newer than it is. A waiter
# that stalls after joining cannot move its own position: its `started` was
# fixed the instant it branded its ticket, so nothing it does later —
# including the very stall that broke the old allocator — can make it look
# like it arrived earlier OR later relative to anyone else already queued.
# That is the property the old scheme lacked and this one restores. `started`
# is NEVER touched again after the ticket's initial brand, including by the
# per-poll heartbeat below — LIVENESS refreshes a separate field, precisely
# so it cannot perturb ORDER.
#
# LIVENESS is `seen` — refreshed by the ticket's OWNER on every poll
# (_suite_lock_queue_brand, called from suite_lock_queue_is_our_turn), not
# `started`. HIMMEL-2623 round 4 tried to answer "is this ticket still worth
# honouring" by exempting any ticket whose pid the probe confirmed alive from
# the TTL, full stop — which fixed a real fairness bug (a long-waiting but
# healthy runner getting pruned on age alone) by introducing a wedge (a
# STOPPED or hung-but-still-resident process's ticket then survived FOREVER,
# since a confirmed-alive pid never ages out). Round 7 (codex-1) separates
# the two questions instead: `started` answers "when did this arrival
# happen" and never changes; `seen` answers "is the owner actively still
# here" and changes every poll. The TTL backstop judges `seen`, so a healthy
# waiter — which refreshes every SUITE_LOCK_WAIT_INTERVAL, always well under
# SUITE_QUEUE_TTL (see SUITE_QUEUE_STALE_AFTER's clamp) — is never reclaimed
# no matter how long it has been queued, while a waiter that goes quiet
# (stopped, wedged, crashed) is reclaimed once its LAST heartbeat is stale,
# whether or not its pid still answers a probe. `_suite_lock_queue_brand`
# rewrites `seen` (and re-affirms `pid`/`host`/whichever `started` it is
# given) via a temp file plus one atomic `mv` — never a partial in-place
# edit — so a crash mid-refresh leaves the OLD owner file completely intact,
# never a corrupt or multi-file ticket that would make _suite_lock_drop's
# "lone owner file" safety check refuse to ever clear a since-dead ticket.
#
# PRUNING is why this survives a dead OR wedged waiter with NO manual sweep,
# the same property the lock itself already has for a dead HOLDER (though
# the lock's own TTL, unlike this one, DOES override a confirmed-alive pid —
# see LIVENESS above for why that is deliberately different here): a ticket
# is dead when either (a) its host matches this host and the pid probe
# reports CONFIRMED dead (ESRCH — a merely REFUSED probe, e.g. EPERM, proves
# nothing and must NOT count as dead, identically to the lock's own probe
# reading), or (b) its `seen` (falling back to `started` when `seen` is
# absent — a ticket branded by a pre-round-7 build mid-rollout, judged
# exactly as it always was rather than treated as immortal for lacking a
# field it never wrote) is at least SUITE_QUEUE_STALE_AFTER old, or (c) it
# carries no usable timestamp AND no probeable same-host pid — there is no
# other event that will ever clean such a ticket up, so it is treated as
# abandoned on sight, mirroring exactly how suite_lock_acquire treats an
# undated foreign holder. Pruning runs before every poll's turn-check, so a
# dead or wedged waiter is cleared inline by the very next live waiter to
# look — no cron, no operator.
#
# COEXISTENCE with the OLD (pre-HIMMEL-2623) code, which is still running
# elsewhere on this machine mid-rollout, holds by construction:
#   (i)   A NEW waiter behind an OLD holder still takes the only ticket in an
#         empty queue (the OLD holder never creates one), is first and only
#         in line, attempts suite_lock_acquire unmodified, sees the held
#         lock, and waits exactly as before. It never touches the holder's
#         directory — only SUITE_LOCK_DIR/owner is ever read, the same read
#         suite_lock_acquire always performed.
#   (ii)  The lock directory gains NO new files: the queue lives at a
#         SEPARATE sibling path, never inside SUITE_LOCK_DIR. An OLD
#         acquirer's own _suite_lock_dir_empty / "has no owner file and is
#         NOT empty" safety checks glob only inside SUITE_LOCK_DIR, so the
#         .q sibling is invisible to them — it cannot be mistaken for lock
#         content by code that has never heard of it.
#   (iii) An OLD waiter (still running the pre-fix poll loop) takes no
#         ticket and races exactly as it does today — no worse than the
#         status quo, and self-healing once every runner on the machine is
#         on the new code.
# --------------------------------------------------------------------------

# suite_lock_ticket_dir — this run's own ticket (${SUITE_LOCK_DIR}.q/$$), or
# empty when running without one (no wait requested, or allocation gave up).
# Read by the EXIT trap via suite_lock_queue_leave, the same belt-and-braces
# pattern suite_lock_release already uses for the lock itself: a killed
# waiter's ticket must not linger and block everyone behind it until
# SUITE_QUEUE_TTL. No separate "ticket number" variable — the pid in
# suite_lock_ticket_dir's own name IS the identity, and its `started` is read
# fresh from its owner file wherever it matters (see suite_lock_queue_is_our_turn).
suite_lock_ticket_dir=""

# suite_lock_ticket_started — the EXACT `started` value branded into our own
# ticket at join time, cached here (HIMMEL-2623 round 6, codex-1). Another
# runner's suite_lock_queue_prune can delete our ticket out from under us
# once SUITE_QUEUE_TTL passes (a real config: this suite's own header queues
# an after-report with SUITE_LOCK_WAIT=21600, well past the 4h default TTL).
# Without this cache, suite_lock_queue_is_our_turn would read our own
# now-missing owner file, find nothing, and fail OPEN — which for a WAITER
# means jumping every older ticket still queued, the exact failure this
# whole mechanism exists to prevent. With it, a vanished ticket is
# RE-CREATED at this ORIGINAL stamp, not a fresh one, so we keep the queue
# position we have been waiting for instead of going to the back of the
# line — see suite_lock_queue_is_our_turn.
suite_lock_ticket_started=""

# suite_lock_queue_blocker_pid / _host — set by suite_lock_queue_is_our_turn on
# every call: cleared when it says "our turn" (0), named to the EARLIEST live
# ticket ahead of ours when it says "not yet" (1). suite_lock_acquire_waiting
# reads these right after its most recent turn-check to decide whether a
# refusal was the LOCK's doing or the QUEUE's — see the GAVE UP / WAITING:
# wording split there (HIMMEL-2623 round 2).
suite_lock_queue_blocker_pid=""
suite_lock_queue_blocker_host=""

# _suite_lock_now_ns — nanoseconds since epoch, on stdout, as a plain decimal
# string (HIMMEL-2623 round 4, codex-2). Prefers GNU `date +%s%N`; falls back
# to whole seconds scaled to the SAME unit (`*10^9`, done by string
# concatenation, never multiplication, so there is no arithmetic overflow to
# reason about) when the platform's `date` does not support %N.
#
# BSD/macOS `date` does not error on an unknown format directive — it prints
# the directive back LITERALLY, so `date +%N` there prints the three
# characters "%N", exit status 0. The exit status therefore cannot tell the
# two platforms apart; only the OUTPUT can — but that probe is NOT run here.
# It runs exactly once, eagerly, near the top of this file where every other
# lock/queue knob is resolved, into _suite_lock_ts_ns_ok: this function is
# always called as $(_suite_lock_now_ns), and a probe-and-cache done INSIDE
# it (an earlier round of this fix) runs in that command substitution's
# SUBSHELL, so the cached value is discarded the instant the subshell exits —
# every call would re-probe forever (HIMMEL-2623 round 6, codex-2). Reading a
# value resolved in THIS shell, never writing one here, is what makes the
# cache actually persist for the life of the process.
_suite_lock_now_ns() {
  if [ "$_suite_lock_ts_ns_ok" -eq 1 ]; then
    date +%s%N
  else
...
# _suite_lock_ts_ns <raw> — normalise a `started` value already read from a
# ticket's owner file to NANOSECONDS on stdout, so two tickets branded by
# different code (a not-yet-upgraded process mid-rollout, or a platform
# without %N) can be compared as the same unit — never as raw integers, which
# would make a whole-seconds value look astronomically OLDER than a
# nanosecond one (its number is ~9 orders of magnitude smaller) even when it
# was actually branded LATER.
#
# Whole-seconds values are and will remain <= 12 digits for millennia yet (a
# realistic `date +%s` today is 10 digits); a value THIS design ever brands
# is always seconds*10^9, so it is <= 12 digits ONLY for epoch seconds under
# 1000 — 1970-01-01, never a live host's clock. The threshold is comfortably
# between the two shapes this file will ever actually see, so it cannot
# misclassify either one. Empty or non-numeric input returns "0" — the
# caller already validates before calling this; it exists so a defect
# upstream fails safe (sorts as the oldest possible tick) instead of
...
# _suite_lock_queue_resolve_stale_after <raw> — validate a `stale_after`
# value read from a ticket's own owner file (HIMMEL-2623 round 8, codex-1):
# the expiry window belongs to the TICKET (its owner knows its own
# SUITE_LOCK_WAIT_INTERVAL; a pruner judging it does not), not to whoever
# happens to be reading it — a fast-polling reader computing SUITE_QUEUE_STALE_AFTER
# from its OWN short interval and applying that to a slow-polling owner's
# ticket prunes a perfectly healthy waiter out from under it. So the ticket
# carries its OWNER's own window, and this validates it before anyone
# trusts it: a positive integer, capped at the ABSOLUTE, reader-INDEPENDENT
# SUITE_QUEUE_STALE_AFTER_CEILING — see that constant's own comment for why
# it must never be computed from the reader's own configuration (round 9,
# codex-1: a reader-relative cap rejected a healthy DEFAULT-config ticket's
# real stale_after and pruned it). Even a capped value is not immortal,
# since a real ticket's `seen` still has to keep advancing to survive it.
# Anything else — absent, non-numeric, non-positive, or over the ceiling —
# falls back to the READER's own SUITE_QUEUE_STALE_AFTER, the same
# back-compat posture `seen` falling back to `started` already uses: a
# ticket branded by a pre-round-8 build mid-rollout is judged by the
# reader's own window, not treated as immortal for lacking a field it never
# wrote.
_suite_lock_queue_resolve_stale_after() {
  case "$1" in
    ''|*[!0-9]*) printf '%s' "$SUITE_QUEUE_STALE_AFTER"; return 0 ;;
  esac
  local _v=$(( 10#$1 ))
  if [ "$_v" -le 0 ] || [ "$_v" -gt "$SUITE_QUEUE_STALE_AFTER_CEILING" ]; then
    printf '%s' "$SUITE_QUEUE_STALE_AFTER"
  else
    printf '%s' "$_v"
  fi
}

# _suite_lock_queue_brand <ticket-dir> <started> <stale_after> — write (or
# REPLACE) <ticket-dir>/owner with this process's pid/host, the given
# `started` (verbatim — the caller decides whether that is a fresh stamp or
# a cached original, see ORDER/LIVENESS above), the given `stale_after`
# (verbatim — never recomputed here; see
# _suite_lock_queue_resolve_stale_after), and a FRESH `seen` (HIMMEL-2623
# round 7). <ticket-dir> must already exist. Returns 0 on success.
#
# Atomic: written to a temp file NEXT TO, never INSIDE, <ticket-dir> — a
# dotfile directly under the QUEUE dir, invisible to a bare `*` glob and,
# even if it were not, not a digit-only name either, so every
# ticket-enumeration loop here (which filters on "is this a numeric
# directory name") ignores it regardless. It is then moved into place with
# one `mv`, a single atomic rename on any POSIX filesystem. This is the
# difference between "the ticket's `owner` file is
# always either the OLD complete content or the NEW complete content" and a
# partial in-place edit: a crash between the write and the rename leaves the
# temp file as harmless litter beside the ticket (this file's tmp-sweep
# conventions apply, same as any other stray /tmp debris) and the REAL
# `owner` file completely untouched — never a multi-file ticket that would
# make _suite_lock_drop's "lone owner file" safety check refuse to ever
# clear a since-dead ticket, which is exactly what an in-ticket-directory
# temp file would have risked.
_suite_lock_queue_brand() {
  local dir="$1" started="$2" stale_after="$3" tmp
  tmp="$(dirname "$dir")/.brand.$$.tmp"
  if ! printf 'pid=%s\nhost=%s\nstarted=%s\nstale_after=%s\nseen=%s\n' \
      "$$" "$(_suite_lock_host)" "$started" "$stale_after" "$(_suite_lock_now_ns)" \
      > "$tmp" 2>/dev/null; then
    rm -f "$tmp" 2>/dev/null
    return 1
  fi
  if ! mv -f "$tmp" "$dir/owner" 2>/dev/null; then
    rm -f "$tmp" 2>/dev/null
    return 1
  fi
  return 0
...
# suite_lock_queue_join — take this run's ticket at ${SUITE_LOCK_DIR}.q/$$.
# Always returns 0: failure to get a ticket is not failure to run (see
# ALLOCATION above). Prints nothing on success; a NOTE to stderr only when it
# gives up.
suite_lock_queue_join() {
  local qdir="${SUITE_LOCK_DIR}.q"
  local tdir="$qdir/$$"
  local _stamp
  mkdir -p "$qdir" 2>/dev/null || {
    printf 'NOTE: could not create the waiter queue directory %s — proceeding without a FIFO ticket (fairness is a nicety, not a gate).\n' \
      "$qdir" >&2
    return 0
  }
  if ! mkdir "$tdir" 2>/dev/null; then
    # The only way our OWN pid can already have a ticket is a leftover
    # directory from a PAST process that happened to reuse it — judge it
    # exactly like any other ticket. Dead: prune it and take the slot. Live:
    # that would have to be literally this same still-running process, which
    # cannot happen — fail open, ticketless, rather than assert something
    # that should be structurally impossible.
    if _suite_lock_queue_ticket_dead "$tdir" && _suite_lock_drop "$tdir" && mkdir "$tdir" 2>/dev/null; then
      :
    else
      printf 'NOTE: could not allocate a FIFO waiter ticket at %s — proceeding without one (fairness is a nicety, not a gate).\n' \
        "$tdir" >&2
      return 0
    fi
  fi
  # A ticket's `started` is the highest-resolution timestamp this host's
  # `date` can give (see ORDER above, and _suite_lock_now_ns) — deliberately
  # NOT `date +%s` the way the lock's own owner file (and this same file's
  # earlier rounds) brand it. Captured into a local FIRST, not inlined into
  # _suite_lock_queue_brand's own call, so the exact same stamp can be cached
  # into suite_lock_ticket_started below — restoring a vanished ticket at
  # THIS value, never a fresh one, is what keeps a pruned waiter's queue
  # position (see suite_lock_queue_is_our_turn). `stale_after` is simply
  # THIS process's own SUITE_QUEUE_STALE_AFTER — at join time the joiner IS
  # the ticket's owner, so branding its own window onto its own ticket is
  # exactly "the ticket carries its owner's window" (HIMMEL-2623 round 8).
  _stamp=$(_suite_lock_now_ns)
  if ! _suite_lock_queue_brand "$tdir" "$_stamp" "$SUITE_QUEUE_STALE_AFTER"; then
    # Branding failed — an I/O error, not a race: this directory is ours by
    # construction (pid-named), so nobody else could have won it out from
    # under us.
    rmdir "$tdir" 2>/dev/null
    printf 'NOTE: could not brand the FIFO waiter ticket at %s — proceeding without one (fairness is a nicety, not a gate).\n' \
      "$tdir" >&2
    return 0
  fi
  suite_lock_ticket_dir="$tdir"
  suite_lock_ticket_started="$_stamp"
  return 0
}

# suite_lock_queue_leave — drop this run's own ticket, if it holds one.
# Idempotent: safe to call after an ordinary release, after giving up, and
# again from the EXIT trap for a path that hit neither.
# shellcheck disable=SC2317,SC2329
suite_lock_queue_leave() {
  [ -n "$suite_lock_ticket_dir" ] || return 0
  _suite_lock_drop "$suite_lock_ticket_dir"
  suite_lock_ticket_dir=""
  suite_lock_ticket_started=""
}

# _suite_lock_queue_ticket_dead <ticket-dir> — 0 (dead) or 1 (still live). See
# PRUNING above for the three ways a ticket is judged dead; this is that
# judgement, built entirely from primitives the lock itself already uses. Does
# not care whether <ticket-dir>'s name is a pid (current scheme) or a sequence
# number (the scheme this replaced) — it only ever reads the owner file inside.
#
# Judges `seen`, NEVER `started` (HIMMEL-2623 round 7, codex-1) — see
# LIVENESS above. Falls back to `started` when `seen` is absent, which only
# happens for a ticket branded by a pre-round-7 build mid-rollout: it is
# judged by the same rule that always applied to it, not exempted for
# lacking a field it never wrote. The THRESHOLD it is judged against is the
# TICKET's own recorded `stale_after`, not this reader's — see
# _suite_lock_queue_resolve_stale_after (HIMMEL-2623 round 8, codex-1).
_suite_lock_queue_ticket_dead() {
  local dir="$1" o_pid o_host o_started o_seen o_ts o_stale_after now_ns age=-1 dated=0 same_host=0 probe_rc=0
  # A just-mkdir'd ticket gets the same brand-window grace the lock and its
  # takeover claim already get (_suite_lock_wait_brand) — a ticket judged
  # before its owner file exists is a live winner mid-brand, not abandoned.
  _suite_lock_wait_brand "$dir" || return 0
  o_pid=$(_suite_lock_owner_field "$dir/owner" pid)
  case "$o_pid" in *[!0-9]*) o_pid='' ;; esac
  o_host=$(_suite_lock_owner_field "$dir/owner" host)
  o_started=$(_suite_lock_owner_field "$dir/owner" started)
  o_seen=$(_suite_lock_owner_field "$dir/owner" seen)
  o_ts="$o_seen"
  case "$o_ts" in ''|*[!0-9]*) o_ts="$o_started" ;; esac
  o_stale_after=$(_suite_lock_queue_resolve_stale_after "$(_suite_lock_owner_field "$dir/owner" stale_after)")
  # `age` is in SECONDS (o_stale_after is a seconds knob), computed from
  # nanosecond-normalised values (HIMMEL-2623 round 4) so a whole-seconds
  # timestamp (an old process mid-rollout) and a nanosecond one compare
  # correctly instead of one looking astronomically newer than the other.
  now_ns=$(_suite_lock_now_ns)
  case "$o_ts" in
    ''|*[!0-9]*) ;;
    *) age=$(( (10#$now_ns - 10#$(_suite_lock_ts_ns "$o_ts")) / 1000000000 )); dated=1 ;;
  esac
  if _suite_lock_same_host "$o_host" "$(_suite_lock_host)"; then same_host=1; fi
  if [ -n "$o_pid" ] && [ "$same_host" -eq 1 ]; then
    _suite_lock_probe_pid "$o_pid" || probe_rc=$?
    # CONFIRMED dead (ESRCH) only — a refused-for-any-other-reason probe
    # (EPERM etc.) proves nothing and must not be read as dead.
    [ "$probe_rc" -eq 1 ] && return 0
  fi
  # The TTL is a backstop against a STALE HEARTBEAT, unconditionally — no
  # "unless the pid is confirmed alive" exemption. HIMMEL-2623 round 4 added
  # exactly that exemption (judging `started`, the arrival time, which never
  # changes) to stop a long-waiting but healthy runner from being pruned on
  # raw age — and it worked, but only by trading it for a wedge: a STOPPED
  # or hung-but-still-resident process's ticket then survived FOREVER, since
  # a confirmed-alive pid never aged out. Round 7 (codex-1) fixes both at
  # once by judging `seen` instead: a healthy waiter refreshes it every
  # SUITE_LOCK_WAIT_INTERVAL, always comfortably under SUITE_QUEUE_STALE_AFTER
  # (see its clamp), so `age` here never approaches the threshold no matter
  # how long the total wait — no exemption needed. A wedged one stops
  # refreshing, `age` grows past the threshold like any other stale ticket,
  # and it is reclaimed exactly like the lock's own TTL reclaims a wedged
  # HOLDER — the two TTLs are aligned again, not deliberately different: a
  # ticket holding no resource still must not be allowed to sit forever once
  # its owner has gone quiet, the same as a lock. Judged against o_stale_after
  # — the TICKET's own recorded window (HIMMEL-2623 round 8), not this
  # reader's SUITE_QUEUE_STALE_AFTER: a fast-polling reader applying its OWN
  # short window to a slow-polling owner's ticket would prune a perfectly
  # healthy waiter and take its position, the exact bug this field exists to
  # prevent.
  if [ "$dated" -eq 1 ] && [ "$age" -ge "$o_stale_after" ]; then
    return 0
  fi
  if [ "$dated" -eq 0 ] && { [ -z "$o_pid" ] || [ "$same_host" -eq 0 ]; }; then
    # No usable timestamp AND nothing this host can ever probe to confirm
    # life — the same "undatable and unverifiable" bucket suite_lock_acquire
    # treats as abandoned, applied here so a foreign or pid-less ticket does
    # not sit in the queue forever with no event left to ever clear it.
    return 0
  fi
  return 1
}

# suite_lock_queue_prune — remove every dead ticket under ${SUITE_LOCK_DIR}.q.
# Never touches this run's OWN ticket (released explicitly, elsewhere) or a
# non-numeric entry (not one of ours — every ticket directory name is a pid).
suite_lock_queue_prune() {
  local qdir="${SUITE_LOCK_DIR}.q" _f
  [ -d "$qdir" ] || return 0
  for _f in "$qdir"/*; do
    [ -d "$_f" ] || continue
    case "${_f##*/}" in
    ...
    [ "$_f" = "$suite_lock_ticket_dir" ] && continue
    _suite_lock_queue_ticket_dead "$_f" && _suite_lock_drop "$_f"
  done
}

# suite_lock_queue_live_count — the number of LIVE tickets under
# ${SUITE_LOCK_DIR}.q, on stdout, as a plain decimal integer (0 when the
# queue dir does not exist or holds none). Prunes first, same as every other
# queue read here — a dead ticket must not count. READ-ONLY otherwise: this
# never takes a ticket of its own and never creates the queue directory if it
# does not already exist — used by a no-wait run (HIMMEL-2623 round 4,
# codex-1), which never waits and so must never appear IN the queue itself.
suite_lock_queue_live_count() {
  local qdir="${SUITE_LOCK_DIR}.q" _f _n=0
  if [ -d "$qdir" ]; then
    suite_lock_queue_prune
    for _f in "$qdir"/*; do
      [ -d "$_f" ] || continue
...
# suite_lock_queue_is_our_turn — 0 when this run holds no ticket (queue
# disabled or allocation gave up — never block on a fairness nicety we do not
# have) or no LIVE ticket has an earlier arrival than ours; 1 otherwise.
# Prunes first: that inline prune, on every poll, is the entire dead-waiter
# cleanup — see PRUNING above.
#
# "Earlier arrival" is `started` in each ticket's own owner file, NORMALISED
# through _suite_lock_ts_ns before every comparison (HIMMEL-2623 round 4) —
# never compared as raw integers, which would let a whole-seconds value from
# an old process mid-rollout look astronomically OLDER than a nanosecond one
# regardless of which actually branded first — tie-broken by pid at that
# resolution — see ORDER above for the precise guarantee. Our OWN `started`
# is read fresh from our OWN ticket's owner file on every call, never from
# suite_lock_ticket_started directly, so the comparison uses the same source
# of truth as everyone else's: the files on disk. suite_lock_ticket_started
# is consulted ONLY when that read comes back empty (see below) — to
# RESTORE, never to read around, the file. This function also WRITES, every
# call: it refreshes our own ticket's `seen` heartbeat (see LIVENESS above),
# which is not a comparison side effect — it is the whole reason this must
# be called on every poll, not just when actually checking the queue.
#
# Also names the blocker: on every call it (re)sets suite_lock_queue_blocker_*
# to the EARLIEST live ticket found ahead of ours (empty when it is our
# turn), so a caller can tell "blocked by the queue" from "blocked by the
# lock" from the OUTSIDE without this function's internals leaking any
# further than that — see the GAVE UP / WAITING: wording split in
# suite_lock_acquire_waiting.
suite_lock_queue_is_our_turn() {
  suite_lock_queue_blocker_pid=""
  suite_lock_queue_blocker_host=""
  [ -n "$suite_lock_ticket_dir" ] || return 0
  suite_lock_queue_prune

  local my_started_raw my_started
  my_started_raw=$(_suite_lock_owner_field "$suite_lock_ticket_dir/owner" started)
  case "$my_started_raw" in
    ''|*[!0-9]*)
      # Our own ticket vanished — pruned by ANOTHER runner (past
      # SUITE_QUEUE_STALE_AFTER, reachable in the real config: this suite's
      # own header queues an after-report with SUITE_LOCK_WAIT=21600, past
      # the 4h default TTL), or removed some other way. This must NEVER be
      # read as "our turn": that would fail OPEN into the exact failure this
      # mechanism exists to prevent — the longest-waiting runner jumping
      # every older ticket still queued (HIMMEL-2623 round 6, codex-1).
      # RE-CREATE it (mkdir -p first: the directory itself may be gone too)
      # at the cached ORIGINAL suite_lock_ticket_started, never a fresh
      # stamp, so we keep the queue position we have been waiting for
      # instead of going to the back of the line. _suite_lock_queue_brand
      # also stamps a fresh `seen`, same as the ordinary refresh below.
      # Re-created with THIS process's own SUITE_QUEUE_STALE_AFTER, exactly
      # as suite_lock_queue_join does at initial creation: this ticket has
      # no OLD `stale_after` left to read (its whole owner file is gone), so
      # its own current window is the only correct value to give it back.
      my_started_raw=""
      if [ -n "$suite_lock_ticket_started" ]; then
        mkdir -p "$suite_lock_ticket_dir" 2>/dev/null
        if _suite_lock_queue_brand "$suite_lock_ticket_dir" "$suite_lock_ticket_started" "$SUITE_QUEUE_STALE_AFTER"; then
          my_started_raw="$suite_lock_ticket_started"
        fi
      fi
      case "$my_started_raw" in
        ''|*[!0-9]*)
          printf 'NOTE: this run'"'"'s own FIFO ticket at %s vanished and could not be restored — proceeding without queue ordering for this poll (fail-open; retried next poll).\n' \
            "$suite_lock_ticket_dir" >&2
          return 0
          ;;
      esac
      ;;
    *)
      # Ticket present: refresh the LIVENESS heartbeat (HIMMEL-2623 round 7)
      # on every poll, preserving `started` AND `stale_after` byte-identically
      # (round 8) — this must never perturb ORDER or silently swap in a
      # different window, only prove we are still here. `stale_after` is
      # read back and passed through rather than recomputed, exactly as
      # `started` already is: for our own ticket it is always this same
      # process's SUITE_QUEUE_STALE_AFTER anyway, but reading it back keeps
      # this call site mechanically identical to the restore branch above
      # and to _suite_lock_queue_resolve_stale_after's own contract. A
      # failed refresh (e.g. a peer's prune won a race against us in this
      # exact instant) is not fatal: the NEXT poll's read above either sees
      # our ticket gone (routed through the restore branch) or sees it
      # intact and tries again — silently, like every other queue write
      # race in this file.
      _suite_lock_queue_brand "$suite_lock_ticket_dir" "$my_started_raw" \
        "$(_suite_lock_queue_resolve_stale_after "$(_suite_lock_owner_field "$suite_lock_ticket_dir/owner" stale_after)")"
      ;;
  esac
  my_started=$(( 10#$(_suite_lock_ts_ns "$my_started_raw") ))

  local qdir="${SUITE_LOCK_DIR}.q" _f _pid _started_raw _started _host
  local blocked=0 have_best=0 best_started=0 best_pid=0
  for _f in "$qdir"/*; do
    [ -d "$_f" ] || continue
    [ "$_f" = "$suite_lock_ticket_dir" ] && continue
    _pid="${_f##*/}"
    case "$_pid" in ''|*[!0-9]*) continue ;; esac
    _started_raw=$(_suite_lock_owner_field "$_f/owner" started)
    _host=$(_suite_lock_owner_field "$_f/owner" host)
    case "$_started_raw" in
      ''|*[!0-9]*)
        # Cannot tell how old this ticket is: treat it as blocking rather
        # than risk jumping something that might actually be older — the
        # same fail-safe direction the lock's own staleness judgement takes.
        # Reported only when no better (numeric, ranked) blocker is known.
        blocked=1
        if [ "$have_best" -eq 0 ]; then
          suite_lock_queue_blocker_pid="$_pid"
          suite_lock_queue_blocker_host="$_host"
        fi
        continue
        ;;
    esac
    _started=$(( 10#$(_suite_lock_ts_ns "$_started_raw") ))
    if [ "$_started" -lt "$my_started" ] || \
       { [ "$_started" -eq "$my_started" ] && [ "$_pid" -lt "$$" ]; }; then
      blocked=1
      if [ "$have_best" -eq 0 ] || [ "$_started" -lt "$best_started" ] || \
         { [ "$_started" -eq "$best_started" ] && [ "$_pid" -lt "$best_pid" ]; }; then
        have_best=1
        best_started="$_started"
        best_pid="$_pid"
        suite_lock_queue_blocker_pid="$_pid"
        suite_lock_queue_blocker_host="$_host"
      fi
...
# naming the CURRENT reason this run has not proceeded, RIGHT NOW.
#
# Two distinct reasons exist since HIMMEL-2623's FIFO queue, and reporting the
# wrong one is worse than reporting nothing: telling an operator the LOCK is
# held when the lock is actually FREE and this run is simply behind an older
# ticket sends them looking at the wrong process (or a holder that does not
# exist — an owner file read on a free lock renders pid=unknown host=unknown,
# which is its own tell but not an explanation). suite_lock_queue_is_our_turn
# already tells the two apart for its own caller; this reads the SAME verdict
# from suite_lock_queue_blocker_pid rather than re-deriving it, so the two can
# never disagree about which case they are in.
_suite_lock_report_wait() {
  if [ -n "$suite_lock_queue_blocker_pid" ]; then
    printf 'WAITING: behind an older waiter (pid=%s host=%s) for the machine lock of scan root "%s"; this run has waited %s of its %ss budget. lock: %s\n' \
      "$suite_lock_queue_blocker_pid" "${suite_lock_queue_blocker_host:-unknown}" "$scan" \
...
# SUITE_LOCK_WAIT budget (HIMMEL-2215) and FIFO queue ordering (HIMMEL-2623).
# Returns 0 to proceed. On refusal, returns the runner's OWN exit code rather
# than a bare 1 — the call site just does `exit "$?"` — so the two refusal
# shapes stay distinguishable: 2 for an instant refusal or a permanent safety
# refusal (waiting was never in play), 5 for a budget that ran out either
# against a still-held lock or still behind an older live ticket.
#
# Every attempt against the LOCK ITSELF is the UNMODIFIED suite_lock_acquire,
# so the whole staleness / CAS / takeover protocol is byte-identical to a
# non-waiting run — this adds a retry cadence, a queue ticket, and a
# heartbeat, never a second way to obtain the lock. With no budget it is a
# straight pass-through (no ticket, no queue dir touched at all), which is why
# the default costs existing callers nothing and why an OLD, pre-HIMMEL-2623
# runner racing this one for the SAME lock never even sees a queue exists (see
# COEXISTENCE above the queue helpers).
#
...
#
# The NO-WAIT path (SUITE_LOCK_WAIT unset or 0) also yields to a non-empty
# queue now, not just to a held lock (HIMMEL-2623 round 4, codex-1): an
# earlier version of this fix let an ad-hoc no-wait run make one direct
# attempt regardless of who was queued, on the reasoning that this was a
# deliberate, documented limit of the design. It was not compatible with what
# this file actually claims elsewhere — that the starvation this whole
# mechanism exists to fix self-heals "once every runner is on the new code"
# — because it does not: a no-wait run launched at the instant a holder
# releases can always beat the oldest queued waiter's next poll (up to
# SUITE_LOCK_WAIT_INTERVAL away), forever, no matter how many runners have
# upgraded. See the no-wait branch below for the fix.
suite_lock_acquire_waiting() {
  if [ "$SUITE_LOCK_WAIT" -le 0 ]; then
    # SUITE_LOCK=0 and the re-entrancy guard bypass EVERYTHING here too,
    # including the queue-yield check just below, checked in this exact
    # order for the exact reason the identical short-circuit ahead of the
    # waiting path exists (HIMMEL-2623 round 2): a nested run is not a queue
    # jumper, and putting the yield check ahead of THESE two would
    # reintroduce that same deadlock on the no-wait path instead.
    if [ "$SUITE_LOCK" = "0" ] || [ "${HIMMEL_SUITE_LOCK_HELD:-}" = "$SUITE_LOCK_DIR" ]; then
      suite_lock_acquire && return 0
      return 2
    fi
    # A no-wait run never takes a ticket (it never waits), so this is a
    # READ-ONLY check: refuse — same rc 2, same refuse-on-sight SHAPE as a
    # held-lock refusal — when a LIVE waiter is already queued, even if the
    # lock itself is free right now. Without this, an ad-hoc no-wait run
    # could always win a lock the instant it frees, arriving before the
    # oldest queued waiter's next poll; see the docstring above.
    local _qcount
    _qcount=$(suite_lock_queue_live_count)
    if [ "$_qcount" -gt 0 ]; then
      {
        printf 'REFUSED: %s queued waiter(s) are already ahead of scan root "%s" for the machine lock.\n' \
          "$_qcount" "$scan"
        printf '  lock: %s\n' "$SUITE_LOCK_DIR"
        printf '  Concurrent runs test the same tree and starve each other (HIMMEL-1338). This is a\n'
        printf '  no-wait, ad-hoc attempt (SUITE_LOCK_WAIT unset or 0) -- it yields to the run(s)\n'
        printf '  already queued for this lock instead of cutting in front of them, even though the\n'
        printf '  lock itself may be free RIGHT NOW: taking it here would let an ad-hoc run starve the\n'
        printf '  FIFO queue forever, no matter how many runners have upgraded to it.\n'
        printf '  To run now anyway, scope this run to a different subtree, or SUITE_LOCK=0 to opt out\n'
        printf '  of the lock entirely. To queue behind them instead of refusing, set\n'
        printf '  SUITE_LOCK_WAIT=<seconds>.\n'
      } >&2
      return 2
    fi
    suite_lock_acquire && return 0
  ...
  # These two short-circuits MUST be checked HERE, before suite_lock_queue_join
  # ever runs, and not merely trusted to suite_lock_acquire's own copies of
  # them below. The reason is the queue gate itself: every attempt in this
  # function is `suite_lock_queue_is_our_turn && suite_lock_acquire`, and when
  # the turn-check says "not yet" suite_lock_acquire is NEVER CALLED — so its
  # short-circuits never get a chance to fire (HIMMEL-2623 round 2, the
  # deadlock finding). Both conditions are reachable in production, not
  # theoretical:
  #   - HIMMEL_SUITE_LOCK_HELD is EXPORTED (set by _suite_lock_claim), and
  #     SUITE_LOCK_WAIT is inherited by every child suite process —
  #     test-suite-concurrency.sh's own header `unset`s it for exactly this
  #     reason ("a queued after-report is launched with SUITE_LOCK_WAIT=21600
  #     -- so a suite invocation of THIS suite inherits that value"). Without
  #     this check, a NESTED run under a real after-report could join the
  #     queue, find an older waiter's ticket, and block for the FULL wait
  #     budget for a lock its own PARENT already holds — a deadlock inside
  #     exactly the after-report path this file exists to protect.
  #   - SUITE_LOCK=0 is a documented escape hatch (Case 4): a lock-disabled
  #     run must never be gated by a queue it does not participate in.
  # Spelled identically to suite_lock_acquire's own guards (a literal copy,
  # not a shared helper) so the two cannot silently drift apart.
  if [ "$SUITE_LOCK" = "0" ] || [ "${HIMMEL_SUITE_LOCK_HELD:-}" = "$SUITE_LOCK_DIR" ]; then
    suite_lock_acquire && return 0
    return 2
  fi
  ...
  suite_lock_quiet=1
  while :; do
    if suite_lock_queue_is_our_turn && suite_lock_acquire; then
      suite_lock_quiet=0
  ...
  suite_lock_quiet=0
  if suite_lock_queue_is_our_turn && suite_lock_acquire; then
    # The lock freed (and it was our turn) in the instant between the last
    # polled attempt and this one. It is still an acquisition after a wait, so
    # it closes the log the same way the in-loop path does — a run that
    # waited and then succeeded must never end on silence. Same fresh
    # measurement as the in-loop path: the carried value is the one taken at
    # the last failure.
    waited=$(( $(date +%s) - start ))
    printf 'ACQUIRED: got the machine lock of scan root "%s" after waiting %s.\n' \
      "$scan" "$(_suite_lock_dur "$waited")" >&2
  ...
  # Re-measure fresh, at the moment of GIVING UP, rather than carrying the
  # value from the last in-loop failure (which is stale by up to one
  # interval, plus whatever this final loud attempt itself cost) — the same
  # accuracy fix the ACQUIRED path already applies at its own return point.
  waited=$(( $(date +%s) - start ))

  # GAVE UP has ONE literal prefix (console monitors grep it — that stays
  # fixed) but two DIFFERENT tails, because the two ways a genuine wait can
  # run out are not the same fact and telling the wrong one is worse than
  # useless: an operator debugging a starved runner sent to check the HOLDER
  # when the lock is actually free (blocked only by an older ticket) is
  # chasing a process that both has nothing to do with the delay and, per the
  # WAITING: heartbeat above, would have rendered as "holder pid=unknown
  # host=unknown" anyway. suite_lock_queue_blocker_pid — set by the loud
  # suite_lock_queue_is_our_turn call just above — says which case this run
  # is actually in; suite_lock_permanent already ruled out the third case.
  if [ -n "$suite_lock_queue_blocker_pid" ]; then
    printf 'GAVE UP: waited %s for the machine lock of scan root "%s" — still behind an older waiter in the queue (pid=%s host=%s); refusing (the verdict above describes the queue as it is now).\n' \
      "$(_suite_lock_dur "$waited")" "$scan" \
      "$suite_lock_queue_blocker_pid" "${suite_lock_queue_blocker_host:-unknown}" >&2
  else
  ...
  # stdout, deliberately — distinct from GAVE UP above, which stays on stderr
  # for the console monitors that already grep it. This line exists for a
  # DIFFERENT consumer: a caller that wants one unambiguous, machine-parsable
  # signal that the run never executed, on the stream a script normally
  # captures for a command's actual output. Byte-identical in BOTH cases
  # above — this is the one signal that must never fork on the reason.
  printf 'LOCK-WAIT EXPIRED after %ss — run NOT executed\n' "$waited"
  suite_lock_queue_leave
  return 5
}
...
if [ "$list_only" -eq 0 ]; then
  suite_lock_acquire_waiting
  lock_rc=$?
  # suite_lock_acquire_waiting already names its own exit code: 2 for an
  # instant or permanent refusal, 5 for a SUITE_LOCK_WAIT budget that expired
  # (lock still held, or still behind an older live queue ticket) — see the
...
if [ "$shard_total" -gt 0 ]; then
  ...
  _shard_tab=$(printf '\t')
  # Shards decide alone on separate runners, so a fallback is exact only when
  # EVERY shard takes it: one shard recovering from something its siblings
  # never saw puts two partitions in force across one matrix, and a suite then
  # runs twice or lands on no shard at all while every shard exits 0 — the
  # HIMMEL-1128 false-green class. Hence exactly TWO decision points:
  #
  #   1. Is the ledger THERE? Shell builtins, no tool, off a committed file,
  #      so every shard answers identically. Absent: fall back to round-robin
  #      with one notice. Present: bin-pack, with no other fallback below.
  #   2. Did the pack come out RIGHT? Asked once, at the output, against the
  #      run list the shell itself built. A sort that died, an awk that exited
  #      early, a full $TMPDIR — each surfaces as a plan that is not the run
  #      list or a slice that is not what the plan says. One refusal.
  #
  # Nothing in between classifies failures by stage or by rc: a stage that
  # dies produces a plan that fails check 2.
  if [ -f "$_shard_ledger" ] && [ -r "$_shard_ledger" ]; then
    # The FINAL run list, through the same filter chain the main loop uses.
    # fd 4, for the same reason the loop uses fd 3. This is built by builtins
    # only, which is what makes it the trustworthy side of check 2.
    _shard_eligible=""
    _shard_eligible_n=0
    ...
    # Join -> sort -> pack, one pipeline, no rc checks. The median an absent
    # suite is given is computed INSIDE the join rather than by a probe of its
    # own: a probe that died on one runner would hand that runner a different
    # median and so a different — but internally complete — partition from its
    # siblings, which check 2 could not see. Folded in here, a failure kills
    # the whole pipeline and the plan comes out short instead.
    #
    ...
    _shard_plan=$(_SHARD_ELIGIBLE="$_shard_eligible" _SHARD_LEDGER="$_shard_ledger" \
      _SHARD_BYTES_FILE="$_shard_bytes_file" LC_ALL=C awk '
          ...
          {
            split($0, f, "\t")
            if (f[1] != "" && f[2] ~ /^[0-9]+$/) { dur[f[1]] = f[2] + 0; vals[++nv] = f[2] + 0 }
          }
          END {
            for (i = 2; i <= nv; i++) {
              v = vals[i]; j = i - 1
            ...
            for (i = 1; i <= n; i++) {
              if (rows[i] == "") continue
              d = (rows[i] in dur) ? dur[rows[i]] : med
              if (d < 1) d = 1
              printf "%012d\t%s\n", d, rows[i]
            }
    ...
    _shard_mine_list=""
    _shard_mine_n=0
    while IFS="$_shard_tab" read -r _shard_bin _shard_path; do
      [ -n "$_shard_path" ] || continue
      [ "$_shard_bin" = "$shard_offset" ] || continue
      _shard_mine_list="${_shard_mine_list}${_shard_path}${_shard_nl}"
      _shard_mine_n=$((_shard_mine_n + 1))
    done <<< "$_shard_plan"

    # Check 2, the whole correctness argument in one place: the plan must hold
    # every eligible suite EXACTLY ONCE — that is 22a/22g's property, union ==
    # the unsharded run list and the shards pairwise disjoint — and this
    # shard's slice must be exactly the plan's rows for i. The eligible list
    # goes in on stdin and the plan through the environment (no -v, whose
    # escape processing would rewrite a backslash in a path); anything wrong,
    # including awk itself dying, leaves the verdict unequal to what the shell
    # counted, so the check is fail-closed and cannot pass vacuously.
    _shard_verdict=$(printf '%s' "$_shard_eligible" \
      | _SHARD_PLAN="$_shard_plan" awk -v me="$shard_offset" '
          { want[$0] = 1; want_n++ }
          END {
            n = split(ENVIRON["_SHARD_PLAN"], rows, "\n")
            for (i = 1; i <= n; i++) {
              if (rows[i] == "") continue
              t = index(rows[i], "\t")
              if (t == 0) exit 1
              bin = substr(rows[i], 1, t - 1)
              suite = substr(rows[i], t + 1)
              if (!(suite in want) || (suite in seen)) exit 1
              seen[suite] = 1
              placed++
              if (bin == me) mine++
            }
            if (placed + 0 != want_n + 0) exit 1
    ...
    elif [ "$_shard_read_bytes" != "$_shard_ledger_bytes" ]; then
    ...
    printf "run-shell-tests.sh: --shard: duration ledger '%s' is missing or unreadable — falling back to round-robin assignment (balance only; the partition stays exact)\n" \
      "$_shard_ledger" >&2
  fi
fi
