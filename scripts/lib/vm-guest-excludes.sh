...
# tree is therefore NOT covered. An unreadable or unresolvable target fails closed.
...
VM_GUEST_SCAN_FN_HEAD='_s() ( d=$(cd -P -- "$1" && pwd -P) || exit 1; shift; for a; do [ "$a" = "$d" ] && exit 0; done; if [ $# -gt 0 ]; then case "$d/" in //|/proc/*|/sys/*|/dev/*|/run/*|/usr/*|/bin/*|/sbin/*|/lib/*|/lib32/*|/lib64/*|/libx32/*|/etc/*|/boot/*|/snap/*|/var/*) exit 0;; esac; fi; find -H "$d" -xdev \( '
# shellcheck disable=SC2016  # literal guest-shell text: nothing may expand HERE
VM_GUEST_SCAN_FN_TAIL=' \) ! -name '\''.env.example'\'' ! -type d -print || exit 1; l=$(find -H "$d" -xdev -type l -exec test -d {} \; -print) || exit 1; [ -n "$l" ] || exit 0; printf '\''%s\n'\'' "$l" | while IFS= read -r x; do _s "$x" "$d" "$@" || exit 1; done ); '
