...
# operator's own text -- OR the file cannot be read, OR a temp-file / write /
# remove step failed (disk full, read-only target); a failed write-through can
# leave the target truncated, since it is written in place to keep a symlink and
# the mode (ponytail: no atomic rename) -- the stripped content is then kept in
# the temp file the error names, never deleted; 2 = wrong argument count.
...
unwire_user_claude_md() {
  ...
  local nb ne nb_rc ne_rc lb le start tmp
  if [ ! -f "$target" ]; then
    echo "  no $target -- nothing to strip"
    return 0
  fi
  # grep -c exits 1 on a zero count (not an error) and 2 on a read failure: an
  # unreadable file must fail, not read as "no block".
  nb_rc=0; nb=$(grep -cxF -- "$begin" "$target") || nb_rc=$?
  ne_rc=0; ne=$(grep -cxF -- "$end" "$target") || ne_rc=$?
  if [ "$nb_rc" -gt 1 ] || [ "$ne_rc" -gt 1 ]; then
