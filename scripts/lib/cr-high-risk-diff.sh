...
cr_diff_is_high_risk() {
    ...
    printf '%s\n' "$paths" | cr_paths_are_high_risk
}

# cr_paths_are_high_risk  —  the PATH PREDICATE alone, reading a newline-
# separated path list on stdin (HIMMEL-2707).
#   rc 0 = HIGH RISK; stdout is the matched paths (comma-joined, length-capped)
#   rc 1 = ordinary diff; stdout is empty
#
...
# file's own comments already require keeping the predicate in step with
# scripts/cr/pr-check-external.sh GATE 6, and a second hand-maintained copy in
# the kickoff would be a third thing to drift.
cr_paths_are_high_risk() {
    local path base matched=""
