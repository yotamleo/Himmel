...
echo "== resolve_node: NVM_SYMLINK backslash separators rewritten to slashes, no cygpath needed (HIMMEL-2741) =="
# Portable version of the drive-letter test above: exercises the actual
# backslash->slash conversion (dropped the bash-only \${var//\\//} for POSIX
# builtins under HIMMEL-2741) without depending on cygpath, so it runs on
# every host including this one. \$tmp itself is a plain Linux path with no
# backslashes, so substituting / -> \\ and back is exact.
if tmp="$(mktemp -d)"; then
    node_dir="$tmp/nvm4w"; make_fake_node "$node_dir" "node.exe"
    win_style="${node_dir//\//\\}"
    out="$(PATH="$UTILS_DIR" NVM_SYMLINK="$win_style" RESOLVE_NODE_NVM_ROOT="$tmp/none" FNM_DIR="$tmp/none" resolve_node)"; rc=$?
    if [ "$rc" -eq 0 ] && { [ "$out" = "$tmp/nvm4w/node.exe" ] || [ "$out" = "$tmp/nvm4w/node" ]; }; then
        pass "NVM_SYMLINK backslash-separated -> '$out'"
    else
        fail "NVM_SYMLINK backslash-separated -> rc=$rc out='$out' (input '$win_style', want $tmp/nvm4w/node.exe or $tmp/nvm4w/node)"
    fi
    rm -rf "$tmp"
else
    fail "NVM_SYMLINK backslash-separated -> mktemp failed"
fi
