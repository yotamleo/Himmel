...
# --- Session H (separate root): the real wake shapes the classifier must
#     handle -- a cross-session message rendered as plain text ("Another
#     Claude session sent a message:...", not the old literal
#     "<cross-session-message" prefix), a monitor/task wake delivered as a
#     `type:"attachment"` row (`attachment.type:"queued_command"`, not
#     `type:"user"` text), a skill body inserted as a `type:"user"` row that
#     must NOT overwrite the wake classification set by the actual triggering
#     row (HIMMEL-2781), a genuine operator-typed slash command carrying the
#     same `<command-name>` prefix but with `isMeta` ABSENT that MUST reset
#     the wake to "op" (HIMMEL-3006 -- supersedes the HIMMEL-2781 fixture row
#     that pinned the bare-prefix text alone as sufficient to exclude a row;
#     the real discriminator is isMeta, not the prefix), an injected
#     command-body row carrying the same prefix WITH `isMeta:true` that must
#     still be excluded, and the cross-session idle-notice text shape that
#     must classify as "mon" (HIMMEL-3007). Also exercises the
#     `first-resp-input` column (ask 2): only the FIRST response to each wake
#     event counts. -----------------------------------------------------
...
{"type":"user","sessionId":"$SID_H","isSidechain":false,"entrypoint":"cli","timestamp":"2026-05-01T00:00:09.000Z","message":{"content":"<command-name>foo</command-name>"}}
...
{"type":"user","sessionId":"$SID_H","isSidechain":false,"isMeta":true,"timestamp":"2026-05-01T00:00:11.000Z","message":{"content":"<command-name>bar</command-name>"}}
{"type":"assistant","sessionId":"$SID_H","isSidechain":false,"timestamp":"2026-05-01T00:00:12.000Z","requestId":"reqH6","message":{"usage":{"input_tokens":13,"cache_read_input_tokens":0,"cache_creation_input_tokens":0,"output_tokens":0}}}
{"type":"user","sessionId":"$SID_H","isSidechain":false,"isMeta":true,"timestamp":"2026-05-01T00:00:13.000Z","message":{"content":"[Cross-session idle notice] \"peer-session\", which you asked to be notified about, is idle now -- it finished a turn at 2026-05-01T00:00:12Z"}}
{"type":"assistant","sessionId":"$SID_H","isSidechain":false,"timestamp":"2026-05-01T00:00:14.000Z","requestId":"reqH7","message":{"usage":{"input_tokens":15,"cache_read_input_tokens":0,"cache_creation_input_tokens":0,"output_tokens":0}}}
EOF
...
# reset wake) reqH4(mon, queued_command attachment) reqH5(op, genuine
# operator slash command -- bare prefix, isMeta ABSENT, resets the wake)
# reqH6(op, injected command body -- same prefix WITH isMeta:true, excluded,
# wake stays op) reqH7(mon, cross-session idle notice text) ->
# op/cs/mon = 3/2/2, input sum 100+5+7+9+11+13+15=160.
# first-resp-input: only the FIRST response per wake event counts ->
# op=100+11=111 (reqH1, then reqH5's operator wake is a NEW op event since
# the wake had moved to mon in between; reqH6 is the second op response, not
# counted), cs=5 (reqH2; reqH3 is the second cs response, not counted),
# mon=9+15=24 (reqH4, then reqH7's idle-notice wake is a NEW mon event).
assert_contains "$OUT_H" "| Session-H | test-slug | n/a | 7 | 160 | 0 | 0 | 0 | 3/2/2 | 0 | 111/5/24 |" "session H: rendered cross-session text, queued_command attachment, skill-body exclusion, isMeta-gated command-body exclusion vs genuine operator slash command, idle-notice classification, first-resp-input"
