...
# --- cr_paths_are_high_risk direct unit tests (HIMMEL-2707) ---
# The parent split the path predicate out of cr_diff_is_high_risk into
# cr_paths_are_high_risk, documented to read a newline-separated path list on
# STDIN. Test it directly, bypassing the gh-backed reader entirely.
probe_paths() {
    OUT=""; RC=0
    OUT=$(printf '%s\n' "$1" | cr_paths_are_high_risk) || RC=$?
}

probe_paths "scripts/hooks/foo.sh"
expect 0 "scripts/hooks/foo.sh" "cr_paths_are_high_risk: hooks path on stdin -> rc 0, matched"

probe_paths "docs/internals/enforcement.md"
expect 1 "" "cr_paths_are_high_risk: docs-only path on stdin -> rc 1, empty"

probe_paths "tools/nightly-cadence-runner.sh"
expect 0 "tools/nightly-cadence-runner.sh" "cr_paths_are_high_risk: *cadence* basename outside listed dirs -> rc 0"

probe_paths ""
expect 1 "" "cr_paths_are_high_risk: empty stdin -> rc 1"

# RED CONTROL: before the HIMMEL-2707 stdin fix, the loop inside
# cr_paths_are_high_risk ended with `done <<EOF / $paths / EOF` -- a leftover
# heredoc from before the path predicate was split out of
# cr_diff_is_high_risk, referencing a local `$paths` that is never set inside
# this function. That heredoc silently expanded to an EMPTY here-doc, so the
# loop read nothing and the function always returned rc 1 (ordinary) no
# matter what was piped in on stdin. This assertion fails against that bug
# and passes only once `done <<EOF ... EOF` is replaced with a plain `done`
# that reads the piped stdin.
probe_paths "scripts/hooks/stdin-fix.sh"
expect 0 "scripts/hooks/stdin-fix.sh" "cr_paths_are_high_risk: RED control -- stdin fix is load-bearing (a high-risk path on stdin must reach rc 0, not the pre-fix always-rc-1)"
