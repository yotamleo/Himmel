...
# ($HIMMEL_VM_AR_SNAPSHOT, default "suite-ready-v3") before every run — the
# same clean-state guarantee a delete-and-recreate cycle would give, with no
# deletion ever asked for.
#
# WHY THE BASELINE SNAPSHOT IS "suite-ready-v3", NOT "suite-ready" OR
# "suite-ready-v2" (three image-level design facts, not incident colour):
#   1. `VBoxManage clonevm` regenerates the guest's MACs, but the base
#      image's netplan pinned `match: macaddress:` to the SOURCE VM's own
#      MAC — so every linked clone got a NIC matching nothing: no DHCP, no
#      ssh, ever. No clone of the original "suite-ready" base could ever
#      have worked. Fixed at IMAGE level: netplan now matches on interface
...
#      is why this had to be baked into a new base snapshot rather than
#      patched per-clone.
#   3. A linked clone INHERITS the source's `/etc/machine-id` and ssh host
#      keys verbatim — confirmed empirically: source and clone shared the
#      same machine-id and all three host-key fingerprints. That is an
#      identity collision across every clone of the same base, not just a
#      cosmetic duplicate. Fixed by shipping an UNPERSONALISED golden image
#      (`ubuntu_new@suite-ready-v3`): machine-id truncated to zero bytes,
#      the dbus id symlinked to it, host keys removed, and a first-boot
#      oneshot that regenerates both. Each CLONE of that image then
#      regenerates its own identity on ITS first boot — which is also why a
#      clone's own baseline snapshot must be taken AFTER that first boot,
#      never before: snapshotting the still-unpersonalised state would just
#      push the same collision one level down, clone-to-clone this time.
# The old "suite-ready" (v1) snapshot is DELIBERATELY KEPT, not tidied up:
# himmel-ar-1 is a linked clone whose disk chain depends on it existing.
# "suite-ready-v2" also still exists on ubuntu_new (fixed netplan/MAC, but
# predates the machine-id/host-key fix) — superseded, not deleted, and not
# the default for the same reason v1 isn't.
#
...
# when its destination already exists, and the "suite-ready-v3" snapshot
...
#   HIMMEL_VM_AR_SNAPSHOT     baseline snapshot name (default suite-ready-v3)
...
SNAPSHOT="${HIMMEL_VM_AR_SNAPSHOT:-suite-ready-v3}"
...
# anything (HIMMEL-2623): a renamed/missing baseline (suite-ready ->
# suite-ready-v3 already happened once) must fail with a clear, specific
# message here — not an opaque VBoxManage error three calls later.
vbox_py '
...
# precondition does not fit the warm ~/himmel checkout the suite-ready-v3
...
guest_ssh "cd $GUEST_DEST && rm -f $REMOTE_LOG && timeout $SUITE_TIMEOUT bash scripts/ci/run-shell-tests.sh --changed-since origin/main >$REMOTE_LOG 2>&1; echo EXITCODE=\$? >>$REMOTE_LOG"
guest_run_rc=$?
