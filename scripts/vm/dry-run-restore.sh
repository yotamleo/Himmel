...
#   snapshot defaults to HIMMEL_VM_AR_SNAPSHOT or "suite-ready-v3" — NOT
#   "suite-ready" (v1, kept deliberately, never deleted: himmel-ar-1's disk
#   chain depends on it), which pinned netplan to the SOURCE VM's own MAC and
#   so could never answer ssh from any clone at all, and NOT "suite-ready-v2"
#   (fixed that, but still exists on ubuntu_new, superseded) — a linked clone
#   also inherits the source's /etc/machine-id and ssh host keys verbatim, an
#   identity collision v2 predates. v3 ships unpersonalised (machine-id
#   zeroed, host keys removed, a first-boot oneshot regenerating both), so
#   this is why a clone's OWN baseline snapshot must be taken AFTER its first
#   boot, not before. See after-report.sh's own header for the full
#   image-level explanation. This script itself verifies the named snapshot
...
VM_NAME="${1:-}"
SNAPSHOT="${2:-${HIMMEL_VM_AR_SNAPSHOT:-suite-ready-v3}}"
