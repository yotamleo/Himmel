...
if ((Split-Path -Leaf $Settings) -eq 'settings.json') {
  ...
  if (Test-Path $localFile) {
    ...
    if ($lj.PSObject.Properties['enabledPlugins']) {
      $lep = $lj.enabledPlugins
      # null == "no overrides" (parity with the bash/doctor `object|null` accept);
      # only a present, NON-null, non-object value is the unusable shape.
      if ($null -ne $lep -and $lep -isnot [System.Management.Automation.PSCustomObject]) {
        Die "reconcile-enabled-plugins: $localFile has a non-object enabledPlugins - refusing to reconcile (overrides unreadable)" 1
      }
      $localOverrides = $lep
    }
  }
