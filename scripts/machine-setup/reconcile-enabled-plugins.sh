...
#
# Usage:
#   bash reconcile-enabled-plugins.sh [--dry-run] [--scope user|project]
...
#                    default) or project (./.claude/settings.json). NOTE: there
#                    is deliberately no `local` scope — settings.local.json is
#                    the protected OVERRIDE INPUT (it wins over the floor), never
#                    a reconcile TARGET; reconciling it would rewrite the
#                    operator's overrides down to the floor, destroying them.
...
# No `local` scope: settings.local.json is the protected override INPUT, never a
# reconcile target (reconciling it would wipe the operator's overrides).
case "$SCOPE" in
  user|project) ;;
  *) echo "reconcile-enabled-plugins: invalid --scope: $SCOPE (expected user|project)" >&2; exit 2 ;;
esac
...
# is the protected override input, and reconciling it would rewrite the
# operator's overrides down to the floor. Compare case-INSENSITIVELY: Windows
# Git-Bash resolves SETTINGS.LOCAL.JSON to the same file, so a case variant must
# not slip past this guard.
SETTINGS_BASENAME_LC="$(basename "$SETTINGS" | tr '[:upper:]' '[:lower:]')"
if [[ "$SETTINGS_BASENAME_LC" == "settings.local.json" ]]; then
  echo "reconcile-enabled-plugins: refusing to target settings.local.json — it is the protected override input, not a reconcile target" >&2
  exit 2
fi
...
# Read a file's enabledPlugins as a compact object, failing on a present-but-
# non-object (non-null) value — only an object or absent/null is a usable map.
# Shared by the template + live + local reads so no path can slip a non-object
# through to the object merge (or, in the ps1 twin, the hashtable conversion).
enabled_plugins_of() {
  local file="$1" label="$2" t bad
  t="$(jq -r '.enabledPlugins | type' "$file" 2>/dev/null)"
  case "$t" in
    null) echo '{}' ;;
    object)
      # Every value must be a real boolean (parity with the ps1 twin, which
      # refuses non-bools because [bool]"false" is $true). jq keeps JSON types,
      # so a string "false" would otherwise pass through and confuse drift logic.
      bad="$(jq -r '.enabledPlugins | to_entries[] | select((.value | type) != "boolean") | .key' "$file" 2>/dev/null | head -1)"
      if [ -n "$bad" ]; then
        echo "reconcile-enabled-plugins: $label ($file) enabledPlugins[\"$bad\"] is not a boolean — refusing" >&2; return 1
      fi
      jq -c '.enabledPlugins' "$file" ;;
    *) echo "reconcile-enabled-plugins: $label ($file) has a non-object enabledPlugins (type: $t) — refusing" >&2; return 1 ;;
  esac
}
...
TMPL_EP=$(enabled_plugins_of "$TEMPLATE" "template") || exit 1
if [[ "$TMPL_EP" == "{}" ]]; then
...
LOCAL_OVERRIDES="{}"
if [[ "$SETTINGS_BASENAME_LC" == "settings.json" ]]; then
  ...
  if [[ -f "$LOCAL_FILE" ]]; then
    ...
    # A parseable file whose enabledPlugins is a non-object is equally unusable
    # (it would break the object merge); enabled_plugins_of refuses it.
    LOCAL_OVERRIDES=$(enabled_plugins_of "$LOCAL_FILE" "settings.local.json") || exit 1
  fi
fi
...
LIVE_EP=$(enabled_plugins_of "$SETTINGS" "settings") || exit 1
...
# Semantic compare (jq `==` is key-order-insensitive): a plain string compare
# would see reordered keys as a spurious change and rewrite + mislabel every run.
if [[ "$(jq -n --argjson a "$LIVE_EP" --argjson b "$NEW_EP" '$a == $b')" == "true" ]]; then
