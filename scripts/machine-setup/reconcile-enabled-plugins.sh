...
if [[ "$SETTINGS_BASENAME_LC" == "settings.json" ]]; then
  ...
  if [[ -f "$LOCAL_FILE" ]]; then
    ...
    # A parseable file whose enabledPlugins is not an OBJECT (string/array/number)
    # is equally unusable — `.enabledPlugins // {}` would pass the non-object
    # through and break the later object merge. Only object or absent is valid.
    if [[ ! "$(jq -r '.enabledPlugins | type' "$LOCAL_FILE" 2>/dev/null)" =~ ^(object|null)$ ]]; then
      echo "reconcile-enabled-plugins: $LOCAL_FILE has a non-object enabledPlugins — refusing to reconcile (overrides unreadable)" >&2
      exit 1
    fi
