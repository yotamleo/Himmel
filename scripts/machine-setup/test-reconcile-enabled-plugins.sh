...
set +e; out=$(bash "$script" --settings "$tmp/settings.local.json" --template "$tmpl" 2>&1); rc=$?; set -e
...
set +e; bash "$script" --scope local --template "$tmpl" >/dev/null 2>&1; rc=$?; set -e
...
set +e; bash "$script" --settings "$tmp/SETTINGS.LOCAL.JSON" --template "$tmpl" >/dev/null 2>&1; rc=$?; set -e
...
set +e; out=$(bash "$script" --settings "$s4d" --template "$tmpl" 2>&1); rc=$?; set -e
