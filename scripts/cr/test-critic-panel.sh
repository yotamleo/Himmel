...
pin_live_base="$(git -C "$WT_REPO" rev-parse main)"
git -C "$WT_REPO" checkout -q feature
...
check_contains "PIN7: refusal prints the captured base SHA" "$(cat "$tmp/pin7-err")" "$pin_base"
check_contains "PIN7: refusal prints the current base SHA" "$(cat "$tmp/pin7-err")" "$pin_live_base"
...
# the REMOTE-TRACKING ref wins even when the local branch verifies (HIMMEL-2787).
# Discriminating proof: local develop is AHEAD of origin/develop by one commit,
# so an origin/<name> three-dot diff includes that ancestral change while a
# bare-local diff excludes it. The critic stub echoes a marker derived from its
# stdin (the computed diff), so the merged output names which ref was diffed.
...
STUBEOF
chmod +x "$B5_STUB"
b5_remote_base="$(git -C "$BR5_REPO" rev-parse refs/remotes/origin/develop)"
b5_rc=0
CRITICS_JSON="$tmp/critics-all.json" CRITIC_FIRST_PASS="$B5_STUB" \
    bash "$PANEL" --worktree "$BR5_REPO" --base develop --base-sha "$b5_remote_base" \
    > "$tmp/b5-out" 2> "$tmp/b5-err" || b5_rc=$?
check "B5: remote base pin ignores the stale local branch instead of false exit 7" "$b5_rc" "0"
check_contains "B5: REMOTE-TRACKING ref used despite a verifying local branch" "$(cat "$tmp/b5-out")" "ORIGIN-REF-USED"
