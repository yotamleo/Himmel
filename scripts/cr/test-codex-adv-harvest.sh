...
# -- T3b (HIMMEL-2707): rc file already holds 124 (the launcher watchdog's own
#    timeout exit) BEFORE this harvest's poll loop ever starts -- e.g. a prior
#    /pr-check round aborted after kickoff and a later one harvests the
#    orphaned run. This must be reported as a distinct timeout, not folded
#    into T3's generic "failed (rc=...)" wording: the rc=0/empty-stdout
#    silent-death class (T5) triggers a bounded PAID retry, and a known-124
#    overrun misread as that class would spend a second paid companion run on
#    a pass that is already known to have overrun. -------------------------
branch="t3b-timeout124"
export CR_PROFILE=free
codex_out="$out_dir/$branch"
printf '99999\n' >"${codex_out}.pid"
printf '124\n' >"${codex_out}.rc"
printf '0\n' >"${codex_out}.pid.cleanup-rc"
run_harvest "$branch"
check "$(cat "$tmp/rc")" "0" "T3b script rc"
check_contains "T3b distinct timeout message" "$(cat "$tmp/err")" "timed out (launcher watchdog, rc=124"
check_not_contains "T3b never falls through to the generic failed wording" "$(cat "$tmp/err")" "codex adversarial pass failed (rc=124"
check "$(cat "$tmp/out")" "" "T3b stdout empty (no findings leaked)"
check_contains "T3b status unavailable" "$(cat "$tmp/err")" "codex-adv-status: unavailable"
unset CR_PROFILE
