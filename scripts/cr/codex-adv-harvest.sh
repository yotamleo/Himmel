...
# critic-panel.sh's own --batch-file self-write: the session no longer has to
# retype a codex-adv finding title
        ...
        if [ -s "$codex_rc_file" ]; then
            ...
            case "$codex_rc" in
                0)
                    ...
                    ;;
                124)
                    # HIMMEL-2707: this rc file already held 124 by the time
                    # this poll loop even started (the `while [ ! -s
                    # "$codex_rc_file" ]...` loop above never entered) -- so
                    # this is NOT this harvest's own timeout detection (that
                    # path is $harvest_timed_out above, with its own explicit
                    # message) but the LAUNCHER's own watchdog
                    # (run-codex-adversarial.sh's timeout_secs loop, armed by
                    # codex-adv-kickoff.sh) having already fired and exited
                    # before this harvest got a turn to poll at all -- e.g. a
                    # prior /pr-check round aborted after kickoff and a later
                    # one is now harvesting the orphaned run. Reported
                    # distinctly as a timeout (not the generic "failed"
                    # below) so it can never read as the HIMMEL-1420
                    # rc=0/silent-death class -- it isn't one; that class
                    # requires rc=0, and the retry it triggers would spend a
                    # SECOND paid run on a pass we already know overran.
                    echo "codex adversarial pass timed out (launcher watchdog, rc=124; stderr: $codex_err_file) -- continuing without it (HIMMEL-2707)" >&2
                    codex_findings=""; codex_avail_status="unavailable"
                    ;;
                *)
