...
# HIMMEL-2707: prove the watchdog bound is load-bearing end to end -- a
# companion that outlives the bound must be KILLED (rc 124) and its process
# genuinely gone afterwards, not merely believed dead. This is the mechanism
# codex-adv-kickoff.sh's own regression test (test-codex-adv-kickoff.sh T12)
# depends on: T12 only proves kickoff now HANDS the launcher a non-zero
# bound (the literal `0` regression this ticket fixes); this proves that
# bound actually terminates an overrunning process rather than sitting inert.
# Companion sleeps 30s against a 1s bound -- the watchdog's own poll loop
# still ticks in fixed 5s steps (same shape as "timeout exits 124" above), so
# this resolves within one ~5s tick, not 30s, and cannot hang the suite.
# Reuses the suite's own process-liveness helper (proc_tree_process_alive,
# sourced from the real scripts/lib/proc-tree.sh above) rather than
# inventing a new liveness check.
rm -f "$tmp/loadbearing.pid" "$tmp/loadbearing.stdout" "$tmp/loadbearing.stderr"
cat > "$tmp/loadbearing-companion.mjs" <<'JS'
await new Promise(resolve => setTimeout(resolve, 30000));
JS
runner_rc=0
bash "$RUNNER" "$tmp/loadbearing-companion.mjs" main "$tmp/loadbearing.stdout" "$tmp/loadbearing.stderr" "$tmp/loadbearing.pid" 1 \
    2>"$tmp/loadbearing-runner.stderr" || runner_rc=$?
check "load-bearing bound kills an overrunning companion (rc 124)" "$runner_rc" "124"
loadbearing_pid=$(cat "$tmp/loadbearing.pid" 2>/dev/null)
if [ -n "$loadbearing_pid" ]; then
    loadbearing_alive_rc=0
    proc_tree_process_alive "$loadbearing_pid" || loadbearing_alive_rc=$?
    check "load-bearing bound leaves no live companion process" "$loadbearing_alive_rc" "1"
else
    echo "FAIL - load-bearing bound test never captured a companion pid"
    fails=$((fails + 1))
fi
