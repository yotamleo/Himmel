...
# --- logon-task registration (HIMMEL-1822) ------------------------------------

function Write-LauncherVbs {
    # The GUI-subsystem launcher the logon task points at. The exe is a
    # console-subsystem (WINDOWS_CUI) binary: the previous registration form
    # (powershell -NoProfile -WindowStyle Hidden -Command "& exe") CREATED a
    # console that -WindowStyle Hidden only suppressed after a visible flash at
    # each sign-in. wscript.exe is GUI-subsystem (allocates no console) and
    # Run(..., 0, False) starts the exe with SW_HIDE from creation. Paths are
    # parameters (not $HOME-derived inside) so the hermetic test suite can aim
    # the writer at a temp dir; the main body always passes the resolved $Dir
    # paths. Overwrite-in-place, so every -Install / -Register rewrites it
    # idempotently and converges the embedded paths.
    param(
    ...
    $body = @"
' Auto-generated by himmel scripts/setup/cli-proxy-lane.ps1 (HIMMEL-1822) - do not edit.
' wscript.exe is GUI-subsystem (allocates no console) and Run's 0 = SW_HIDE, so
' the console-subsystem proxy starts hidden from creation, not hidden after a flash.
CreateObject("WScript.Shell").Run """$ExePath"" -config ""$ConfigPath""", 0, False
"@
    # WITH a UTF-8 BOM (CR round, codex-adv): the interpolated exe/config paths
    # are pure ASCII for a typical install, but a non-ASCII install root (e.g. a
    # non-English username) would otherwise be decoded by WScript's legacy ANSI
    # code-page fallback and corrupt the path - the same [PS1-NO-BOM] defect
    # class shell-lint.sh already polices for this repo's own .ps1 files
    # (HIMMEL-1432). A BOM is a no-op for pure-ASCII content, so this is safe
    # for every install, not just non-ASCII ones.
    [System.IO.File]::WriteAllText($LauncherPath, $body, (New-Object System.Text.UTF8Encoding $true))
}

function Get-ProxyTaskDefenderHelp {
    # HIMMEL-1822: Windows Defender's ML heuristic flags the registration
    # command line itself (a scheduled task launching a WSH script at logon is
    # a textbook persistence pattern) as Trojan:Win32/Commando.A!ml and removes
    # the task - three observed removals on this fleet (2026-08-14, 2026-08-16
    # x2). Detection firing here is EXPECTED, not evidence of compromise, and
    # the sanctioned fix is an operator-added exclusion - the registration
    # command is deliberately NOT obfuscated to dodge it.
    #
    # The exclusion is scoped to $Vbs specifically (CR round, codex-adv
    # Important), not the whole install directory: start-hidden.vbs is the WSH
    # script the heuristic actually keys on ("a scheduled task launching a WSH
    # script at logon"), so excluding just it clears the detection without
    # also pulling config.yaml and the codex OAuth token JSON out of real-time
    # scanning.
    #
    # $PSCommandPath (CR round, codex-adv Suggestion), not a hardcoded relative
    # path: the doc examples invoke this from the repo root
    # (.\scripts\setup\cli-proxy-lane.ps1), so a bare .\cli-proxy-lane.ps1
    # here would fail unless the operator's cwd happened to already be
    # scripts/setup/.
    @"
Windows Defender most likely removed the 'cli-proxy-api' logon task: it flags
this exact registration as Trojan:Win32/Commando.A!ml (a scheduled task
launching a WSH script at logon is a textbook persistence pattern, so the ML
heuristic fires on it). Until this is resolved the proxy will NOT auto-start
at logon. Operator fix (elevated PowerShell):
  Add-MpPreference -ExclusionPath "$Vbs"
then re-run: $PSCommandPath -Register
"@
}

function Register-ProxyLogonTask {
    # Register the at-logon task whose ACTION is the GUI-subsystem launcher:
    # wscript.exe //B //Nologo start-hidden.vbs. schtasks /create /f
    # force-replaces any existing registration, so re-running -Register is
    # idempotent (same /tn, no duplicates) AND upgrades a machine still
    # carrying the old PowerShell-wrapper action to the launcher form.
    Assert-Exe; Assert-Config
    Write-LauncherVbs -LauncherPath $Vbs -ExePath $Exe -ConfigPath $Cfg
    $tr = "wscript.exe //B //Nologo `"$Vbs`""
    schtasks /create /tn 'cli-proxy-api' /sc onlogon /rl limited /f /tr $tr | Out-Null
    if ($LASTEXITCODE -ne 0) {
        throw "schtasks /create failed (exit $LASTEXITCODE) - run elevated if it is a permissions error. $(Get-ProxyTaskDefenderHelp)"
    }
    # Defender removes the task asynchronously right after creation, so an
    # immediate query racing the removal can observe it present a moment
    # before Defender deletes it. Stopping at the FIRST successful probe would
    # miss exactly that race (the probe right after /create is near-certain to
    # see it present, before Defender has had time to act) - poll through the
    # WHOLE settle window (~5s) and take the LAST probe's result, so the
    # verdict reflects whether the task survived the window, not just whether
    # it existed for an instant.
    $taskPresent = $false
    ...
    if (-not $taskPresent) {
        # A GENUINE removal persists (Defender does not put the task back), but
        # a lone transient `schtasks /query` failure at that exact final probe
        # does not (CR round, codex-adv Important: an earlier fix that treated
        # either of the last two SCHEDULED probes as sufficient could instead
        # mask a real removal that happened between them). Re-check once more,
        # immediately, with no further delay - this rules out the one-off
        # glitch without weakening detection of a real removal, which is still
        # gone on the retry exactly because it is real.
        schtasks /query /tn 'cli-proxy-api' 2>$null | Out-Null
        $taskPresent = ($LASTEXITCODE -eq 0)
    }
    if (-not $taskPresent) {
        throw "logon task 'cli-proxy-api' is ABSENT right after registration. $(Get-ProxyTaskDefenderHelp)"
    }
    Write-Host "registered logon task 'cli-proxy-api' (hidden wscript launcher, runs the proxy at sign-in)"
...
"@
    ...
    # Launcher next to the exe (HIMMEL-1822): a fresh machine has no
    # start-hidden.vbs yet, and -Register points the logon task at it. Rewritten
    # on every -Install so the embedded paths always track the resolved root.
    Write-LauncherVbs -LauncherPath $Vbs -ExePath $Exe -ConfigPath $Cfg
    Write-Host "wrote hidden launcher: $Vbs"
}
...
if ($Register) {
    # HIMMEL-1822: hidden wscript launcher as the task action + Defender
    # removal detection live in the function (see its comments).
