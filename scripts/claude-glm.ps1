...
<#
  ...
  enumerated paths (2 = missing key, 3 = egress refusal, 4 = failed seed). Any
  OTHER failure (e.g. a guard config that exists but is not a readable file)
  fails CLOSED via a PowerShell terminating error (exit 1) rather than matching
  the bash exit code. Spec: himmel/specs/design/ws1-claude-glm-wrapper.md
