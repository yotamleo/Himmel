...
# --- 8a. HIMMEL-2653: --warn-at handover nudge ------------------------------
# fill 55 (>= default 50) -> the warning line appears in human mode.
place "$SID_A" "$(snapshot 55 200000 45 "$(now_ms)" "leg-A")" > /dev/null
err="$(CLAUDE_CODE_SESSION_ID="$SID_A" bash "$SCRIPT" 2>&1 >/dev/null)"
contains "warn-at: fill 55 warns at default threshold" "$err" "WARNING - 55% of the context window used (>= 50% handover threshold)"
contains "warn-at: names the handover rule" "$err" "hand over to a fresh session at 50% fill"

# fill 45 (< default 50) -> no warning line.
place "$SID_A" "$(snapshot 45 200000 55 "$(now_ms)" "leg-A")" > /dev/null
err="$(CLAUDE_CODE_SESSION_ID="$SID_A" bash "$SCRIPT" 2>&1 >/dev/null)"
not_contains "warn-at: fill 45 stays silent" "$err" "WARNING"

# fill 55 with --percent: stdout is exactly the number, and stderr gains
# NOTHING relative to the pre-change baseline (no lag line and no warning).
place "$SID_A" "$(snapshot 55 200000 45 "$(now_ms)" "leg-A")" > /dev/null
outp="$(CLAUDE_CODE_SESSION_ID="$SID_A" bash "$SCRIPT" --percent 2>/dev/null)"
check "warn-at: --percent stdout is exactly 55" "$outp" "55"
...
out="$(CLAUDE_CODE_SESSION_ID="$SID_A" bash "$SCRIPT" --warn-at bogus 2>/dev/null)"; rc=$?
check "warn-at bogus: still exit 0" "$rc" "0"
err="$(CLAUDE_CODE_SESSION_ID="$SID_A" bash "$SCRIPT" --warn-at bogus 2>&1 >/dev/null)"
contains "warn-at bogus: falls back to 50" "$err" "using 50"
contains "warn-at bogus: still warns at the fallback threshold" "$err" "WARNING - 55%"

# CONTEXT_FILL_WARN_AT env is honoured, and the --warn-at flag wins over it.
err="$(CLAUDE_CODE_SESSION_ID="$SID_A" CONTEXT_FILL_WARN_AT=90 bash "$SCRIPT" 2>&1 >/dev/null)"
not_contains "warn-at env: raised threshold silences fill 55" "$err" "WARNING"
err="$(CLAUDE_CODE_SESSION_ID="$SID_A" CONTEXT_FILL_WARN_AT=90 bash "$SCRIPT" --warn-at 50 2>&1 >/dev/null)"
contains "warn-at flag beats env" "$err" "WARNING - 55%"

# CR (CodeRabbit): --warn-at must not swallow a following option-like token as
# its threshold value -- --warn-at --percent used to eat --percent as WARN_AT
# (the non-numeric guard fell back to 50) while leaving mode=human, so the
# caller's --percent request silently vanished with only a bogus-threshold
# warning as the trace. Reject before that can happen.
out="$(bash "$SCRIPT" --warn-at --percent 2>/dev/null)"; rc=$?
check "warn-at swallowing --percent -> exit 2" "$rc" "2"
check "warn-at swallowing --percent: stdout stays empty (no silent human fallback)" "$out" ""
err="$(bash "$SCRIPT" --warn-at --percent 2>&1 >/dev/null)"
contains "warn-at swallowing --percent: same missing-value error" "$err" "--warn-at requires a value"

out="$(bash "$SCRIPT" --warn-at --cache-path 2>/dev/null)"; rc=$?
check "warn-at swallowing --cache-path -> exit 2" "$rc" "2"
check "warn-at swallowing --cache-path: stdout stays empty" "$out" ""

# A valid numeric threshold still works.
err="$(CLAUDE_CODE_SESSION_ID="$SID_A" bash "$SCRIPT" --warn-at 70 2>&1 >/dev/null)"
not_contains "warn-at 70: fill 55 stays silent (below the raised threshold)" "$err" "WARNING"

# Pinned choice (--*|-h, not a bare -*): a NEGATIVE number is NOT treated as a
# missing value -- it stays on the existing documented numeric/range check
# below, which warns and falls back to 50, rather than becoming a hard exit-2
# here.
out="$(CLAUDE_CODE_SESSION_ID="$SID_A" bash "$SCRIPT" --warn-at -5 2>/dev/null)"; rc=$?
check "warn-at -5: NOT rejected as a missing value (stays on the numeric-fallback path)" "$rc" "0"
err="$(CLAUDE_CODE_SESSION_ID="$SID_A" bash "$SCRIPT" --warn-at -5 2>&1 >/dev/null)"
contains "warn-at -5: falls back to 50 via the existing non-numeric guard" "$err" "ignoring non-numeric --warn-at/CONTEXT_FILL_WARN_AT=-5 - using 50"
contains "warn-at -5: still warns at the fallback threshold" "$err" "WARNING - 55%"
...
# CR round 3 (HIMMEL-2653): the --warn-at-composing loop must not silently let
# two DIFFERENT output-mode flags coexist with the last one winning -- that is
# exactly the "trailing argument" shape the old single-arg parser rejected.
bash "$SCRIPT" --percent --cache-path >/dev/null 2>&1; check "conflicting output modes (--percent --cache-path) -> exit 2" "$?" "2"
bash "$SCRIPT" --cache-path --percent >/dev/null 2>&1; check "conflicting output modes (--cache-path --percent) -> exit 2" "$?" "2"
errc="$(bash "$SCRIPT" --percent --cache-path 2>&1 >/dev/null)"
contains "conflicting output modes names both flags" "$errc" "conflicting output modes: --percent and --cache-path"
CLAUDE_CODE_SESSION_ID="$SID_A" bash "$SCRIPT" --percent --percent >/dev/null 2>&1
check "repeating the SAME output mode flag is harmless" "$?" "0"
