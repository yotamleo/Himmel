...
echo "[0/11] Verifying foundational tools on PATH..."
_missing=()
...
echo "[0.4/11] Verifying JIRA_PROJECT_KEY..."
_jira_mode=optional
...
# directory.
echo "[0.5/11] Resolving USER_SLUG..."
...
echo "[1/11] Installing pre-commit..."
...
echo "[2/11] Installing git hooks (pre-commit, pre-push, commit-msg)..."
pre-commit install
pre-commit install --hook-type pre-push
pre-commit install --hook-type commit-msg

echo "[3/11] Installing Jira + Bitbucket CLIs..."
...
# --- qmd collection ---
echo "[4/11] Registering qmd collection 'himmel'..."
...
echo "[7/11] Telegram bridge onboarding..."
...
echo "[8/11] Warp integration onboarding..."
if ! bash "$REPO_ROOT/scripts/setup/onboard-warp.sh"; then
  echo "  WARNING: onboard-warp reported a problem; setup continues." >&2
fi
echo ""
...
echo "[9/11] Installing Claude plugins (user scope)..."
if command -v claude >/dev/null 2>&1; then
...
# idempotent.
echo "[10/11] Wiring statusline (user scope)..."
...
echo "[11/11] OPTIONAL: claude-squad (cs)..."
_install_cs=0
if [ "$WITH_CS" = "1" ]; then
  _install_cs=1
elif [ -t 0 ] && [ -t 1 ]; then
