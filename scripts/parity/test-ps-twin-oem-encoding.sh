...
#
#   HALF 1 (structural, pure bash, below): scans shipped tracked .ps1 files for a
...
# Both `sort` calls are pinned to `LC_ALL=C`: `sort`'s collation is
# locale-dependent, and the EXEMPT_ENTRIES fingerprints below were pinned
# under C collation. Left to the host default, a station running e.g.
# en_US.UTF-8 reorders the matched-line text differently before hashing and
# every pinned fingerprint mismatches even though the captured lines
# themselves never changed -- pin it so the fingerprint is host-independent.
fingerprint_detector_lines() {
  local file="$1"
  {
    ...
    grep -nE "$P4" "$file"
    grep -nE "$P5" "$file"
  } 2>/dev/null | LC_ALL=C sort -t: -k1,1n -u | cut -d: -f2- \
...
# archive/ never ships. Snapshot-preserved public copies have independent
# content; private still checks them like every other tracked twin. HIMMEL-2803
# removed gen-changelog.ps1 from this list: its fenced private-only block is now
# scrubbed during snapshot, so the converging public artifact stays in scope.
SNAPSHOT_PRESERVED_PS1=()
PUBLIC_PROJECTION=0
[ ! -f "$REPO/.himmel-public-projection" ] || PUBLIC_PROJECTION=1
...
# The propagator config is private-only. Verify this public-safe literal list
# against its authoritative .ps1 entries without importing its other variables.
if [ "$PUBLIC_PROJECTION" -eq 0 ] && [ -f "$REPO/scripts/lib/public-clone-paths.sh" ]; then
  preserve_ps1="$(
...
preserved_count=0
PS1_FILES="$(cd "$REPO" && git ls-files '*.ps1' ':!archive/')"
# Filter once for all three detector passes and the counted fleet below.
shipped_ps1=''
for f in $PS1_FILES; do
  if is_snapshot_preserved "$f"; then
    [ ! -f "$REPO/$f" ] || preserved_count=$((preserved_count + 1))
    continue
  fi
  shipped_ps1="${shipped_ps1}${f}
"
done
PS1_FILES="$shipped_ps1"
for f in $PS1_FILES; do
  full="$REPO/$f"
  [ -f "$full" ] || continue
...
for entry in "${OUTENC_EXEMPT_ENTRIES[@]}"; do
  path="${entry%%|*}"
  is_snapshot_preserved "$path" && continue
  rest="${entry#*|}"
  pinned_fp="${rest%%|*}"
...
# Canary floor -- 57 non-archive files carry an anchored fix assignment on
# private (HIMMEL-2743). A quoted/commented fix is not an assignment. A mass
# revert or bulk deletion must still trip this even if detector hits disappear.
# Public excludes only tracked, present preserved copies from BOTH sides.
EXPECT_MIN_FIXLINE_COUNT=57
EXPECT_MIN_FIXLINE_COUNT=$((EXPECT_MIN_FIXLINE_COUNT - preserved_count))
if [ "$fixline_count" -lt "$EXPECT_MIN_FIXLINE_COUNT" ]; then
  echo "FAIL: only $fixline_count .ps1 file(s) carry the HIMMEL-2256 fix line, floor is $EXPECT_MIN_FIXLINE_COUNT -- investigate a shipped fix-line regression or deletion; recalibrate EXPECT_MIN_FIXLINE_COUNT in $0 only after reviewing a genuine shipped-scope change." >&2
  FAIL=$((FAIL + 1))
else
