...
# ---------------------------------------------------------------------------
# r10-codex-1 (/pr-check round 10, critical): a symlink at the hook path
# pointing at an EXISTING file that already contains the "Generated by..."
# marker text is grep-matched as "ours" and skipped by the backup check --
# but the write below still follows the symlink and overwrites that
# target, which can be anywhere the marker text happens to appear. `-e`/`-L`
...
if [ -n "${R10C1:-}" ]; then
  ...
  r10c1_out=$(bash "$R10C1/scripts/hooks/install-nostash-hooks.sh" 2>&1)
  r10c1_rc=$?
  check "r10-codex-1: installer succeeds with the marker-bearing symlink pre-planted" "$r10c1_rc" "0"
  [ "$r10c1_rc" -ne 0 ] && echo "  r10-codex-1 output: $r10c1_out"
  if [ -e "$r10c1_outside_target" ]; then
    r10c1_after_sha=$(git hash-object --no-filters "$r10c1_outside_target")
  else
    r10c1_after_sha="MISSING"
  fi
  check "r10-codex-1: the outside symlink target is NOT overwritten by the generated hook" "$r10c1_after_sha" "$r10c1_before_sha"
  if [ -L "$R10C1/.git/hooks/pre-commit" ]; then
    check "r10-codex-1: .git/hooks/pre-commit is a real file, not still the symlink" "still-a-symlink" "real-file"
  else
    check "r10-codex-1: .git/hooks/pre-commit is a real file, not still the symlink" "real-file" "real-file"
  fi
fi

# ---------------------------------------------------------------------------
# r10-codex-2 (/pr-check round 10, important): backing up a foreign hook to
# the fixed name `<hook>.bak-preinstall` clobbers an earlier backup left by a
...
if [ -n "${R10C2:-}" ]; then
  ...
  chmod +x "$R10C2/.git/hooks/pre-commit"
  printf 'earlier backup -- must survive\n' > "$R10C2/.git/hooks/pre-commit.bak-preinstall"
  r10c2_before_sha=$(git hash-object --no-filters "$R10C2/.git/hooks/pre-commit.bak-preinstall")
  r10c2_out=$(bash "$R10C2/scripts/hooks/install-nostash-hooks.sh" 2>&1)
  r10c2_rc=$?
  r10c2_after_sha=$(git hash-object --no-filters "$R10C2/.git/hooks/pre-commit.bak-preinstall")
  check "r10-codex-2: an earlier .bak-preinstall is never clobbered" "$r10c2_after_sha" "$r10c2_before_sha"
  [ "$r10c2_rc" -ne 0 ] && echo "  r10-codex-2 output (installer refused, expected): $r10c2_out"
fi

# ---------------------------------------------------------------------------
# r11-codex-1 (/pr-check round 11, critical): r10-codex-2's fix checked
# clobber-and-moved each hook in the SAME pass, called sequentially for
# pre-commit then commit-msg. If pre-commit's move succeeded but commit-msg's
# clobber check then refused, the script exited having ALREADY moved
# pre-commit's original hook aside with nothing regenerated in its place --
...
if [ -n "${R11C1:-}" ]; then
  ...
  chmod +x "$R11C1/.git/hooks/pre-commit"
  r11c1_pc_before_sha=$(git hash-object --no-filters "$R11C1/.git/hooks/pre-commit")
  printf '#!/bin/sh\necho foreign commit-msg\n' > "$R11C1/.git/hooks/commit-msg"
  chmod +x "$R11C1/.git/hooks/commit-msg"
  printf 'earlier commit-msg backup -- must survive\n' > "$R11C1/.git/hooks/commit-msg.bak-preinstall"
  r11c1_cm_bak_before_sha=$(git hash-object --no-filters "$R11C1/.git/hooks/commit-msg.bak-preinstall")
  r11c1_out=$(bash "$R11C1/scripts/hooks/install-nostash-hooks.sh" 2>&1)
  r11c1_rc=$?
  check "r11-codex-1: installer refuses (the SECOND hook's backup would clobber)" "$r11c1_rc" "1"
  [ "$r11c1_rc" -eq 0 ] && echo "  r11-codex-1 output (expected a refusal): $r11c1_out"
  if [ -e "$R11C1/.git/hooks/pre-commit" ]; then
    r11c1_pc_after_sha=$(git hash-object --no-filters "$R11C1/.git/hooks/pre-commit")
  else
    r11c1_pc_after_sha="MISSING"
  fi
  check "r11-codex-1: pre-commit's foreign hook is left in place, not moved aside" "$r11c1_pc_after_sha" "$r11c1_pc_before_sha"
  if [ -e "$R11C1/.git/hooks/pre-commit.bak-preinstall" ]; then
    check "r11-codex-1: no pre-commit.bak-preinstall was created" yes no
  else
    check "r11-codex-1: no pre-commit.bak-preinstall was created" no no
  fi
  r11c1_cm_bak_after_sha=$(git hash-object --no-filters "$R11C1/.git/hooks/commit-msg.bak-preinstall")
  check "r11-codex-1: the earlier commit-msg backup is not clobbered" "$r11c1_cm_bak_after_sha" "$r11c1_cm_bak_before_sha"
fi

# ---------------------------------------------------------------------------
# r11-codex-2 (/pr-check round 11, critical): the ownership check matched the
# marker sentence as a SUBSTRING anywhere in the file, so a foreign hook that
# merely quotes or embeds that sentence somewhere other than its actual
# generated-preamble line was wrongly judged "ours" and silently overwritten
...
if [ -n "${R11C2:-}" ]; then
  ...
  chmod +x "$R11C2/.git/hooks/pre-commit"
  r11c2_out=$(bash "$R11C2/scripts/hooks/install-nostash-hooks.sh" 2>&1)
  r11c2_rc=$?
  check "r11-codex-2: installer succeeds" "$r11c2_rc" "0"
  [ "$r11c2_rc" -ne 0 ] && echo "  r11-codex-2 output: $r11c2_out"
  if [ -e "$R11C2/.git/hooks/pre-commit.bak-preinstall" ]; then
    check "r11-codex-2: the foreign hook (marker text embedded off line 2) was backed up, not silently overwritten" yes yes
  else
    check "r11-codex-2: the foreign hook (marker text embedded off line 2) was backed up, not silently overwritten" no yes
  fi
fi
