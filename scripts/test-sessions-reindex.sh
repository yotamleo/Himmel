...
note "2026/05/2026-05-10-1200-himmel-main.md" session himmel 2026-05-10T12:00:00Z # leak-allow: hostname smoke-test fixture note for this project's own private repo
note "2026/06/2026-06-02-0900-himmel-main.md" session himmel 2026-06-02T09:00:00Z # leak-allow: hostname second fixture note reusing the same private repo name
...
note "2026/06/some-concept.md" note himmel 2026-06-04T10:00:00Z # leak-allow: hostname fixture asserting a non-session note is skipped
printf -- '---\ntype: backfill-index\n---\n' > "$VAULT/sessions/_backfill-old.md"
# A pre-existing hub that MUST NOT be clobbered:
printf -- '---\ntype: repo-hub\n---\n\n# himmel\n\nCURATED PROSE KEEP ME\n' > "$VAULT/sessions/himmel.md" # leak-allow: hostname pre-existing hub fixture keyed on this project's private repo
...
# must read ONLY frontmatter, so repo stays himmel (no `evil` hub). # leak-allow: hostname comment describing the frontmatter-scoping test
DECOY_REPO_FIXTURE="himmel"  # leak-allow: hostname expanded below so the heredoc body's source text carries no literal repo name
cat > "$VAULT/sessions/2026/06/2026-06-06-1200-himmel-main.md" <<NOTE # leak-allow: hostname heredoc target filename for this fixture note
---
date: 2026-06-06T12:00:00Z
type: session
repo: $DECOY_REPO_FIXTURE
---
...
for b in 2026-05-10-1200-himmel-main 2026-06-02-0900-himmel-main 2026-06-03-1000-salus-master; do # leak-allow: hostname loop over the fixture basenames defined above
...
# Count == 5 session notes (3 himmel/salus + ext-repo + decoy-body). # leak-allow: hostname doc comment referencing the fixture repo name
grep -q '^count: 5' "$IDX" && pass "count is 5" || fail "count wrong ($(grep '^count:' "$IDX"))" # leak-allow: hostname assertion on the generated index count, no hostname text here
...
if [ "$(grep -n '\[\[2026-06-06-1200-himmel-main\]\]' "$IDX" | cut -d: -f1)" -lt "$(grep -n '\[\[2026-06-02-0900-himmel-main\]\]' "$IDX" | cut -d: -f1)" ]; then # leak-allow: hostname ordering check on the same fixture note names
...
grep -q 'Repo hubs:.*\[\[himmel\]\]' "$IDX" && pass "index links repo hubs" || fail "index missing repo-hub links" # leak-allow: hostname assertion the index links this fixture's repo hub
...
grep -q 'CURATED PROSE KEEP ME' "$VAULT/sessions/himmel.md" && pass "existing hub not clobbered" || fail "existing hub was clobbered" # leak-allow: hostname assertion the pre-existing hub fixture survives
...
bash "$SCRIPT" --vault "$VAULT" >/dev/null 2>&1
grep -q 'CURATED PROSE KEEP ME' "$VAULT/sessions/himmel.md" && pass "idempotent: hub still intact" || fail "second run clobbered hub" # leak-allow: hostname idempotency assertion on the same fixture hub file
