...
note "2026/05/2026-05-10-1200-himmel-main.md" session himmel 2026-05-10T12:00:00Z
note "2026/06/2026-06-02-0900-himmel-main.md" session himmel 2026-06-02T09:00:00Z
...
# must read ONLY frontmatter, so repo stays himmel (no `evil` hub).
cat > "$VAULT/sessions/2026/06/2026-06-06-1200-himmel-main.md" <<'NOTE'
---
...
for b in 2026-05-10-1200-himmel-main 2026-06-02-0900-himmel-main 2026-06-03-1000-salus-master; do
...
# Count == 5 session notes (3 himmel/salus + ext-repo + decoy-body).
...
if [ "$(grep -n '\[\[2026-06-06-1200-himmel-main\]\]' "$IDX" | cut -d: -f1)" -lt "$(grep -n '\[\[2026-06-02-0900-himmel-main\]\]' "$IDX" | cut -d: -f1)" ]; then
...
grep -q 'Repo hubs:.*\[\[himmel\]\]' "$IDX" && pass "index links repo hubs" || fail "index missing repo-hub links"
...
grep -q 'CURATED PROSE KEEP ME' "$VAULT/sessions/himmel.md" && pass "existing hub not clobbered" || fail "existing hub was clobbered"
...
bash "$SCRIPT" --vault "$VAULT" >/dev/null 2>&1
grep -q 'CURATED PROSE KEEP ME' "$VAULT/sessions/himmel.md" && pass "idempotent: hub still intact" || fail "second run clobbered hub"
