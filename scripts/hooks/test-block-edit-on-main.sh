...
# T50 (HIMMEL-2526): a Write whose target IS `.single-writer` itself, on a
# main-branch repo, where `.single-writer` is untracked AND gitignored (via
# .git/info/exclude, not a tracked .gitignore) -> BLOCK. Before this ticket
# the untracked+gitignored exemption (HIMMEL-876, T29 above) had no basename
# carve-out for `.single-writer`, so a Bash-mediated `cat > .single-writer`
# could silently create the marker and disable this whole fence from inside
# the checkout it protects. Mirrors T33/T34's secret-basename carve-out shape.
mkrepo "$SANDBOX/swwrite" main
...
# empty GIT_TEMPLATE_DIR (or some other as-yet-unidentified condition seen
# once under a ~200-suite load run) `git init` does NOT create .git/info/,
# and the bare `>>` then fails ENOENT, turning this row from meaningful into
# a hard failure rather than a vacuous pass.
mkdir -p "$SANDBOX/swwrite/.git/info"
echo ".single-writer" >> "$SANDBOX/swwrite/.git/info/exclude"
if ! git -C "$SANDBOX/swwrite" check-ignore -q -- .single-writer; then
    echo "FAIL T50 fixture: .single-writer not ignored in swwrite — row would be vacuous"
    FAILED=$((FAILED + 1))
fi
assert_rc "T50 Write .single-writer (untracked+gitignored) on main blocks (HIMMEL-2526)" 2 \
    "$(rc_of "$SANDBOX/swwrite/.single-writer")"
