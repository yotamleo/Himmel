...
#         (usage-aware slot: ASAP if quota-gauge returns, else the binding
...
# >>> WS9-QUOTA-GAUGE (HIMMEL-654): best-effort cross-lane quota-gauge observation.
...
# function definitions today (their sole `exit` is _qg_is_main-gated).
_qg_lib="$hook_dir/../lib/quota-gauge-ledger.sh"
...
quota_gauge_note_claude() {
    command -v quota_gauge_append >/dev/null 2>&1 || return 0
    quota_gauge_append "$(quota_gauge_row claude "$@")" 2>/dev/null || true
    return 0
}
# <<< WS9-QUOTA-GAUGE
...
# >>> WS9-QUOTA-GAUGE (HIMMEL-654): observe the tripping window ONCE per trip
...
case "$window" in
    five_hour) _qg_window="5h" ;;
    seven_day) _qg_window="weekly" ;;
    *)         _qg_window="" ;;
esac
case "$resets_display" in
    unknown|bucket*) _qg_reset="" ;;
    *)               _qg_reset="$resets_display" ;;
esac
quota_gauge_note_claude "arm-threshold" "$util" "$_qg_window" "$_qg_reset" "auto-arm threshold trip"
# <<< WS9-QUOTA-GAUGE
