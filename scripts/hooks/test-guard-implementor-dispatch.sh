...
# mkcache_both <path> <five_hour_util> <seven_day_util> - HIMMEL-2653 fixture
# carrying BOTH windows, each with a live (future) resets_at.
mkcache_both() {
    printf '{"five_hour":{"utilization":%s,"resets_at":"%s"},"seven_day":{"utilization":%s,"resets_at":"%s"}}' \
        "$2" "$(( $(date +%s) + 3600 ))" "$3" "$(( $(date +%s) + 3600 ))" > "$1"
}
...
run_hook() {
    ...
    # HIMMEL-2735: the guard honours IMPL_GUARD_OK / IMPL_GUARD_DISABLE as
    # explicit session overrides that short-circuit it before any of the
    # policy this suite exercises runs. If either is set in the environment
    # of the shell running this suite (e.g. a leg armed with
    # IMPL_GUARD_OK=1), it leaks into the hook's env and every assertion
    # reads the override line instead of the verdict it expects. Strip both
    # here so the suite tests the guard, not its own escape hatch; RC12
...
# HIMMEL-2658: the *haiku* case glob above matches on SUBSTRING, so a
# [1m]-suffixed id (the arming-time context-window choice arm-resume.sh's
# --context now produces) must take the same early exit as the unsuffixed
# id -- a future refactor to an exact-match comparison would silently
# start gating every 1m-armed Haiku dispatch.
RC29C=$(run_hook haiku-full-id-1m "$REG_CLAUDEX" "$(payload general-purpose 'claude-haiku-4-5-20251001[1m]' 'Implement the fix' 'Write the code.')" IMPL_GUARD_CACHE_PATH="$CACHE_HARD")
assert_rc "[1m]-suffixed full Haiku identifier still always allows" 0 "$RC29C"
assert_empty "[1m]-suffixed Haiku identifier always-allow is silent" "$(combined_output haiku-full-id-1m)"
...
# sat at a quiet 45% all day and the live gate never fired. A quiet five-hour
# window must not mask a HARD-tripping weekly one.
CACHE_WEEKLY_HARD="$TMP/cache-weekly-hard.json"; mkcache_both "$CACHE_WEEKLY_HARD" 10 90
RC71=$(run_hook weekly-hard "$REG_NONE" "$(payload general-purpose sonnet 'Implement the fix' 'Write the code.')" IMPL_GUARD_CACHE_PATH="$CACHE_WEEKLY_HARD")
assert_rc "weekly HARD (90) with quiet five-hour (10) denies" 2 "$RC71"
assert_contains "weekly HARD refusal names the 7-day window" "7-day bank at 90% (>= HARD 85%)" "$(cat "$TMP/err-weekly-hard")"
assert_contains "weekly HARD refusal keeps lane-routing advice" "himmel-ops:glm-subagent" "$(cat "$TMP/err-weekly-hard")"

# The exact case the live gate missed all day: 74% weekly (>= WARN 70%) with a
# quiet 45% five-hour reading must WARN, not stay silent.
CACHE_WEEKLY_WARN="$TMP/cache-weekly-warn.json"; mkcache_both "$CACHE_WEEKLY_WARN" 45 74
RC72=$(run_hook weekly-warn "$REG_NONE" "$(payload general-purpose sonnet 'Implement the fix' 'Write the code.')" IMPL_GUARD_CACHE_PATH="$CACHE_WEEKLY_WARN")
assert_rc "weekly WARN (74) with quiet five-hour (45) allows" 0 "$RC72"
assert_contains "weekly WARN emits visible allow decision" '"permissionDecision":"allow"' "$(cat "$TMP/out-weekly-warn")"
assert_contains "weekly WARN names the 7-day window" "7-day bank at 74% (>= WARN 70%)" "$(cat "$TMP/out-weekly-warn")"

# No regression: a HARD five-hour reading must still deny even when the
# weekly window is quiet.
CACHE_FIVE_HARD_WEEKLY_LOW="$TMP/cache-five-hard-weekly-low.json"; mkcache_both "$CACHE_FIVE_HARD_WEEKLY_LOW" 82 10
RC73=$(run_hook five-hard-weekly-low "$REG_NONE" "$(payload general-purpose sonnet 'Implement the fix' 'Write the code.')" IMPL_GUARD_CACHE_PATH="$CACHE_FIVE_HARD_WEEKLY_LOW")
assert_rc "five-hour HARD (82) with quiet weekly (10) still denies" 2 "$RC73"
assert_contains "regression: refusal names the 5-hour window" "5-hour bank at 82% (>= HARD 80%)" "$(cat "$TMP/err-five-hard-weekly-low")"

# A cache carrying only five_hour (the pre-2653 shape) must behave exactly as
# before — no 7-day chatter leaks into an otherwise unchanged HARD refusal.
RC74=$(run_hook seven-absent "$REG_NONE" "$(payload general-purpose sonnet 'Implement the fix' 'Write the code.')" IMPL_GUARD_CACHE_PATH="$CACHE_HARD")
assert_rc "seven_day absent: five-hour-only path still HARD denies" 2 "$RC74"
assert_contains "seven_day absent: message unchanged" "5-hour bank at 85%" "$(cat "$TMP/err-seven-absent")"
assert_not_contains "seven_day absent: no 7-day chatter leaks into the message" "7-day" "$(cat "$TMP/err-seven-absent")"

# Independent fail-open, proof 1: an unusable five_hour (null) must NOT
# suppress a usable, tripping seven_day.
CACHE_FIVE_NULL_SEVEN_HARD="$TMP/cache-five-null-seven-hard.json"
printf '{"five_hour":{"utilization":null,"resets_at":"%s"},"seven_day":{"utilization":90,"resets_at":"%s"}}' \
    "$(( $(date +%s) + 3600 ))" "$(( $(date +%s) + 3600 ))" > "$CACHE_FIVE_NULL_SEVEN_HARD"
RC75=$(run_hook five-null-seven-hard "$REG_NONE" "$(payload general-purpose sonnet 'Implement the fix' 'Write the code.')" IMPL_GUARD_CACHE_PATH="$CACHE_FIVE_NULL_SEVEN_HARD")
assert_rc "five_hour unusable (null) does not suppress a usable, tripping seven_day" 2 "$RC75"
assert_contains "independent fail-open: refusal names the 7-day window" "7-day bank at 90% (>= HARD 85%)" "$(cat "$TMP/err-five-null-seven-hard")"

# Independent fail-open, proof 2: only when BOTH windows are unusable does the
# hook fail open, naming both.
CACHE_BOTH_UNUSABLE="$TMP/cache-both-unusable.json"
printf '%s' '{"five_hour":{"utilization":null},"seven_day":{"utilization":"garbage"}}' > "$CACHE_BOTH_UNUSABLE"
RC76=$(run_hook both-unusable "$REG_NONE" "$(payload general-purpose sonnet 'Implement the fix' 'Write the code.')" IMPL_GUARD_CACHE_PATH="$CACHE_BOTH_UNUSABLE")
assert_rc "both windows unusable fails open" 0 "$RC76"
assert_contains "both-unusable names both windows in the warning" "neither the 5-hour nor the 7-day" "$(cat "$TMP/err-both-unusable")"

echo ""
echo "=== HIMMEL-2653 CR round 2: invalid threshold overrides fall back, never suppress ==="

# Zero (the exact reported shape): with BOTH windows tripping their OWN HARD
# threshold at default values, the tie-break at line ~642 divides by HARD as a
# denominator. An unvalidated IMPL_GUARD_HARD=0 makes that division fatal in
# awk, the command substitution returns empty, and hard_window is wiped out
# even though five_hour already tripped -- a silent allow on what should be a
# HARD refusal. Both windows here trip regardless of the override (90 >= 80
# default HARD, 90 >= 85 default WEEKLY_HARD), so this isolates the tie-break
# path rather than the trip check itself.
CACHE_BOTH_HARD="$TMP/cache-both-hard.json"; mkcache_both "$CACHE_BOTH_HARD" 90 90
RC77=$(run_hook hard-zero "$REG_NONE" "$(payload general-purpose sonnet 'Implement the fix' 'Write the code.')" IMPL_GUARD_CACHE_PATH="$CACHE_BOTH_HARD" IMPL_GUARD_HARD=0)
assert_rc "IMPL_GUARD_HARD=0 still HARD-refuses via the default threshold" 2 "$RC77"
assert_contains "zero override warns and names the default" "IMPL_GUARD_HARD=0 is not a positive number — using default 80" "$(cat "$TMP/err-hard-zero")"
assert_contains "zero override refusal uses the default 80% threshold" "bank at 90% (>= HARD 80%)" "$(cat "$TMP/err-hard-zero")"

# Negative: a WARN-tripping five_hour reading must still advise using the
# default WARN threshold, not be silenced by a negative override.
RC78=$(run_hook warn-negative "$REG_NONE" "$(payload general-purpose sonnet 'Implement the fix' 'Write the code.')" IMPL_GUARD_CACHE_PATH="$CACHE_WARN" IMPL_GUARD_WARN=-5)
assert_rc "IMPL_GUARD_WARN=-5 still allows (WARN is advisory)" 0 "$RC78"
assert_contains "negative override warns and names the default" "IMPL_GUARD_WARN=-5 is not a positive number — using default 65" "$(cat "$TMP/err-warn-negative")"
assert_contains "negative override advisory uses the default 65% threshold" "WARN 65%" "$(cat "$TMP/out-warn-negative")"

# Non-numeric: reuse the existing weekly-HARD fixture (quiet five_hour,
# tripping seven_day) with a garbage IMPL_GUARD_WEEKLY_HARD override -- the
# 7-day refusal must still fire off the default 85% threshold.
RC79=$(run_hook weekly-hard-nonnumeric "$REG_NONE" "$(payload general-purpose sonnet 'Implement the fix' 'Write the code.')" IMPL_GUARD_CACHE_PATH="$CACHE_WEEKLY_HARD" IMPL_GUARD_WEEKLY_HARD=notanumber)
assert_rc "IMPL_GUARD_WEEKLY_HARD=notanumber still HARD-refuses via the default" 2 "$RC79"
assert_contains "non-numeric override warns and names the default" "IMPL_GUARD_WEEKLY_HARD=notanumber is not a positive number — using default 85" "$(cat "$TMP/err-weekly-hard-nonnumeric")"
assert_contains "non-numeric override refusal uses the default 85% threshold" "7-day bank at 90% (>= HARD 85%)" "$(cat "$TMP/err-weekly-hard-nonnumeric")"

echo ""
echo "=== HIMMEL-2653 CR round 3: explicit HARD->WARN downgrade must survive WARN_T > HARD ==="

# The existing "HARD without resets_at downgrades to allow" case (RC24) cannot
# distinguish the explicit downgrade from *_IS_WARN's own threshold check,
# because it uses the DEFAULT thresholds (WARN_T=65 <= HARD=80) where a util
# of 85 trips WARN on its own regardless. Isolate the downgrade with an
# operator config where WARN_T > HARD and a util strictly between them: HARD
# refuses eligibility-wise but cannot (no resets_at), and *_IS_WARN's own
# check (62 >= 65?) is false -- only the explicit downgrade produces a WARN.
CACHE_WARN_GT_HARD="$TMP/cache-warn-gt-hard.json"
printf '%s' '{"five_hour":{"utilization":62}}' > "$CACHE_WARN_GT_HARD"
RC80=$(run_hook downgrade-warn-gt-hard "$REG_NONE" "$(payload general-purpose sonnet 'Implement the fix' 'Write the code.')" IMPL_GUARD_CACHE_PATH="$CACHE_WARN_GT_HARD" IMPL_GUARD_HARD=60 IMPL_GUARD_WARN=65)
assert_rc "HARD-eligible reading with no resets_at, WARN_T>HARD, still allows" 0 "$RC80"
assert_contains "downgrade emits a visible allow decision" '"permissionDecision":"allow"' "$(cat "$TMP/out-downgrade-warn-gt-hard")"
assert_contains "downgrade advisory uses the (higher) WARN threshold" "WARN 65%" "$(cat "$TMP/out-downgrade-warn-gt-hard")"
