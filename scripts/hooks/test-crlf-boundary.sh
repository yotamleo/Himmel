...
# A second fixture root OUTSIDE /tmp, for path-based fences that treat any
# `*/tmp/*` path as a scratch/devnull exemption (is_temp_or_devnull in
# block-write-into-main-checkout.sh and its siblings) — a redirect target
# under $TMP above would be silently ALLOWED regardless of branch, which is
# not the thing under test there.
BWIMC_FIX="$(mktemp -d "${HOME:?}/.himmel-crlf-bwimc.XXXXXX")" || { echo "FATAL: mktemp -d (HOME) failed" >&2; exit 1; }
[ -n "$BWIMC_FIX" ] || { echo "FATAL: mktemp -d (HOME) returned empty" >&2; exit 1; }

trap 'rm -rf "$TMP" "$BWIMC_FIX"' EXIT
...
# Destination-based fence (HIMMEL-2526): the target is resolved from the
# command text itself, and here it's an absolute path, so the hook's cwd
# (which check_bounded's JSON never sets) is irrelevant to resolution — unlike
# block-terminal-write-fence.sh's class (b) above. check_bounded proves the
# LF/terminal-CR/full-CRLF triple: a CR glued onto the target token (terminal
# CR) or trailing every line (CRLF) must not let the target slip past
# extraction — the write must still deny exactly like the LF form. Fixture
# lives under $BWIMC_FIX (outside /tmp), not $TMP — a target path containing
# "/tmp/" is exempted outright by the hook's is_temp_or_devnull check, which
# would make this row vacuous.
BWIMC_MAIN="$BWIMC_FIX/bwimc-main"
_mkrepo "$BWIMC_MAIN" main

check_bounded block-write-into-main-checkout Bash 2 \
    "echo x > $BWIMC_MAIN/bad.txt" \
    "redirect write into a checkout on main blocks (HIMMEL-2526)"
check_bounded block-write-into-main-checkout Bash 0 "$A_OK" \
    "allow git status"
