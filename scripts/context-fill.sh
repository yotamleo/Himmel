...
usage() {
  cat <<'USAGE'
usage: context-fill.sh [--percent | --cache-path | --help] [--warn-at N]
...
mode_flag=""
warn_at_arg=""
# A loop (not the old single-arg case) so --warn-at N can compose with
# --percent/--cache-path/(no flag) (HIMMEL-2653). Any other/unknown token
# still fails the documented way (exit 2) rather than silently ignoring it.
# --percent and --cache-path remain mutually exclusive output modes: the old
# single-arg parser rejected two of them together via `[ "$#" -gt 1 ]`, and a
# bare loop over both would silently let the LAST one win instead. mode_flag
# tracks which output-mode flag (if any) already won, so a second, DIFFERENT
# one is rejected the same way an unknown token is; repeating the SAME flag
# (--percent --percent) is harmless and allowed.
while [ "$#" -gt 0 ]; do
  case "$1" in
    --percent|--cache-path)
      if [ -n "$mode_flag" ] && [ "$mode_flag" != "$1" ]; then
        printf 'context-fill: conflicting output modes: %s and %s\n' "$mode_flag" "$1" >&2
        usage >&2
        exit 2
      fi
      mode_flag="$1"
    ...
    --warn-at)
      shift
      if [ "$#" -lt 1 ]; then
        printf 'context-fill: --warn-at requires a value\n' >&2
        usage >&2
        exit 2
      fi
      # CR (CodeRabbit): an option-like token (--percent, --cache-path,
      # --warn-at again, --help/-h) must not be swallowed as the threshold
      # value -- that silently eats the caller's real flag (e.g. --warn-at
      # --percent left mode=human while only warning about a bogus
      # threshold). --*|-h only, not a bare -*: a NEGATIVE number (-5) stays
      # on the existing documented range check below, which already warns
      # and falls back to 50 -- this must not turn that into a hard exit.
      case "$1" in
        --*|-h)
          printf 'context-fill: --warn-at requires a value\n' >&2
          usage >&2
...
case "$WARN_AT" in
  ''|*[!0-9]*)
    printf 'context-fill: ignoring non-numeric --warn-at/CONTEXT_FILL_WARN_AT=%s - using 50\n' "$WARN_AT" >&2
    WARN_AT=50
    ;;
esac
