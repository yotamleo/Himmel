...
#                  (no qmd on PATH).
#   mcp-registered graphify-mcp present / server missing / file missing
...
#                  the bin/initMarker deepening against a synthetic fixture
#                  descriptor (no live manifest item still carries both
#                  fields, HIMMEL-2634) — registered-but-broken (unresolved binary /
...
#                  the (since-removed, HIMMEL-2634) tokensave-mcp descriptor,
#                  applied here proactively; plus (round 3,
...
# ── git-hooks: installed is not enforcing (HIMMEL-3306) ────────────────────
# pre-commit framework stubs skip ("config file not found") on every commit and
# push when the target has no .pre-commit-config.yaml, so they gate nothing.
# The probe must not attest that as installed.
hooks_stub_repo="$work/hooks-stub-repo"; mkdir -p "$hooks_stub_repo/.git/hooks"
write_stub() {
  printf '#!/usr/bin/env bash\n# File generated by pre-commit: https://pre-commit.com\nexec pre-commit hook-impl\n' > "$1"
  chmod +x "$1"
}
for _hook in pre-commit commit-msg pre-push; do write_stub "$hooks_stub_repo/.git/hooks/$_hook"; done
outStubNoCfg=$(probe_hooks "$hooks_stub_repo")
echo "$outStubNoCfg" | jq -e '.actual == "absent"' >/dev/null \
  || fail "HIMMEL-3306 inert stubs (all three, no config) must not read present: (got: $outStubNoCfg)"
echo "$outStubNoCfg" | jq -e '.detail | contains(".pre-commit-config.yaml")' >/dev/null \
  || fail "HIMMEL-3306 inert-stub detail must name the missing .pre-commit-config.yaml: (got: $outStubNoCfg)"

# Only some of the stubs are inert (the others are native dispatchers) => degraded.
hooks_mixed_repo="$work/hooks-mixed-repo"; mkdir -p "$hooks_mixed_repo/.git/hooks"
write_stub "$hooks_mixed_repo/.git/hooks/pre-commit"
for _hook in commit-msg pre-push; do
  printf '#!/usr/bin/env bash\nexit 0\n' > "$hooks_mixed_repo/.git/hooks/$_hook"
  chmod +x "$hooks_mixed_repo/.git/hooks/$_hook"
done
outStubMixed=$(probe_hooks "$hooks_mixed_repo")
echo "$outStubMixed" | jq -e '.actual == "degraded" and (.detail | contains("pre-commit") and contains(".pre-commit-config.yaml"))' >/dev/null \
  || fail "HIMMEL-3306 one inert stub among native hooks should read degraded and name the config: (got: $outStubMixed)"

# The same stubs WITH the adopter's own config are the framework working: present.
: > "$hooks_stub_repo/.pre-commit-config.yaml"
outStubCfg=$(probe_hooks "$hooks_stub_repo")
echo "$outStubCfg" | jq -e '.actual == "present"' >/dev/null \
  || fail "HIMMEL-3306 pre-commit stubs with a .pre-commit-config.yaml must read present: (got: $outStubCfg)"
echo "ok: git-hooks reads pre-commit stubs with no .pre-commit-config.yaml as inert (absent/degraded), present once the config exists (HIMMEL-3306)"
...
: > "$mcp_present_home/fixture-mcp-stub"
chmod +x "$mcp_present_home/fixture-mcp-stub"
printf '{"mcpServers":{"fixture":{"command":"%s"}}}' "$(winpath "$mcp_present_home/fixture-mcp-stub")" > "$mcp_present_home/.claude.json"
...
// a fixture-mcp descriptor that DOES carry bin/initMarker.
const item = { id: 'fixture-mcp', probe: { type: 'mcp-registered', server: 'fixture' } };
...
echo "ok: mcp-registered (fixture-mcp) present/server-missing/file-missing/null-root/malformed via ctx.env.HOME, configError distinguishes broken config from clean absence"
...
printf '{"mcpServers":{"fixture":{"command":"fixture-mcp"}}}' > "$mcp_scope_home/.claude.json"
...
# broken server looked identical to a working one. Proven against a
# SYNTHETIC descriptor carrying bin/initMarker (no live manifest item still
# carries both fields — HIMMEL-2634 removed the one that did, tokensave-mcp —
# so this block builds its own fixture rather than depend on one), keeping
# the registered command itself pinned to an absolute-path stub that always
...
fixture_mcp_item='{ "id": "fixture-mcp", "probe": { "type": "mcp-registered", "server": "fixture", "bin": "fixture-bin", "initMarker": ".fixture-marker" } }'
mcp_deep_home="$work/mcp-deep-home"; mkdir -p "$mcp_deep_home"
: > "$mcp_deep_home/fixture-mcp-stub"
chmod +x "$mcp_deep_home/fixture-mcp-stub"
printf '{"mcpServers":{"fixture":{"command":"%s"}}}' "$(winpath "$mcp_deep_home/fixture-mcp-stub")" > "$mcp_deep_home/.claude.json"
mcp_deep_target_init="$work/mcp-deep-target-init"; mkdir -p "$mcp_deep_target_init/.fixture-marker"
...
pathDeepPresent=$(build_path "$deep_bin_present" bash git jq -- fixture-bin)
cat > "$deep_bin_present/fixture-bin" <<'STUB'
#!/usr/bin/env bash
exit 0
STUB
chmod +x "$deep_bin_present/fixture-bin"

deep_bin_absent="$work/deep-bin-absent"; mkdir -p "$deep_bin_absent"
pathDeepAbsent=$(build_path "$deep_bin_absent" bash git jq -- fixture-bin)

outDeepP=$(PATH="$pathDeepPresent" "$node_bin" -e "
const { runProbe } = require('$probes_lib_w');
const item = $fixture_mcp_item;
...
echo "$outDeepP" | jq -e '.actual == "present"' >/dev/null \
  || fail "mcp-registered deepened (fixture-mcp) fully satisfied (bin resolvable + .fixture-marker present) should read present: (got: $outDeepP)"
...
echo "$outDeepBinMissing" | jq -e '.actual == "degraded"' >/dev/null \
  || fail "mcp-registered deepened (fixture-mcp) missing binary should read degraded, not present: (got: $outDeepBinMissing)"
...
echo "$outDeepNoInit" | jq -e '.actual == "degraded"' >/dev/null \
  || fail "mcp-registered deepened (fixture-mcp) missing initMarker (no .fixture-marker/ in target) should read degraded: (got: $outDeepNoInit)"
...
# OUTER shell's PATH is scrubbed of any real `fixture-bin` (so node's own
...
cat > "$deep_bin_ctxonly/fixture-bin" <<'STUB'
#!/usr/bin/env bash
exit 0
STUB
chmod +x "$deep_bin_ctxonly/fixture-bin"
scrub_stub="$work/scrub-stub-bin"; mkdir -p "$scrub_stub"
pathScrubbedOfFixtureBin=$(build_path "$scrub_stub" bash git jq -- fixture-bin)
outDeepCtxEnvPath=$(PATH="$pathScrubbedOfFixtureBin" "$node_bin" -e "
const { runProbe } = require('$probes_lib_w');
const item = $fixture_mcp_item;
...
# stub dir from the bin/initMarker block above (already has 'fixture-bin').
mcpcmd_bare_home="$work/mcpcmd-bare-home"; mkdir -p "$mcpcmd_bare_home"
printf '{"mcpServers":{"srv":{"command":"fixture-bin"}}}' > "$mcpcmd_bare_home/.claude.json"
...
#
# Scheduler responses are faked via stub `schtasks`/`crontab` executables
# wired through pipeline-cadence.sh's own PIPELINE_SCHTASKS/PIPELINE_CRONTAB
# test seams (the same seams scripts/luna/test-pipeline-cadence.sh's own
# suite uses) — real stub processes on this suite's own hermetic-executable
# convention, never a JS mock. pipeline-cadence.sh dispatches `status` to
# cmd_status (schtasks) on Windows and cron_status (crontab -l) everywhere
# else (see its own trailing `case "$SUBCMD"` dispatch) — WITHOUT the
# `crontab` stub, cron_status's `cron_read` shells out to the REAL system
# crontab, so this probe's actual/cleanAbsence verdict silently depends on
# whatever cadence entries happen to be armed on the machine running the
# suite (HIMMEL-2642: this read GREEN on a CI runner with an empty crontab
# and RED, in the opposite direction, on a station with the real pipeline
# cadence actually armed). Both fakes are driven off the SAME cc_sched_state
# marker directory below, so one `touch`/`rm` against a task marker flips
# both the Windows and POSIX code paths identically — the suite's outcome no
# longer depends on which platform branch pipeline-cadence.sh took, or on
# this machine's real scheduler/crontab state. The Windows wscript
# smoke-probe cadence_registered_status runs underneath is neutralized the
# same way that suite does it (CADENCE_WSCRIPT_BIN / CADENCE_WSH_POWERSHELL
# fakes), so an ARMED read is deterministic instead of depending on this
# machine's real WSH policy.
...
# HIMMEL-2642: the POSIX twin of cc_schtasks, wired through pipeline-cadence.sh's
# own PIPELINE_CRONTAB test seam (cron_read's `"${PIPELINE_CRONTAB:-crontab}" -l`)
# — cron_status is what `status` actually dispatches to on Linux/macOS (see the
# trailing `case "$SUBCMD"` in pipeline-cadence.sh), so without this fake the
# probe reads the REAL user crontab. `-l` lists one synthetic line per marker
# file under $cc_sched_state/tasks/ carrying the `# HIMMEL-Pipeline-<Name>`
# suffix cron_status greps for — the exact marker convention the real armed
# crontab entries also use — so the SAME `touch`/`rm` against a task marker
# that drives cc_schtasks above drives this fake identically. `fail-query`
# fails `-l` itself (a real crontab error, not the trusted "no crontab yet"
# rc=1/empty-stderr signature cron_read special-cases), exercising the same
# "scheduler unqueryable" path the schtasks fake's own fail-query does.
cc_crontab="$work/cc-crontab-fake.sh"
cat > "$cc_crontab" <<FAKE
#!/usr/bin/env bash
STATE="$cc_sched_state"
FAKE
cat >> "$cc_crontab" <<'FAKE'
if [ -e "$STATE/fail-query" ]; then
    echo "crontab: temp file error - errno 13" >&2
    exit 1
fi
if [ "$1" = "-l" ]; then
...
# HIMMEL-2642: armedPipelineSchedules (probes.js) keys the runner-file
# extension on `ctx.platform || process.platform` — `.bat` on win32, `.sh`
# everywhere else — and this ctx never sets ctx.platform, so it resolves
# against the REAL host running the suite. A `.bat`-only fixture reads
# "no leg armed" (a false cleanAbsence) on any non-Windows host. Creating
# both extensions makes the case host-independent, same as this suite's
# other cross-platform fixtures.
: > "$ea_bat/pipeline-health.bat"
: > "$ea_bat/pipeline-health.sh"
