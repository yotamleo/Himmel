...
# vaultPath case in test-wizard-probes.sh — and that suite's `fail()` exits
# the WHOLE suite on first failure, so an entirely UNRELATED earlier failure
# in that long suite (the cadence-coherence PIPELINE_CRONTAB gap this same
# ticket fixed) silently masked this exact regression for as long as it was
# outstanding: the case never even ran. A direct, non-probe call into the
# library itself cannot be masked that way. Every other input class (`~/`,
# bare `~`, a non-tilde path, and a bare `~something` with no separator) is
# asserted in the SAME pass to stay byte-identical to before this fix, so a
# future change to any of those branches trips this case too, not just the
# `~\` one.
#
# Panel finding (head f4dede6a, codex-1, AGREED/Important): `expandHome`
# returns `path.join(...)`, which is `\`-separated under native Windows Node
# — hand-asserting a hardcoded `/`-joined expected string here would be the
# EXACT mirror of the defect this case exists to catch (it would fail on
# Windows while the function behaves correctly). Every expected value below
# is therefore built with Node's own `path.join` INSIDE this same node -e
# call and compared joined-to-joined — nothing is hand-assembled with `/` or
# `\` on either the node or the jq/shell side. `expectedNested` is built as
# `path.join(home, 'Documents', 'luna')` — three SEPARATE segment
# arguments, never a pre-joined 'Documents/luna' or 'Documents\\luna'
# string — because that is the one comparison that actually proves the
# result has real, distinct nested segments rather than one mangled
# 'Documents\luna' segment (the regression this whole case guards against);
# joining a pre-built remainder string would silently re-admit the bug this
# case exists to catch. `~something` (bareTildeNoSep) has no separator to
# normalize at all — expandHome returns it verbatim — so it's compared
# literally, against the SAME `inputBareTildeNoSep`/`inputNonTilde`
# variables node used as the call argument (never retyped in the jq
# expression), so there is nothing to hand-assemble there either.
homeL="$work/homeL"; mkdir -p "$homeL"
outL=$(HOME="$homeL" USERPROFILE="$(winpath "$homeL")" "$node_bin" -e "
const { expandHome } = require(process.env.STATUS_REPORT_LIB);
const path = require('path');
const home = process.env.HOME;
const inputNonTilde = '/already/absolute/path';
const inputBareTildeNoSep = '~something';
console.log(JSON.stringify({
  backslash: expandHome('~\\\\Documents\\\\luna'),
  forwardSlash: expandHome('~/Documents/luna'),
  bareTilde: expandHome('~'),
  nonTilde: expandHome(inputNonTilde),
  bareTildeNoSep: expandHome(inputBareTildeNoSep),
  expectedNested: path.join(home, 'Documents', 'luna'),
  home,
  inputNonTilde,
  inputBareTildeNoSep,
}));
")
echo "$outL" | jq -e '.backslash == .expectedNested' >/dev/null \
  || fail "case l: a ~\\-prefixed path must expand to path.join(\$HOME, 'Documents', 'luna') -- REAL nested directory segments, not one mangled 'Documents\\luna' segment (got: $outL)"
echo "$outL" | jq -e '.forwardSlash == .expectedNested' >/dev/null \
  || fail "case l: a ~/-prefixed path must resolve to the SAME platform-native join as the ~\\ case above — behaviour byte-identical to before this fix (got: $outL)"
echo "$outL" | jq -e '.bareTilde == .home' >/dev/null \
  || fail "case l: a bare ~ must still expand to exactly \$HOME (got: $outL)"
echo "$outL" | jq -e '.nonTilde == .inputNonTilde' >/dev/null \
  || fail "case l: a non-tilde path must be returned unchanged (got: $outL)"
echo "$outL" | jq -e '.bareTildeNoSep == .inputBareTildeNoSep' >/dev/null \
  || fail "case l: a bare ~something (no separator) must still fall through unexpanded — expandHome's own header comment documents this as deliberate (got: $outL)"
echo "ok: case l — expandHome() direct unit coverage (platform-neutral: every expectation built via Node's own path.join, none hand-assembled with / or \\): ~\\ now resolves to real nested directory segments; ~/, bare ~, a non-tilde path, and a bare ~something all stay byte-identical to before"
