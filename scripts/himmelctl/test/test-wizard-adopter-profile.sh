...
# ── caseAT (HIMMEL-2693): the install trusts the workspace it just wired ────
# Before this ticket, himmelctl wrote permissions.allow /
# permissions.additionalDirectories into .claude/settings.json but never
# trusted the workspace, so Claude Code silently ignored both until a human
# ran `claude` interactively once — the install summary never said so either.
# bin.js's applyWorkspaceTrust() now calls the existing
# scripts/lib/ensure-workspace-trust.sh (HIMMEL-386) for the install target;
# a failure there is non-fatal (the helper's own contract) and becomes a
# still-manual row instead.
#
# AT1 -- the renderer.
# shellcheck disable=SC2016
"$node_bin" -e '
...
const applied = a.buildSummary(ans, [], { derived:"x", dryRun:false,
  workspaceTrust: { applied:true, dir:"/some/dir" } });
if (!applied.installed.some((l) => /workspace trusted: \/some\/dir/.test(l))) fail("a successful trust write produced no installed row");
if (!applied.installed.some((l) => /hasTrustDialogAccepted/.test(l))) fail("the success row must name the key it wrote");
if (/workspace trust/.test(manualText(applied))) fail("a successful trust write must not ALSO produce a still-manual row");

const failedRun = a.buildSummary(ans, [], { derived:"x", dryRun:false,
  workspaceTrust: { applied:false, dir:"/some/dir", reason:"could not write /some/home/.claude.json: EACCES" } });
const ft = manualText(failedRun);
if (!/workspace trust NOT set/.test(ft)) fail("a failed trust write produced no still-manual row");
if (!/permissions\.allow/.test(ft)) fail("the row must state the consequence (permissions.allow/additionalDirectories ignored)");
if (!/EACCES/.test(ft)) fail("the row must carry the underlying reason");
if (!/accept the trust dialog/.test(ft)) fail("the row must carry the exact remedy");
if (/workspace trusted/.test(failedRun.installed.join("\n"))) fail("a failed trust write must not be reported as installed");

const dry = a.buildSummary(ans, [], { derived:"x", dryRun:true, workspaceTrust: null });
if (!dry.planned.some((l) => /workspace trust/.test(l))) fail("--dry-run must preview the trust step, not stay silent");
if (dry.installed.length !== 0) fail("--dry-run must never claim anything was installed");

const noProbe = a.buildSummary(ans, [], { derived:"x", dryRun:false, workspaceTrust: undefined });
if (/workspace trust/i.test(noProbe.installed.join("\n") + manualText(noProbe))) fail("no workspaceTrust result at all must produce no row (never claim a step that did not run)");
' "$(winpath "$repo_root/scripts/himmelctl/lib/adopter-profile.js")" \
  || fail "caseAT1: buildSummary does not report workspace trust honestly"

# AT2 -- the probe, against the REAL ensure-workspace-trust.sh (RED controls,
# both directions, HOME-isolated via WORKSPACE_TRUST_CONFIG — the helper's own
# test seam, scripts/lib/test-ensure-workspace-trust.sh — never the operator's
# real ~/.claude.json).
atTarget="$work/at-target"; mkdir -p "$atTarget"
atCfgOk="$work/at-cfg-ok.json"
# shellcheck disable=SC2016
probe_trust() {
  # $1 = WORKSPACE_TRUST_CONFIG, $2 = target dir to cd into. Prints the JSON result.
  ( cd "$2" && TRUST_WRITE_JITTER_MS=0 WORKSPACE_TRUST_CONFIG="$1" "$node_bin" -e '
console.log(JSON.stringify(require(process.argv[1]).applyWorkspaceTrust()));
' "$(winpath "$repo_root/scripts/himmelctl/bin.js")" )
}
...
[ -f "$atCfgOk" ] || fail "caseAT2: ensure-workspace-trust.sh did not create $atCfgOk"
"$node_bin" -e '
...
# never a thrown/rc!=0 probe (the helper's own non-fatal contract). A REGULAR
# FILE where a directory is expected (rather than chmod 555) so the write
# fails deterministically (ENOTDIR) regardless of root/Windows/Git-Bash
# permission semantics, which do not honor a directory's write bit the same
# way everywhere (panel finding [codex-1]).
atHomeBad="$work/at-home-bad"; : > "$atHomeBad"
atBadCfg="$atHomeBad/.claude.json"
atBadOut=$(probe_trust "$atBadCfg" "$atTarget") || fail "caseAT2: the probe function itself must not fail even when the write fails"
grepq "$atBadOut" '"applied":false' \
  || fail "caseAT2: an unwritable config dir must probe applied:false, got [$atBadOut]"
grepq "$atBadOut" '"reason"' \
