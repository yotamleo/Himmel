...
// HIMMEL-3306: a hook file that pre-commit generated. With no
// .pre-commit-config.yaml beside it, it prints "config file not found.
// Skipping" and exits 0 on every commit -- present, but it enforces nothing.
// Any other hook body (a native himmel dispatcher, an adopter's own script) is
// counted as enforcing: this probe cannot judge what a foreign hook does.
function isInertPrecommitStub(hookPath, configPresent) {
  if (configPresent) return false;
  try {
    return /File generated by pre-commit/.test(fs.readFileSync(hookPath, 'utf8'));
  } catch (_e) {
    return false;
  }
}
...
function probeGitHooks(item, ctx) {
  const targetPath = ctx.targetPath || ctx.repoRoot;
  const hooksDir = gitHooksDir(targetPath);
  if (!hooksDir) {
  ...
  if (missing.length === 0) {
    // Installed is not enforcing (HIMMEL-3306): stubs with no config gate nothing.
    const configPresent = fs.existsSync(path.join(targetPath, '.pre-commit-config.yaml'));
    const inert = item.probe.hooks.filter((hook) => isInertPrecommitStub(path.join(hooksDir, hook), configPresent));
    if (inert.length > 0) {
      const actual = inert.length === item.probe.hooks.length ? 'absent' : 'degraded';
      return {
        actual,
        detail: `hook type(s) installed but inert — pre-commit stubs with no .pre-commit-config.yaml in ${targetPath} skip on every run: ${inert.join(', ')} (${hooksDir}); re-run himmelctl install to place the native gates`,
      };
    }
...
// server (graphify-mcp carries scopes:["project", "user"]) always read
// absent. Both files share the same
...
function probeMcpRegistered(item, ctx) {
  ...
  // exist, proving the CONSUMING project is actually initialized (e.g. a
  // per-checkout-indexed MCP server is registered-but-useless without its
  // own project graph) — is an OPTIONAL descriptor field, opt-in per item; a
  // probe naming none keeps the registered-command check as the only
  // deepening. `bin` (a descriptor-named binary that must resolve on PATH,
  // independent of what's actually registered) is ALSO still supported for
  // an item where it adds real signal beyond the registered-command check
  // (e.g. a server whose registered command happens to be the same binary,
  // but `bin` catches a PATH-vs-registered-path mismatch the
  // registered-command check alone wouldn't); graphify-mcp's `bin` was
...
// for the mcp-registered bin/initMarker deepening): update_codex() itself
// treats a binary with no `$CODEX_HOME/plugins/cache` dir as "never
// provisioned, skip" — so this probe
