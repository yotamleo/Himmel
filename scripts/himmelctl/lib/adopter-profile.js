...
function buildSummary(answers, laneRows, opts) {
  ...
  // HIMMEL-2693: bin.js's applyWorkspaceTrust() already RAN
  // ensure-workspace-trust.sh for the install target. A trust-seed failure is
  // reported here, not thrown: the helper's own contract is that this step is
  // never fatal to the install, so the entries it just wired into
  // .claude/settings.json stay silently ignored until the adopter runs the
  // remedy below.
  //
  // `o.workspaceTrust === null` (never `undefined`) is the explicit --dry-run
  // signal from the call site, same convention as gitHooks/userSlug, but
  // unlike those two read-only probes this IS the action, so a preview must
  // still say so rather than staying silent (caseJ's structural dry/applied
  // symmetry check never sets this field at all, so `undefined` stays silent
  // in both directions — only an explicit `null` previews).
  if (o.workspaceTrust) {
    if (o.workspaceTrust.applied) {
