...
//   3 = declined by the operator or teardown refused
//   a wet run against a non-fixture HOME,
//   any other non-zero code = the teardown script's own rc.
...
commands:
  ...
  profile                 inspect/toggle the two-tier plugin profile — thin
                          ...
                          (same engine as /profile):
                            profile list [--json]
                            profile lean | profile full
                            profile enable <spec> | profile disable <spec>
                          <spec> is plugin@marketplace, or a bare name when
                          unambiguous; --dry-run prints the plugin-toggle
...
function parseArgs(argv) {
  ...
  if (args.subcommand !== null) {
    ...
    // HIMMEL-2838: bare --scope loads the same kind of shipped profile
    // (cmdInstall step 0) and hits the identical silent-drop hazard, so it
    // gets the same refusal, not just --from-profile.
    if (args.subcommand === 'install' && (args.fromProfile !== null || args.scope !== null)) {
      for (const key of ['lanes', 'withCodex', 'withHermes']) {
        if (args[key] !== OPTION_DEFAULTS[key]) {
          const combinedWith = args.fromProfile !== null ? '--from-profile' : '--scope';
          console.error(`himmelctl: ${OPTION_FLAGS[key]} cannot be combined with ${combinedWith}`);
          console.error(`  (the profile already carries its lane selection — edit the profile, or drop ${combinedWith})`);
          process.exitCode = 2;
          return args;
        }
      }
...
// Interactive iff stdin is a TTY and --dry-run was not passed. --scope is a
// discoverable non-interactive door (HIMMEL-2752): it loads a shipped
// profile instead of walking the wizard, so a confirm prompt here would
// contradict its own "non-interactive install (no wizard)" framing when run
// from a real TTY. The HIMMELCTL_INTERACTIVE env var (1/0) forces a side for
// automation/CI and for hermetic tests that cannot allocate a real PTY; when
// unset, TTY detection decides.
function isInteractive(args) {
  if (args.dryRun || args.scope) return false;
...
// Should we prompt the user with the question engine? --from-profile and
// --scope (HIMMEL-2752: the shipped-profile non-interactive door) both skip
// it (answers come from a file either way — T3); HIMMELCTL_INTERACTIVE forces
// a side for CI / hermetic tests that cannot allocate a real PTY; otherwise
// stdin being a TTY decides.
function shouldPrompt(args) {
  if (args.fromProfile || args.scope) return false;
  const v = process.env.HIMMELCTL_INTERACTIVE;
  if (v === '1') return true;
  if (v === '0') return false;
  return Boolean(process.stdin.isTTY);
...
//     session as consent to run an installer. A caller may pass its own EOF
//     value to distinguish stdin running out from a human typing n (HIMMEL-2755).
...
// HIMMEL-2693: himmelctl wires permissions.allow / permissions.additionalDirectories
// into .claude/settings.json, but Claude Code ignores both until the workspace
// is trusted (projects[<dir>].hasTrustDialogAccepted in ~/.claude.json) — a
// human has to run `claude` interactively there once and accept the trust
// dialog first. scripts/lib/ensure-workspace-trust.sh (HIMMEL-386) already owns
// that exact read-modify-write and is called by arm-resume.sh and the luna
// cadence; this delegates to it for the install target rather than
// re-implementing the ~/.claude.json write in JS — same shell-out convention
// as install-plugins.sh (deriveCommand()) and the two probes above.
//
// A trust-seed failure is NOT fatal to the install (the helper's own
// contract, see its header) — the caller reports it as a still-manual row
// instead of aborting. Not called under --dry-run (see the call site): a
// preview must never write to ~/.claude.json.
function applyWorkspaceTrust() {
  const target = process.cwd();
  // CR finding [coderabbitai]: report the SAME key form the helper actually
  // writes to ~/.claude.json, not the raw process.cwd() — on native Windows
  // node, process.cwd() returns backslashes, but the helper's own key
  // derivation runs everything through `cygpath -m` there, so a `\`-form dir
  // in the still-manual remedy (`projects["<dir>"]...`) would not match the
  // real key. toBashPath() (backslash -> forward-slash) reproduces cygpath
  // -m's output for an already-drive-letter Windows path, and is a no-op on
  // POSIX — same value the helper receives as its own argv[1].
  const dir = toBashPath(target);
  const script = path.join(repoRoot(), 'scripts', 'lib', 'ensure-workspace-trust.sh');
  if (!fs.existsSync(script)) {
...
async function printAdopterEpilogue(answers, derived, dryRun, pluginResult, vaultScaffolded, lunaResult) {
  ...
  const summary = adopterProfileLib.buildSummary(answers, rows, {
    ...
    // HIMMEL-2693: null under --dry-run — a preview must never write to
    // ~/.claude.json (this epilogue also renders the PREVIEW, before adopt.sh
    // has been spawned at all, same reasoning as gitHooks/userSlug above).
    workspaceTrust: dryRun ? null : applyWorkspaceTrust(),
  });
...
async function runPlan(answers, args) {
  ...
  // must not leave the adopter with no `himmelctl` on PATH to run
  // `himmelctl ensure` and resume. Best-effort, same posture as adopt.sh's own
  // git-hook placement (HIMMEL-2818): never blocks on or masks adopt.sh's rc.
  const shimOk = applyHimmelctlPathShim(args);
  if (rc !== 0) {
    if (shimOk) console.error('himmelctl: install failed (see above), but the himmelctl launcher is on PATH — resolve the error, then run `himmelctl ensure` to resume.');
    return rc;
  }
    ...
    if (answers.channel === undefined) {
      if (isNewAdopter) {
        ...
        try {
          ...
          // Same enum loadProfile()'s checkEnum('channel', ...) enforces on
          // a --from-profile file — this raw cache read bypasses that
          // validator, so re-check it here rather than trusting any string.
          if (['stable', 'pre'].includes(prior.channel)) answers.channel = prior.channel;
        } catch {
...
// uninstall.sh's documented steps (jira-env-keys' .env, qmd-index,
...
// ── profile (HIMMEL-2733) ─────────────────────────────────────────────────
//
// A THIN passthrough, same shell-out shape as deriveUpdateCommand/cmdUpdate
// (resolveBash() + toBashPath() for the Windows Git-Bash path fix): forward
// argvRest VERBATIM to plugin-profile.sh and propagate its exit code. The
// script owns its own verb/flag grammar (list/lean/full/enable/disable,
// --json, --dry-run, --template PATH, a bare-or-full <spec>) plus every bit
// of the resolution/refusal logic — none of that is reimplemented here.
function cmdProfile(argvRest) {
  const script = toBashPath(path.join(repoRoot(), 'scripts', 'machine-setup', 'plugin-profile.sh'));
  return runSpawn({ argv: [resolveBash(), script, ...argvRest] });
}
...
async function cmdEnsure(args) {
  ...
  if (args.items) {
    ...
    // item, or any item with no `install` descriptor at all) never lands
    // in towardEnabled even when it's named in --items, so the dependent
    // would still install on top of a genuinely-missing prerequisite.
    // `--items` membership is never sufficient on either side (enabled or
    // disabled) — only ACTUAL convergence this run is.
...
// claude-plugins-pluginSet, graphify-mcp) would be left wired
...
async function main() {
  ...
  // `profile` is a THIN PASSTHROUGH to plugin-profile.sh (HIMMEL-2733) with
  // its OWN verb/flag grammar (list/lean/full/enable/disable, --json,
  // --dry-run, --template PATH, a bare-or-full <spec>) — same reason `config`
  // is special-cased above, before parseArgs/ALLOWED_OPTIONS (flags-only) can
  // see it. Only the leading `profile` token is consumed; everything after it
  // reaches the script verbatim, unlike `config`'s own --dry-run skip (that
  // flag belongs to plugin-profile.sh's own grammar here, not a global one).
  if (argv[0] === 'profile') {
    return cmdProfile(argv.slice(1));
  }
