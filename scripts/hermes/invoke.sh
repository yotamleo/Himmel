...
case "$flag_reason" in
    ...
    *)
        ...
        if [ -n "$budget_line" ]; then
            ...
            if [ "$rc" -ne 0 ]; then
                outcome=error
                # HIMMEL-2751: until now this branch was SILENT. The three
                # loud paths above (124/125/126) are the ones this script
                # itself decides; an ordinary non-zero rc from hermes just
                # propagated with nothing on stderr — and because the spawn
                # above is `> >(tee "$capture_log") 2>&1`, hermes' own
                # diagnosis went to STDOUT, not stderr. Any caller that reads
                # stderr to explain a failure (scripts/telegram/triage.ts did
                # exactly that) therefore got an empty string. The telegram
                # bridge logged `triage exited 1:` for two days while every
                # group message fail-opened to the expensive lane, because
                # "No usable credentials found for provider 'deepseek'" was
                # sitting on the stream nobody was reading. Echo the rc and a
                # tail of the capture to stderr so the reason travels with the
                # failure. model/provider only — invoke.sh never receives a
                # credential in argv, so this cannot leak one.
                fail_tail=""
                if [ -n "$capture_log" ] && [ -f "$capture_log" ]; then
                    fail_tail="$(tail -n 20 "$capture_log" 2>/dev/null | tr -d '\r')"
                fi
                echo "invoke.sh: hermes exited $rc (model=${model:-<hermes default>} provider=${provider:-<hermes default>}) — output tail: ${fail_tail:-<no output captured>}" >&2
            fi
