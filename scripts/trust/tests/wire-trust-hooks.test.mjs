...
//
// HIMMEL-1560: the install path now handshakes the recorder, so a fixture with
// a recorder must ship one that can actually answer. A '// stub' comment exits
// 0 with empty stdout, which the handshake reads (correctly) as INCAPABLE — so
// every install test would refuse on the stub before reaching the wiring it is
// actually about. Copying the REAL recorder makes these tests exercise wiring
// against a genuine capability response, and it cannot drift out of sync the
// way a hand-written fake would the day ENTRIES grows a new verb.
function fixture(name, obj = BASE, { withRecorder = true } = {}) {
  const root = path.join(TMP, name);
  fs.mkdirSync(path.join(root, '.claude'), { recursive: true });
  if (withRecorder) {
    fs.mkdirSync(path.join(root, 'scripts', 'trust'), { recursive: true });
    fs.copyFileSync(REAL_RECORDER, path.join(root, 'scripts', 'trust', 'shadow-ledger.mjs'));
  }
...
describe('a noncanonical ledger invocation is refused, not silently doubled', () => {
  ...
  test('a differently-formatted settings file is REFUSED, not reformatted', () => {
    ...
    fs.mkdirSync(path.join(root, 'scripts', 'trust'), { recursive: true });
    // Real recorder (HIMMEL-1560): install handshakes the recorder BEFORE the
    // format check, so a '// stub' would refuse on capability and never reach
    // the formatting guard this test exists for.
    fs.copyFileSync(REAL_RECORDER, path.join(root, 'scripts', 'trust', 'shadow-ledger.mjs'));
    fs.mkdirSync(path.join(root, '.claude'), { recursive: true });
...
// HIMMEL-1560 — the capability handshake. Existence is not compatibility: a
// newer wiring tool pointed at an older recorder installs hooks invoking verbs
// that recorder does not implement, and the recorder's silent fall-through
// (fail-open is the hook contract) means those hooks fire, exit 0, write
// nothing, while `trust status` reports N/N. The handshake asks the artefact
// at the resolved path to PROVE it implements every verb this tool wires, and
// fails closed on every answer it cannot trust.
describe('the capability handshake (HIMMEL-1560)', () => {
  const REQUIRED = ENTRIES.map((s) => s.verb);

  // A fixture whose recorder is a hand-written script rather than the real one,
  // for the synthetic fail-closed cases where we control exactly what the
  // recorder answers (or whether it answers at all). For the skew case the real
  // historical recorder is used instead — a stub would not prove the actual skew
  // is caught.
  function withFakeRecorder(name, body, obj = BASE) {
    const root = path.join(TMP, name);
    fs.mkdirSync(path.join(root, 'scripts', 'trust'), { recursive: true });
    fs.writeFileSync(path.join(root, 'scripts', 'trust', 'shadow-ledger.mjs'), body, 'utf8');
    fs.mkdirSync(path.join(root, '.claude'), { recursive: true });
    const p = path.join(root, '.claude', 'settings.json');
    fs.writeFileSync(p, serialize(obj), 'utf8');
    return p;
  ...
  test('the current recorder passes the handshake with all seven verbs', () => {
    const cap = recorderCapabilities(fixture('cap-current'));
    assert.equal(cap.ok, true);
    assert.deepEqual(cap.verbs.slice().sort(), REQUIRED.slice().sort());
  });

  test('a recorder that answers nothing is INCAPABLE, not a pass', () => {
    // An old recorder has no `capabilities` branch; the silent fall-through
    // exits 0 with empty stdout. Silence is the honest signal of incapability
    // — reading it as a pass is the exact defect this handshake exists to close.
    const cap = recorderCapabilities(
      withFakeRecorder('cap-empty', '// no capabilities branch: silent fall-through, exit 0\n'),
    );
    assert.equal(cap.ok, false);
    assert.equal(cap.reason, 'empty_stdout');
    assert.deepEqual(cap.missing, REQUIRED, 'an empty answer proves nothing, so every required verb is missing');
  });

  test('a non-zero exit is refused', () => {
    const cap = recorderCapabilities(
      withFakeRecorder('cap-nonzero', 'if (process.argv[2] === "capabilities") process.exit(2);\n'),
    );
    assert.equal(cap.ok, false);
    assert.equal(cap.reason, 'nonzero_exit');
  });

  test('malformed JSON is refused', () => {
    const cap = recorderCapabilities(
      withFakeRecorder('cap-malformed', 'if (process.argv[2] === "capabilities") process.stdout.write("{not json");\n'),
    );
    assert.equal(cap.ok, false);
    assert.equal(cap.reason, 'unparseable_json');
  });

  test('a wrong recorder identity is refused', () => {
    const body = 'if (process.argv[2] === "capabilities") process.stdout.write(JSON.stringify({recorder:"not-the-ledger",verbs:'
      + JSON.stringify(REQUIRED) + '}));\n';
    const cap = recorderCapabilities(withFakeRecorder('cap-wrongid', body));
    assert.equal(cap.ok, false);
    assert.equal(cap.reason, 'wrong_recorder_id', 'the verbs cannot be trusted from a responder with the wrong identity');
  });

  test('a non-array verb list is refused', () => {
    const body = 'if (process.argv[2] === "capabilities") process.stdout.write(JSON.stringify({recorder:"himmel-shadow-ledger",verbs:"pre"}));\n';
    const cap = recorderCapabilities(withFakeRecorder('cap-nonarray', body));
    assert.equal(cap.ok, false);
    assert.equal(cap.reason, 'verbs_not_string_array');
  });

  test('a verb list with a non-string entry is refused', () => {
    const body = 'if (process.argv[2] === "capabilities") process.stdout.write(JSON.stringify({recorder:"himmel-shadow-ledger",verbs:["pre",42]}));\n';
    const cap = recorderCapabilities(withFakeRecorder('cap-nonstring', body));
    assert.equal(cap.ok, false);
    assert.equal(cap.reason, 'verbs_not_string_array');
  });

  test('a recorder missing ONE verb is refused and that verb is named', () => {
    const advertised = REQUIRED.filter((v) => v !== 'heartbeat');
    const body = 'if (process.argv[2] === "capabilities") process.stdout.write(JSON.stringify({recorder:"himmel-shadow-ledger",verbs:'
      + JSON.stringify(advertised) + '}));\n';
    const cap = recorderCapabilities(withFakeRecorder('cap-missing-one', body));
    assert.equal(cap.ok, false);
    assert.equal(cap.reason, 'missing_verbs');
    assert.deepEqual(cap.missing, ['heartbeat']);
  });

  test('install REFUSES an incapable recorder and names the missing verb', () => {
    const advertised = REQUIRED.filter((v) => v !== 'notify');
    const body = 'if (process.argv[2] === "capabilities") process.stdout.write(JSON.stringify({recorder:"himmel-shadow-ledger",verbs:'
      + JSON.stringify(advertised) + '}));\n';
    const f = withFakeRecorder('cap-install-refuse', body);
    const original = fs.readFileSync(f, 'utf8');
    const r = run([], f);
    assert.equal(r.status, 1);
    assert.match(r.stderr, /did not prove it can record every verb/);
    assert.match(r.stderr, /notify/);
    assert.match(r.stderr, /record nothing/);
    assert.equal(fs.readFileSync(f, 'utf8'), original, 'an incapable recorder writes nothing');
  });

  // The real historical skew, not a synthetic stub. c49716d5 predates the
  // `capabilities` verb, so its recorder answers with empty stdout (its own
  // silent fall-through) — proving the handshake catches the actual drift the
  ...
  test('the historical recorder at c49716d5 is refused (real skew, not a stub)', (t) => {
    ...
    const root = path.join(TMP, 'cap-historical');
    fs.mkdirSync(path.join(root, 'scripts', 'trust'), { recursive: true });
    fs.writeFileSync(path.join(root, 'scripts', 'trust', 'shadow-ledger.mjs'), show.stdout, 'utf8');
    fs.mkdirSync(path.join(root, '.claude'), { recursive: true });
    const f = path.join(root, '.claude', 'settings.json');
    fs.writeFileSync(f, serialize(BASE), 'utf8');
    const original = fs.readFileSync(f, 'utf8');

    const cap = recorderCapabilities(f);
    assert.equal(cap.ok, false, 'the historical recorder must not pass the handshake');

    const r = run([], f);
    assert.equal(r.status, 1);
    assert.match(r.stderr, /did not prove it can record every verb/);
    // Whichever way the old recorder fails to answer, the refusal must name the
    // verbs it could not prove — silence is not a pass.
    assert.match(r.stderr, /\bpre\b/);
    assert.equal(fs.readFileSync(f, 'utf8'), original, 'nothing is written');
  });

  test('--off still works against an incapable recorder', () => {
    // Removing dead wiring is exactly what you want against an old recorder;
    // the handshake is install-only, never --off, so an incapable recorder must
    // not strand previously-wired hooks.
    const f = withFakeRecorder('cap-off-incapable', '// silent fall-through, exit 0\n');
    // Wired hooks in place, as if a capable recorder wired them and the checkout
    // has since been swapped for an older one.
    fs.writeFileSync(f, serialize(install(BASE).settings), 'utf8');
    assert.equal(statusOf(JSON.parse(fs.readFileSync(f, 'utf8'))).wired, 7);

    const r = run(['--off'], f);
    assert.equal(r.status, 0, r.stderr);
    assert.equal(statusOf(JSON.parse(fs.readFileSync(f, 'utf8'))).wired, 0);
  });

  test('--check against an incapable recorder REPORTS and writes nothing', () => {
    const advertised = REQUIRED.filter((v) => v !== 'heartbeat');
    const body = 'if (process.argv[2] === "capabilities") process.stdout.write(JSON.stringify({recorder:"himmel-shadow-ledger",verbs:'
      + JSON.stringify(advertised) + '}));\n';
    const f = withFakeRecorder('cap-check-incapable', body);
    const original = fs.readFileSync(f, 'utf8');
    const r = run(['--check'], f);
    assert.equal(r.status, 0);
    assert.match(r.stdout, /did not prove it can record every verb/);
    assert.match(r.stdout, /heartbeat/);
    assert.match(r.stdout, /trust on.*REFUSE/);
