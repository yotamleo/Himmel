...
// HIMMEL-1560: the dispatch table IS the capability declaration.
//
// The wiring tool used to assert only that a file EXISTS at the resolved
// recorder path, and existence is not compatibility. The verb set grew from
// three to six in HIMMEL-1539 and to seven in HIMMEL-1547, while the CLI
// dispatch below is a deliberate silent fall-through — an unrecognised verb
// matches no branch and returns 0 without recording anything, because
// fail-open is the hook contract. So a newer wiring tool pointed at an older
// checkout installs entries invoking verbs that recorder does not implement:
// those hooks fire, exit 0, write nothing, and `status` still reports 7/7.
//
// `capabilities` answers that question by printing THESE KEYS. Making the
// table the single source for both dispatch and advertisement is the point: a
// verb cannot be implemented without being advertised, or advertised without
// being implemented, because there is only one object. A version constant
// would drift from the code it describes; string-matching the source would
// pass on a file that merely mentions the verb names.
const HOOK_COMMANDS = Object.freeze({
  pre: cmdPre,
  post: cmdPost,
  postfail: cmdPostFailure,
  denied: cmdDenied,
  permreq: cmdPermissionRequest,
  notify: cmdNotify,
  heartbeat: cmdHeartbeat,
});

// An identity marker, so a caller can distinguish "the recorder answered my
// handshake" from "some other program happened to print JSON on stdout". A
// recorder predating this verb prints NOTHING and exits 0 — silence is the
// answer the fall-through gives — so callers must treat empty stdout as
// INCAPABLE rather than as a pass.
const RECORDER_ID = 'himmel-shadow-ledger';

export function capabilities() {
  return { recorder: RECORDER_ID, verbs: Object.keys(HOOK_COMMANDS).sort() };
}
...
function main(argv) {
  ...
  // Answered BEFORE the hook paths below, because those read stdin and this is
  // a tool-to-tool query with no payload — reading stdin here would hang the
  // handshake against a caller that has nothing to send.
  if (cmd === 'capabilities') {
  ...
  // Unchanged behaviour: an unrecognised verb still falls through silently and
  // returns 0. That is the hook contract, not an oversight — a non-zero
  // PreToolUse exit BLOCKS the tool call. What HIMMEL-1560 adds is a way for
  // the WIRING tool to find out in advance, instead of discovering it as
  // missing rows.
  const handler = Object.hasOwn(HOOK_COMMANDS, cmd) ? HOOK_COMMANDS[cmd] : null;
  if (handler) handler(payload, now);
  return 0;
}
