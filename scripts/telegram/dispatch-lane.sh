...
# Some stdin lanes require an explicit wrapper opt-in, before model args.
STDIN_BRIEF_FLAG="$(printf '%s\n' "$LANE_JSON" | jq -r '.dispatch.flags.stdinBrief // ""')"
if [ "$DELIVERY" = "stdin" ] && [ -n "$STDIN_BRIEF_FLAG" ]; then
  CMD[${#CMD[@]}]="$STDIN_BRIEF_FLAG"
fi
