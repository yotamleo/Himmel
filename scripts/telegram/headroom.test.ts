// scripts/telegram/headroom.test.ts
// WS9 (HIMMEL-654) — headroom ledger tests. Path resolver, append helper,
...
const BASH_LIB = join(REPO_ROOT, "scripts", "lib", "headroom-ledger.sh").replaceAll("\\", "/");

let tmp: string;
beforeEach(() => { tmp = mkdtempSync(join(tmpdir(), "headroom-")); });
afterEach(() => { rmSync(tmp, { recursive: true, force: true }); });

const NOW_TS = "2026-07-04T12:00:00Z";

function rec(partial: Partial<HeadroomRecord>): HeadroomRecord {
  return {
...
test("HEADROOM_FIELDS canonical order (10 keys, schema-locked)", () => {
  expect([...HEADROOM_FIELDS]).toEqual([
    "v", "ts", "lane", "source", "used_pct", "window", "reset_at", "tier", "glm_peak", "note",
  ]);
});

test("no forbidden fields in the record schema (AC23/T23)", () => {
  const src = readFileSync(join(import.meta.dir, "headroom.ts"), "utf8");
  expect(src).not.toMatch(/fresh_for_s/);
  expect(src).not.toMatch(/concurrency/);
  // no $-budget field on the record line
  expect(serializeHeadroom(rec({}))).not.toMatch(/"budget"/);
});

test("ledgerPath honors HIMMEL_HEADROOM_LEDGER override (T22)", () => {
  expect(ledgerPath({ HOME: "/h", HIMMEL_HEADROOM_LEDGER: "/custom/x.jsonl" })).toBe("/custom/x.jsonl");
});

test("ledgerPath default is <HOME>/.himmel/headroom.jsonl under a Windows-style HOME (T22)", () => {
  expect(ledgerPath({ HOME: "C:/Users/test" })).toBe(join("C:/Users/test", ".himmel", "headroom.jsonl"));
});

test("appendHeadroom writes one \\n-terminated line; 10 keys canonical order; null rendered", () => {
  ...
  expect(keys).toEqual([...HEADROOM_FIELDS]);
...
test("appendHeadroom creates the parent dir on append (not on resolve)", () => {
  const nested = join(tmp, "deep", "dir", "x.jsonl");
  appendHeadroom(rec({}), nested);
  expect(existsSync(nested)).toBe(true);
});

test("byte-identical bash<->TS serialization (T22)", () => {
  const cases: Array<{ args: string[]; record: HeadroomRecord }> = [
    {
  ...
  for (const { args, record } of cases) {
    const tsLine = serializeHeadroom(record);
...
test("two interleaved appends -> 2 complete parseable lines (T16)", () => {
  ...
  appendHeadroom(rec({ lane: "glm", source: "monitor-endpoint", used_pct: 62, window: "5h", tier: "pro", glm_peak: true }), f);
  appendHeadroom(rec({ lane: "codex", source: "codex-sqlite", used_pct: 40, window: "weekly", tier: "pro", glm_peak: null }), f);
...
test("T17/AC6 same-lane concurrent appendHeadroom race -> no loss/merge", async () => {
  const f = join(tmp, "race.jsonl").replaceAll("\\", "/");
  // Lean volume: Cygwin fork is ~280ms/append (headroom_row's `date` + 4
  ...
  // probe; T17's job is the CODE PATH (headroom_append wraps printf >> with
  ...
  for (let w = 0; w < W; w++) {
    const script = [
      ...
      `i=0`,
      `while [ "$i" -lt ${R} ]; do`,
      `  headroom_append "$(headroom_row glm monitor-endpoint ${w} 5h "" "race-w${w}-i$i" "$ts")" 2>/dev/null || true`,
      `  i=$((i+1))`,
      `done`,
    ].join("\n");
    const sub = Bun.spawn(["bash", "-c", script], {
      stdout: "pipe", stderr: "pipe",
      env: { ...process.env, HIMMEL_HEADROOM_LEDGER: f },
    });
    subs.push(sub);
  }
...
// ── Task 2: headroomRead reader (T10-T15, T18, T21, T26) ─────────────────────
...
function writeLedger(rows: string[]): string {
  const p = join(tmp, "headroom.jsonl");
  writeFileSync(p, rows.length ? rows.join("\n") + "\n" : "");
  return p;
}

test("T10 fresh GLM row (age 30s < 120 budget) -> fresh:true status:known", () => {
  const p = writeLedger([serializeHeadroom(rec({ lane: "glm", used_pct: 62, ts: ago(30) }))]);
  const r = headroomRead({ path: p, nowMs: NOW_MS });
...
test("T11 stale GLM row (age 300s > 120 budget) -> fresh:false status:unknown", () => {
  const p = writeLedger([serializeHeadroom(rec({ lane: "glm", used_pct: 62, ts: ago(300) }))]);
  const r = headroomRead({ path: p, nowMs: NOW_MS });
...
test("T12 Claude budget 0 -> a 1s-old visible row still reads unknown", () => {
  const p = writeLedger([serializeHeadroom(rec({ lane: "claude", used_pct: 93, source: "arm-threshold", ts: ago(1) }))]);
  const r = headroomRead({ path: p, nowMs: NOW_MS });
...
test("T13 absent lane -> row null, status unknown (no fabricated 0/100)", () => {
  const p = writeLedger([serializeHeadroom(rec({ lane: "glm", used_pct: 62, ts: ago(10) }))]);
  const r = headroomRead({ path: p, nowMs: NOW_MS });
...
test("T14 latest invisible row -> unknown; earlier real reading NOT surfaced (F10)", () => {
  const p = writeLedger([
    serializeHeadroom(rec({ lane: "glm", used_pct: 40, ts: ago(20) })),                       // older real reading
    serializeHeadroom(rec({ lane: "glm", used_pct: null, source: "invisible", ts: ago(5) })), // newest = invisible
  ]);
  const r = headroomRead({ path: p, nowMs: NOW_MS });
  expect(r.glm.status).toBe("unknown");
...
test("T15 truncated trailing partial line skipped; latest-per-lane still correct", () => {
  const good = serializeHeadroom(rec({ lane: "glm", used_pct: 62, ts: ago(10) }));
  const p = join(tmp, "headroom.jsonl");
...
test("T18 GLM burst after a fresh Codex row does not evict Codex (within N)", () => {
  const rows: string[] = [serializeHeadroom(rec({ lane: "codex", used_pct: 50, window: "weekly", ts: ago(60) }))];
  for (let i = 0; i < 220; i++) rows.push(serializeHeadroom(rec({ lane: "glm", used_pct: 60, ts: ago(30) })));
  const r = headroomRead({ path: writeLedger(rows), nowMs: NOW_MS });
  expect(r.glm.status).toBe("known");
...
test("T26 absent lane beyond lookbackN -> unknown, scan bounded at N", () => {
  const rows: string[] = [];
  for (let i = 0; i < 200; i++) rows.push(serializeHeadroom(rec({ lane: "glm", used_pct: 60, ts: ago(30) })));
  const r = headroomRead({ path: writeLedger(rows), nowMs: NOW_MS, lookbackN: 50 });
...
test("T21/AC11 exactly one headroomRead impl; no second-language reader twin", () => {
  const src = readFileSync(join(REPO_ROOT, "scripts", "telegram", "headroom.ts"), "utf8");
  expect((src.match(/export function headroomRead/g) ?? []).length).toBe(1);
  expect(existsSync(join(REPO_ROOT, "scripts", "lib", "headroom-read.sh"))).toBe(false);
});
...
test("T1 buildGlmRow maps a live-shaped reading; round-trips; no fresh_for_s (AC1)", () => {
  ...
  expect(JSON.parse(serializeHeadroom(row)).used_pct).toBe(62);
  expect(serializeHeadroom(row)).not.toContain("fresh_for_s");
});
