...
export function composeWorkerPrompt(task: string, sessionDir: string, branch: string, opts?: { shared?: boolean }): string {
  ...
  return [
    ...
    `COMMIT EARLY (HIMMEL-1200): the MOMENT your tests pass — or the change is otherwise coherent — git commit your work on ${branch}, then keep refining in FOLLOW-UP commits. Do NOT hold all your work for one final commit: a committed-but-imperfect branch is recoverable by the parent, but an uncommitted timeout at the wall loses everything.`,
