// scripts/telegram/headroom-codex.ts
...
// headroom row. A best-effort PIGGYBACK on the (rare) hermes Codex dispatch:
...
export function buildCodexRow(parsed: { usedPct: number } | null, nowMs: number): HeadroomRecord {
...
export function codexProbeAppend(readBlob: () => string, nowMs: number, append: (row: HeadroomRecord) => void): void {
  let blob = "";
  try { blob = readBlob(); } catch { blob = ""; }
  const parsed = blob ? parseCodexUsage(blob) : null;
  if (parsed === null) console.error("headroom-codex: ~/.codex/logs_2.sqlite unreadable or no secondary used_percent — recording an invisible codex row");
