...
// HIMMEL-2734: a symlink is Linux's stand-in for Windows 8.3 aliasing: one
// directory has two spellings, so resolve() alone cannot make them compare
// equal. Capture real filesystem results at registration time so a Windows
// privilege refusal can register an actual SKIP, without skipping the separate
// fixture assertion. The finally removes both the alias and its target before
// any test runs; the control compares the captured spellings, not mock paths.
{
  const root = mkdtempSync(join(tmpdir(), "cx-path-alias-"));
  const realDir = join(root, "real");
  const aliasPath = join(root, "alias");
  const name = "HIMMEL-2734: RED alias spelling differs; GREEN realpath collapses it";
  try {
    mkdirSync(realDir);
    let canSymlink = true;
    try {
      symlinkSync(realDir, aliasPath, "dir");
    } catch (e) {
      const code = (e as NodeJS.ErrnoException).code;
      if (process.platform !== "win32" || (code !== "EPERM" && code !== "EACCES")) throw e;
      canSymlink = false;
    }
    if (!canSymlink) {
      test.skip(name, () => {});
    } else {
      const aliased = resolve(aliasPath);
      const canonical = realpathSync.native(aliasPath);
      const target = realpathSync.native(realDir);
      test(name, () => {
        expect(aliased).not.toBe(canonical); // RED control: the spelling hazard exists.
        expect(canonical).toBe(target); // GREEN: both spellings reach the real directory.
      });
    }
  } finally { rmSync(root, { recursive: true, force: true }); }
}

// HIMMEL-2734: control the TMP_ROOT/tmpdir() lockstep in removeFixture's
// under-tmp guard: a canonical child fails an aliased-root prefix check, but
// passes when the root is canonicalized the same way. This runs on Linux
// precisely because it constructs the alias itself, rather than relying on
// the host's tmpdir being aliased. Windows 8.3 behaviour proper is verified
// only by the public CI windows-latest leg, not here. Capture real paths at
// registration time to match the alias control's Windows privilege SKIP;
// finally removes the whole tree before the captured prefix checks run.
{
  const root = mkdtempSync(join(tmpdir(), "cx-tmp-lockstep-"));
  const realDir = join(root, "real");
  const aliasPath = join(root, "alias");
  const name = "HIMMEL-2734: under-tmp guard requires canonical child/root lockstep";
  try {
    mkdirSync(realDir);
    let canSymlink = true;
    try {
    ...
    if (!canSymlink) {
      ...
      const child = join(aliasPath, "fixture-x");
      mkdirSync(child);
      const canonicalChild = realpathSync.native(child);
      const aliasedRoot = resolve(aliasPath);
      const canonicalRoot = realpathSync.native(aliasPath);
      test(name, () => {
        expect(canonicalChild.startsWith(aliasedRoot + sep)).toBe(false); // MISMATCH: tmpdir() spelling refuses its own fixture.
        expect(canonicalChild.startsWith(canonicalRoot + sep)).toBe(true); // LOCKSTEP: TMP_ROOT spelling accepts it.
      });
    }
  } finally { rmSync(root, { recursive: true, force: true }); }
}

test("HIMMEL-2734: fixtureDir returns an already-canonical path that teardown accepts", () => {
  const dir = fixtureDir("cx-canonical-");
  try {
    expect(dir).toBe(realpathSync.native(dir));
  } finally { expect(removeFixture(dir)).toBe(true); }
});
