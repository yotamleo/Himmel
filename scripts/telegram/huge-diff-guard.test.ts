...
import { copyFileSync, mkdirSync, mkdtempSync, rmSync, appendFileSync, writeFileSync, readFileSync, existsSync } from "node:fs";
import {
  HUGE_DIFF_SHARE,
  HUGE_DIFF_MIN_BYTES,
  parseLsTreeSizes,
...
// HIMMEL-2642: graphify-out/ is a permanent, operator-authorized PRIVATE_PATHS
// exclusion (scripts/lib/public-clone-paths.sh, HIMMEL-1740) — a ~430k-line
// knowledge-graph artifact that never propagates to the public mirror. The
// public tree therefore has ZERO files under graphify-out/, so every REAL GIT
// case below — which seeds its fixture repo from the SHIPPED
// graphify-out/graph.json (see makeGraphRepo) — has nothing to copy. This is
// NOT missing test coverage to restore: the exclusion is deliberate, so gate
// on the actual artifact (never a CI/OS proxy) so any clone that DOES carry
// graphify-out/ — every operator machine, and any future public clone that
// stops excluding it — still runs every case below for real.
const HAS_GRAPHIFY_FIXTURE = existsSync(resolve("graphify-out/graph.json"));
// test.skipIf silently omits the case's name from bun's default (non-verbose)
// summary line — print the reason once, loudly, so a skip here is never
// mistaken for a suite that never had these cases at all.
if (!HAS_GRAPHIFY_FIXTURE) {
  console.warn(
    "[huge-diff-guard.test] SKIP all (REAL GIT) cases: graphify-out/graph.json not found at the repo root " +
      "— this tree legitimately lacks the private-only graphify-out/ artifact (PRIVATE_PATHS exclusion, HIMMEL-1740). " +
      "Not a failure; re-run from a checkout that carries it (any operator machine) to exercise these cases.",
  );
}
...
test.skipIf(!HAS_GRAPHIFY_FIXTURE)("(REAL GIT, shipped artifacts): a branch that regenerates graph.json gets a warning NAMING the path and its share", () => {
...
test.skipIf(!HAS_GRAPHIFY_FIXTURE)("(REAL GIT, shipped artifacts): the .gitattributes -diff entry makes `git diff --stat main...branch` report graph.json as Bin, not a line count (PR #1686 verification)", () => {
  const { repo, git } = makeGraphRepo();
  try {
    git(["checkout", "-b", "feat/graph-refresh"]);
    appendFileSync(join(repo, "graphify-out", "graph.json"), `,\n  {"regenerated": "${"x".repeat(8192)}"}`);
...
test.skipIf(!HAS_GRAPHIFY_FIXTURE)("(REAL GIT): a normal small-file branch produces NO warning", () => {
  const { repo, git } = makeGraphRepo();
  try {
...
test.skipIf(!HAS_GRAPHIFY_FIXTURE)("(REAL GIT): three-dot semantics — main's post-branch commits do NOT enter the probe (two-dot would report them as branch deletions)", () => {
  const { repo, git } = makeGraphRepo();
  try {
...
test.skipIf(!HAS_GRAPHIFY_FIXTURE)("(REAL GIT): main's post-fork growth of a path the branch only lightly touched is NOT attributed to the branch (names and sizes share ONE revision pair)", () => {
  const { repo, git } = makeGraphRepo();
  try {
...
test.skipIf(!HAS_GRAPHIFY_FIXTURE)("(REAL GIT): mirror — a large artifact dominating the BRANCH is still reported while main has also moved", () => {
  const { repo, git } = makeGraphRepo();
  try {
    git(["checkout", "-b", "feat/graph-refresh"]);
...
test.skipIf(!HAS_GRAPHIFY_FIXTURE)("(REAL GIT): a huge blob at the MERGE-BASE that main's tip no longer carries still warns (the deletion side IS the ingest hazard)", () => {
  const { repo, git } = makeGraphRepo();
  try {
...
test.skipIf(!HAS_GRAPHIFY_FIXTURE)("(REAL GIT): a changed path with LEADING whitespace resolves to ITS size — the lookup key is the verbatim path", () => {
  const { repo, git } = makeGraphRepo();
  try {
...
test.skipIf(!HAS_GRAPHIFY_FIXTURE)("(REAL GIT): a real probe against a missing base ref fails open silently", () => {
...
test.skipIf(!HAS_GRAPHIFY_FIXTURE)("(REAL GIT): a repo whose default branch is master still warns — the base is RESOLVED, not hard-coded", () => {
...
test.skipIf(!HAS_GRAPHIFY_FIXTURE)("(REAL GIT): a repo whose default branch exists ONLY as remote-tracking (origin/HEAD set, no local master) still warns — the base is a ref that RESOLVES (HIMMEL-1804 item 1)", () => {
  const { repo, git } = makeGraphRepo("master");
  try {
    git(["checkout", "-b", "feat/graph-refresh"]);
    appendFileSync(join(repo, "graphify-out", "graph.json"), `,\n  {"regenerated": "${"x".repeat(2 * 1024 * 1024)}"}`);
...
test.skipIf(!HAS_GRAPHIFY_FIXTURE)("(REAL GIT): a repo where NO base resolves (no origin/HEAD, no main, no master, empty init.defaultBranch) stays SILENT — no throw", () => {
...
test.skipIf(!HAS_GRAPHIFY_FIXTURE)("(REAL GIT): every `git diff` command the warning PRINTS uses the three-dot form the guard computed — never two-dot", () => {
  const { repo, git } = makeGraphRepo();
  try {
    git(["checkout", "-b", "feat/graph-refresh"]);
    appendFileSync(join(repo, "graphify-out", "graph.json"), `,\n  {"regenerated": "${"x".repeat(2 * 1024 * 1024)}"}`);
...
test.skipIf(!HAS_GRAPHIFY_FIXTURE)("(REAL GIT): a 1 MiB file among sub-MiB siblings does NOT warn — padded sizes must weigh their true bytes", () => {
  const { repo, git } = makeGraphRepo();
  try {
...
test.skipIf(!HAS_GRAPHIFY_FIXTURE)("(REAL GIT): the changed-path set is INDEPENDENT of diff.renames — a rename weighs BOTH sides' paths, deterministically", () => {
  const { repo, git } = makeGraphRepo();
  try {
...
test.skipIf(!HAS_GRAPHIFY_FIXTURE)("(REAL GIT): an un-attributed large text rename under diff.renames=false is NOTICED — the guard models the dump the worker will ingest", () => {
  const { repo, git } = makeGraphRepo();
  try {
...
test.skipIf(!HAS_GRAPHIFY_FIXTURE)("(REAL GIT): a branch NAME may legally carry Unicode bidi/separator controls — the warning escapes base and branch, not just the path", () => {
  const { repo, git } = makeGraphRepo();
  try {
