...
# HIMMEL-2786: use the real codex row and chokepoint, not a stub dispatcher,
# so stdin metadata that omits the explicit opt-in silently loses this brief.
CODEX_REG="$TMP/codex-lane.json"
CODEX_WRAPPER="$SCRIPT_DIR/../codex/dispatch-codex-exec.sh"
if command -v cygpath >/dev/null 2>&1; then CODEX_WRAPPER="$(cygpath -m "$CODEX_WRAPPER")"; fi
jq --arg wrapper "$CODEX_WRAPPER" '{lanes: [.lanes[] | select(.id == "codex-exec") |
    .probe = {kind:"always"} | del(.dormant, .readiness) |
    .dispatch.command = ["bash", $wrapper]]}' "$REAL_REG" > "$CODEX_REG"
CODEX_NOOP="$TMP/codex-noop.sh"
printf '#!/usr/bin/env bash\nexit 0\n' > "$CODEX_NOOP"
CODEX_BRIEF="$TMP/codex-brief.md"
run_registry env LANES_REGISTRY="$CODEX_REG" CODEX_BIN="$STUB" \
  CODEX_ACL_NORMALIZE="$CODEX_NOOP" CODEX_REAP_HELPER="$CODEX_NOOP" \
  CODEX_JOBS_DIR="$TMP/codex-jobs" HIMMEL_FLOW_RUNS_LEDGER="$TMP/codex-ledger.jsonl" \
  STUB_INPUT_FILE="$CODEX_BRIEF" bash "$DISPATCH" --lane codex-exec \
  --brief-file "$BRIEF" --name explicit-stdin --cwd "$REPO" --model gpt-5.5
if [ "$RC" -eq 0 ] && contains "$(cat "$CODEX_BRIEF" 2>/dev/null)" 'do the test' \
   && contains "$(cat "$CODEX_BRIEF" 2>/dev/null)" '## Worker contract'; then
  ok "generic codex dispatch delivers its brief through explicit stdin opt-in"
else
  bad "generic codex stdin delivery (rc=$RC out=$OUT)"
fi

DORMANT_ROWS="$(jq -r '.lanes[] | select(.class == "impl" and .dormant) | [.id, .dormant.optInEnv] | @tsv' "$REAL_REG")"
# HIMMEL-2744: a dormant lane only ever reaches dispatch-lane.sh's own
# DORMANT gate (~lines 229-235) if scripts/lanes/resolve.mjs's probe filter
# (~line 35) lets it through first. The 6 real dormant lanes' probes read
# host state (ZAI_API_KEY, `codex`/`wsl`/`hermes` on PATH, ...), so on a bare
# CI runner -- or any host with no lanes configured -- none of them pass and
# this case would exercise nothing there: the exact HIMMEL-2642 class. Give
# it a hermetic fixture instead: a copy of REAL_REG with every dormant
# lane's probe forced to {"kind":"always"}, so a lane is probe-AVAILABLE and
# DORMANT simultaneously -- the state the gate exists to refuse -- on every
# host. LANES_REGISTRY is an explicit full-override (resolve.mjs's
# loadRegistry skips the local overlay whenever it is set), so this fixture
# is not perturbed by a machine's lanes.local.json either.
DORMANT_FIXTURE_REG="$TMP/dormant-lanes.json"
jq '(.lanes[] | select(.dormant) | .probe) = {"kind":"always"}' "$REAL_REG" > "$DORMANT_FIXTURE_REG"
# Ask resolve.mjs, against the fixture, which lane ids currently pass their
# probe -- expected to be every declared dormant lane, always. Kept as a
# lookup rather than assumed so the existing fail-closed fallback below still
# fires if the fixture injection itself did not take (bad jq output, a probe
# that did not actually get overridden, ...) -- HIMMEL-2744's RED control.
...
# leave PROBE_AVAILABLE_IDS empty exactly like a fixture that failed to take
# would -- indistinguishable from the inside, but a completely different
# failure. Capture the lookup's own exit status and treat a non-zero rc OR
# empty stdout as a hard failure naming the cause, rather than silently
# degrading into "every lane is probe-unavailable" (which would still
# increment DORMANT_COUNT to 6 further down and report a false ok).
PROBE_JSON="$(LANES_REGISTRY="$DORMANT_FIXTURE_REG" node "$SCRIPT_DIR/../lanes/resolve.mjs" --json)"
PROBE_RC=$?
PROBE_AVAILABLE_IDS=""
if [ "$PROBE_RC" -ne 0 ] || [ -z "$PROBE_JSON" ]; then
  bad "dormant probe lookup (LANES_REGISTRY=$DORMANT_FIXTURE_REG node resolve.mjs --json exited $PROBE_RC with $( [ -z "$PROBE_JSON" ] && echo empty || echo non-empty ) stdout -- cannot tell which dormant lanes are probe-available under the fixture; refusing to silently treat every lane as probe-unavailable)"
else
...
while IFS=$'\t' read -r dormant_lane dormant_env || [ -n "$dormant_lane" ]; do
  ...
  run_registry env -u "$dormant_env" LANES_REGISTRY="$DORMANT_FIXTURE_REG" bash "$DISPATCH" --lane "$dormant_lane" --brief-file "$BRIEF" --name dormant --cwd "$REPO"
...
if [ -n "$DORMANT_SKIPPED" ]; then
  bad "dormant refusal not exercised for:$DORMANT_SKIPPED -- resolve.mjs reports the fixture-forced probe does not pass for ${DORMANT_SKIPPED# }, so dispatch-lane.sh never reaches its DORMANT gate for it here (unavailable-lane refusal instead) -- the fixture injection above did not take for this lane"
fi
