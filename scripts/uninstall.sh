...
#   HIMMEL_UNINSTALL_REPO_ROOT — fixture repo for scripts + framework hooks
#                          (HIMMEL-2754; default is this script's repo)
...
repo_has_framework_hooks() {
  ...
  if command -v git >/dev/null 2>&1; then
    ...
    if resolved_hooks_dir="$(git -C "$REPO_ROOT" rev-parse --git-path hooks 2>/dev/null)"; then
      hooks_resolved=1
