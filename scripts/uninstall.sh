...
#         (machine-setup/uninstall-plugins.sh, at the scope recorded in the
#         install profile — HIMMEL-2694; user-scope by default, but a
#         project/local-scope install affects only this repo)
...
# HIMMEL-2694: the scope plugins/marketplaces were installed at — read from
# the recorded install profile (himmelctl's own install-profile.json, whose
# `scope` field is exactly what askQuestions()/loadProfile() validate to
# 'project'|'user') so [4/7] passes the SAME --scope back to `claude plugin
# uninstall`/`marketplace remove` that install-plugins.sh used to install
# them — a project-scope install refuses to uninstall at any other scope
# ("Plugin ... is enabled at project scope ..."). Falls back to
# install-plugins.sh's own default ("user") when the profile is missing,
# unreadable, carries no `scope` field (a pre-2694 cache), or jq is not on
# PATH — uninstall.sh has no other hard jq dependency, so its absence
# degrades the same way a missing profile does rather than aborting.
PLUGIN_SCOPE="user"
...
if [ "$SKIP_PLUGINS" -eq 0 ]; then
  ...
  if [ "$PLUGIN_SCOPE" = "user" ]; then
    ...
    _plugin_scope_upper="$(printf '%s' "$PLUGIN_SCOPE" | tr '[:lower:]' '[:upper:]')"
    echo "     (${_plugin_scope_upper}-SCOPE, recorded in $_profile_file: affects only this repo)"
  fi
else
...
if [ "$SKIP_PLUGINS" -eq 1 ]; then
  ...
  echo "  using: $_claude_bin (scope: $PLUGIN_SCOPE)"
  _plug_args=(--scope "$PLUGIN_SCOPE")
