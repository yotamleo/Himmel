...
<#
  ...
  exit codes. himmel already runs its own rtk hook, and no other optimizer may
  claim a token boundary without clearing docs/token-economy.md's measurement
  gate — so OmniRoute's bundled compression stack must be provably OFF ("one
  optimizer per boundary"). NOTE: the cavemanConfig/cavemanOutputMode keys below
  are OmniRoute's OWN schema names for its bundled engines; they are unrelated to
  the response-compression plugin himmel removed in HIMMEL-2033, and must stay
  asserted here. This lint is a POSITIVE assertion over the authoritative engine-key
