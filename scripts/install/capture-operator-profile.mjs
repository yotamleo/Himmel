...
function capturePlugins(configDir, repoRoot) {
  ...
  // HIMMEL-2733: onDemandPlugins keys are the ON-DEMAND tier — installed,
  // shipped disabled (template[k] === false). An operator who has legitimately
  // enabled one is not drift; it's reported separately in onDemandEnabled.
  // Same defensive shape-check as `template` above: a malformed onDemandPlugins
  // (non-object/null/array) degrades to {} — no keys exempted from the classic
  // drift buckets — rather than throwing or silently exempting everything.
  const onDemand = (templateJson && typeof templateJson.onDemandPlugins === 'object' && templateJson.onDemandPlugins !== null && !Array.isArray(templateJson.onDemandPlugins))
    ? templateJson.onDemandPlugins : {};
  const onDemandKeys = new Set(Object.keys(onDemand));
  const enabledBeyondTemplate = templateReadFailed ? null : Object.keys(effective).filter((k) => effective[k] === true && template[k] !== true && !onDemandKeys.has(k)).sort();
  ...
  const onDemandEnabled = templateReadFailed ? null : Object.keys(onDemand).filter((k) => effective[k] === true).sort();
  return { delta: { enabledBeyondTemplate, templateTrueButDisabled, onDemandEnabled, templateReadFailed } };
}
...
function renderSummary(profile, delta) {
  ...
  lines.push(`  on-demand plugins enabled (informational, not drift): ${delta.plugins.templateReadFailed ? 'unknown (template read failed)' : delta.plugins.onDemandEnabled.length}`);
