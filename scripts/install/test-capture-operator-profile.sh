...
# itself before reporting it — and (HIMMEL-2733 tier-aware plugin drift):
# (qq) a disabled onDemandPlugins spec is not in any drift bucket, (rr) an
# enabled onDemandPlugins spec is reported in delta.plugins.onDemandEnabled
# and excluded from enabledBeyondTemplate, (ss) a malformed onDemandPlugins
# (non-object) degrades safely — no throw, no false-clean exemption.
set -euo pipefail
...
# ── HIMMEL-2733 tier-aware plugin drift (cases qq/rr/ss) ───────────────────
# Base repo (no .git, cheap to copy) + template carrying onDemandPlugins
# on-demand specs "od1@mp"/"od2@mp", each mirrored `false` in enabledPlugins
# per the two-tier contract check-template-himmel-plugins.sh enforces.
FIX_REPO_OD="$work/repo-ondemand"
cp -r "$FIX_REPO_NONGIT" "$FIX_REPO_OD"
cat > "$FIX_REPO_OD/docs/setup/settings-template.json" <<'EOF'
{
  "enabledPlugins": { "a@mp": true, "b@mp": false, "od1@mp": false, "od2@mp": false },
  "onDemandPlugins": { "od1@mp": { "neededBy": "x" }, "od2@mp": { "neededBy": "y" } }
}
EOF

# case qq: od1@mp disabled (effective false, same as template) → not in
# enabledBeyondTemplate, not in onDemandEnabled — an on-demand spec left
# disabled is fully conforming, not drift.
FIX_CONFIG_OD_DISABLED="$work/config-od-disabled/.claude"
mkdir -p "$FIX_CONFIG_OD_DISABLED"
cat > "$FIX_CONFIG_OD_DISABLED/settings.json" <<'EOF'
{ "enabledPlugins": { "a@mp": true, "od1@mp": false } }
EOF
odDisabledOut=$(HIMMEL_CAPTURE_REPO_ROOT="$(winpath "$FIX_REPO_OD")" CLAUDE_CONFIG_DIR="$(winpath "$FIX_CONFIG_OD_DISABLED")" run_json)
odDisabledCheck=$("$node_bin" -e "
const j = JSON.parse(require('fs').readFileSync(0, 'utf8'));
const p = j.delta.plugins;
const problems = [];
if (p.enabledBeyondTemplate.includes('od1@mp')) problems.push('od1@mp (disabled) should not be in enabledBeyondTemplate, got ' + JSON.stringify(p.enabledBeyondTemplate));
if (p.onDemandEnabled.includes('od1@mp')) problems.push('od1@mp (disabled) should not be in onDemandEnabled, got ' + JSON.stringify(p.onDemandEnabled));
console.log(problems.join('|'));
" <<< "$odDisabledOut")
if [ -z "$odDisabledCheck" ]; then pass "case qq: a disabled onDemandPlugins spec is not in any drift bucket"; else fail "case qq: on-demand disabled handling" "$odDisabledCheck"; fi

# case rr: od1@mp enabled by the operator (effective true, template false) →
# reported in onDemandEnabled (informational), explicitly excluded from
# enabledBeyondTemplate (classic drift). Negative control: od2@mp (also
# on-demand, but left disabled here) must appear in neither.
FIX_CONFIG_OD_ENABLED="$work/config-od-enabled/.claude"
mkdir -p "$FIX_CONFIG_OD_ENABLED"
cat > "$FIX_CONFIG_OD_ENABLED/settings.json" <<'EOF'
{ "enabledPlugins": { "a@mp": true, "od1@mp": true } }
EOF
odEnabledOut=$(HIMMEL_CAPTURE_REPO_ROOT="$(winpath "$FIX_REPO_OD")" CLAUDE_CONFIG_DIR="$(winpath "$FIX_CONFIG_OD_ENABLED")" run_json)
odEnabledCheck=$("$node_bin" -e "
const j = JSON.parse(require('fs').readFileSync(0, 'utf8'));
const p = j.delta.plugins;
const problems = [];
if (!p.onDemandEnabled.includes('od1@mp')) problems.push('od1@mp (enabled) should be in onDemandEnabled, got ' + JSON.stringify(p.onDemandEnabled));
if (p.enabledBeyondTemplate.includes('od1@mp')) problems.push('od1@mp (enabled) should be EXCLUDED from enabledBeyondTemplate, got ' + JSON.stringify(p.enabledBeyondTemplate));
if (p.onDemandEnabled.includes('od2@mp')) problems.push('od2@mp (left disabled) should not be in onDemandEnabled, got ' + JSON.stringify(p.onDemandEnabled));
console.log(problems.join('|'));
" <<< "$odEnabledOut")
if [ -z "$odEnabledCheck" ]; then pass "case rr: an enabled onDemandPlugins spec is reported in onDemandEnabled and excluded from enabledBeyondTemplate"; else fail "case rr: on-demand enabled handling" "$odEnabledCheck"; fi

# case ss: onDemandPlugins is malformed (an array, not an object) → must
# degrade safely (no throw, rc 0) and must NOT silently exempt od1@mp from
# the classic drift bucket (a false-clean pass) — it falls back to reading
# as ordinary enabledBeyondTemplate drift instead.
FIX_REPO_OD_MALFORMED="$work/repo-ondemand-malformed"
cp -r "$FIX_REPO_NONGIT" "$FIX_REPO_OD_MALFORMED"
cat > "$FIX_REPO_OD_MALFORMED/docs/setup/settings-template.json" <<'EOF'
{
  "enabledPlugins": { "a@mp": true, "b@mp": false },
  "onDemandPlugins": [ "od1@mp" ]
}
EOF
set +e
odMalformedOut=$(HIMMEL_CAPTURE_REPO_ROOT="$(winpath "$FIX_REPO_OD_MALFORMED")" CLAUDE_CONFIG_DIR="$(winpath "$FIX_CONFIG_OD_ENABLED")" run_json)
rcOdMalformed=$?
set -e
odMalformedCheck=$("$node_bin" -e "
const j = JSON.parse(require('fs').readFileSync(0, 'utf8'));
const p = j.delta.plugins;
const problems = [];
if (p.templateReadFailed !== false) problems.push('templateReadFailed should be false (template itself is valid JSON), got ' + p.templateReadFailed);
if (!Array.isArray(p.onDemandEnabled) || p.onDemandEnabled.length !== 0) problems.push('onDemandEnabled should degrade to [], got ' + JSON.stringify(p.onDemandEnabled));
if (!p.enabledBeyondTemplate.includes('od1@mp')) problems.push('od1@mp should fall back into enabledBeyondTemplate (not silently exempted), got ' + JSON.stringify(p.enabledBeyondTemplate));
console.log(problems.join('|'));
" <<< "$odMalformedOut")
if [ "$rcOdMalformed" -eq 0 ] && [ -z "$odMalformedCheck" ]; then
  pass "case ss: a malformed (array) onDemandPlugins degrades safely — no throw, no false-clean exemption"
else
  fail "case ss: malformed onDemandPlugins handling" "rc=$rcOdMalformed $odMalformedCheck"
fi
