...
# --- T4b (HIMMEL-3334 F2): the un-swept claude-hud.json path (new, alongside
...
[ -f "$FAKEHOME/.claude-routed/claude-hud.json" ] || { echo "FAIL: claude-hud.json (new path) not seeded"; FAILS=$((FAILS+1)); }
grep -q "new-path" "$FAKEHOME/.claude-routed/claude-hud.json" || { echo "FAIL: seeded claude-hud.json content mismatch"; FAILS=$((FAILS+1)); }
...
grep -q "new-path-updated" "$FAKEHOME/.claude-routed/claude-hud.json" || { echo "FAIL: new-path hud change did not trigger reseed"; FAILS=$((FAILS+1)); }
rm -f "$FAKEHOME/.claude/claude-hud.json"
t "deleted new-path hud source mirrors removal" 0
[ ! -f "$FAKEHOME/.claude-routed/claude-hud.json" ] || { echo "FAIL: stale claude-hud.json (new path) survived source deletion"; FAILS=$((FAILS+1)); }
