...
# ── SC6H (HIMMEL-2849): [5/8] native-gate hook detection/removal must key off
# the invocation directory — same target [6/8]'s project-settings unwire
# above already uses ($PWD) — never $REPO_ROOT, which for a project-scope
# adopt into a repo SEPARATE from the himmel checkout resolves to the
# checkout providing uninstall.sh's own sibling scripts. Pre-fix, [5/8]
# scanned the CHECKOUT's .git/hooks (never populated here) and reported "no
# native gates found" while the adopter project's real native-gate hooks
# (what adopt.sh actually placed at $PWD) survived untouched — success
# claimed, HIMMEL-2839's guarantee not held. A fixture checkout (own copy of
# uninstall.sh, no HIMMEL_UNINSTALL_REPO_ROOT override) keeps this hermetic:
# the real checkout running this suite is never touched either way.
H_BIN="$TMP/h-bin"
mkdir -p "$H_BIN"
link_hermetic_tool git "$H_BIN"
...
mkdir -p "$H_PROJECT"
git init -q "$H_PROJECT"
mkdir -p "$H_PROJECT/.git/hooks"
for hook in commit-msg pre-commit pre-push; do
    printf '#!/usr/bin/env bash\n# HIMMEL-2771: native invariant gate; lint hooks require pre-commit.\nexit 0\n' \
        > "$H_PROJECT/.git/hooks/$hook"
    chmod 755 "$H_PROJECT/.git/hooks/$hook"
...
assert_rc "SC6H project native-gate removal exits 0" 0 "$rc"
assert_has "SC6H completion reported" "Uninstall complete." "$out"
for hook in commit-msg pre-commit pre-push; do
    assert_has "SC6H removed $hook reported" "removed native gate: $H_PROJECT/.git/hooks/$hook" "$out"
...
# native-gate + payload-dir logic this case targets. HIMMEL-2862: the public
# runner has no real pre-commit installed (only the station does), so
# link_hermetic_tool's `command -v pre-commit` fails there before the PATH
# scrub — ship a private shim instead of linking the real binary. The
# assertions below are about the hooks target under core.hooksPath, not about
# pre-commit's own behaviour, so a no-op exit-0 stub is deterministic on both
# hosts.
H_BIN4="$TMP/h-bin4"
mkdir -p "$H_BIN4"
link_hermetic_tool git "$H_BIN4"
printf '#!/usr/bin/env bash\nexit 0\n' > "$H_BIN4/pre-commit"
chmod 755 "$H_BIN4/pre-commit"
...
# toolchain.
# HIMMEL-2862: as with SC6J's H_BIN4, ship a private exit-0 pre-commit shim
# instead of link_hermetic_tool (the runner has no real pre-commit on PATH to
# link) — SC6K-SC6R below only need pre-commit to be RESOLVABLE, never to
...
# failed" per the codex-1 /pr-check finding. Originally simulated via git's
...
# ── SC6N (HIMMEL-2854, codex-1 round 4, HIMMEL-2862): HOOKS_REPO_ROOT can
# itself be a SUBDIRECTORY of the real repo root — a refusing rev-parse
# rejects from a subdirectory exactly like it does from the root, but `.git`
# lives only at the ancestor, so a single non-recursive check at
# HOOKS_REPO_ROOT alone would wrongly take the SC6K clean skip and leave the
# ancestor repo's installed hooks untouched. Same dangling-gitdir `.git` FILE
# as SC6L, planted at the ancestor so the walk-up still finds it.
...
# HIMMEL-3253: the pre-commit framework's own identity set (is_our_script() in
# pre_commit/commands/install_uninstall.py): CURRENT_HASH first, then the five
# PRIOR_HASHES. A hook the framework installed carries exactly one of these.
FRAMEWORK_HASHES=(138fd403232d2ddd5efb44317e38bf03 4d9958c90bc262f47553e2c073f14cfe
    d8ee923c46731b42cd95cc869add4062 49fd668cb42069aa1b6048464be5d395
    79f09a650522a87b0da915d0d983b2de e358c9dae00eac5d06b38dfdb1e33a8c)
# framework_hook_text [hash] — what `pre-commit install` writes (default: CURRENT_HASH).
framework_hook_text() {
    printf '#!/usr/bin/env bash\n# File generated by pre-commit: https://pre-commit.com\n# ID: %s\n' "${1:-${FRAMEWORK_HASHES[0]}}"
}
...
framework_hook_text > "$TMP/u-gitdir/hooks/pre-commit"
...
# U4l — RED HIMMEL-3253: an adopter hook that merely MENTIONS pre-commit but
# carries none of the framework's six hashes is not a framework hook — `pre-commit
# uninstall` could never remove it — so with pre-commit absent it must NOT halt
# [5/8]. Pre-fix this halted rc=2 with steps 6-8 skipped (the bare-substring test).
u_backup_fixture u4l
u_hooks_reset
printf '%s' "$ADOPTER_HOOK_TEXT" > "$U_REPO/.git/hooks/pre-commit"
u_run
assert_rc 'U4l mention-only hook completes' 0 "$rc"
assert_has 'U4l completion reported' 'Uninstall complete.' "$out"
assert_has 'U4l absent pre-commit is a note' "note: \`pre-commit\` not found and this repo carries no framework hooks — nothing to uninstall" "$out"
assert_not_has 'U4l no halted steps' 'skipped — halted' "$out"
assert_not_has 'U4l no pre-commit-not-found halt' 'not found — this step did NOT run' "$out"
# "did not halt" is not "finished the job": steps 6-8 must have actually RUN.
assert_has 'U4l settings unwire ran' '[6/8] Unwiring' "$out"
assert_has 'U4l marketplaces ran' 'plugin marketplace remove himmel' "$calls"
if [ ! -e "$U_CACHE" ]; then echo 'PASS U4l step 8 removed cache'
else echo 'FAIL U4l step 8 left cache'; FAILED=$((FAILED + 1)); fi
if [ "$(cat "$U_REPO/.git/hooks/pre-commit"; echo x)" = "${ADOPTER_HOOK_TEXT}x" ]; then
    echo 'PASS U4l adopter hook untouched'
else
    echo 'FAIL U4l adopter hook changed'; FAILED=$((FAILED + 1))
fi
u_hooks_reset

# U4n — the positive control that keeps U4l honest (a fix that merely stops
# detecting hooks is a vacuous pass): a marker-free hook carrying CURRENT_HASH or
# ANY of the five PRIOR_HASHES is a real framework hook and still halts [5/8] with
# pre-commit absent, naming the file. An older pre-commit is exactly who this is for.
u4n_i=0
for u4n_hash in "${FRAMEWORK_HASHES[@]}"; do
    u4n_i=$((u4n_i + 1))
    u_fixture "u4n$u4n_i"
    u_hooks_reset
    framework_hook_text "$u4n_hash" > "$U_REPO/.git/hooks/commit-msg"
    u_run
    assert_rc "U4n hash $u4n_i (${u4n_hash:0:8}) still halts" 2 "$rc"
    assert_has "U4n hash $u4n_i first failure is hooks" 'Halted at: [5/8]' "$out"
    assert_has "U4n hash $u4n_i halt names the file" "$U_REPO/.git/hooks/commit-msg" "$out"
    u_hooks_reset
done
# U4n-miss — a one-character-short CURRENT_HASH is not the framework's hash: the
# match is the whole 32-hex string, not a prefix or a shape.
u_fixture u4nmiss
u_hooks_reset
framework_hook_text "${FRAMEWORK_HASHES[0]%?}" > "$U_REPO/.git/hooks/commit-msg"
u_run
assert_rc 'U4n-miss truncated hash is not a framework hook' 0 "$rc"
assert_not_has 'U4n-miss no halted steps' 'skipped — halted' "$out"
u_hooks_reset
...
# chmod 444 stops nobody who is root (a privileged CI or VM run): probe first and
# skip loudly rather than fail a case whose premise cannot be built there.
: > "$TMP/u24-probe"; chmod 444 "$TMP/u24-probe"
if [ -w "$TMP/u24-probe" ]; then
  echo 'SKIP U24 a chmod 444 file is still writable here (running as root) -- unwritable-target case not exercised'
else
  mkdir -p "$TMP/u24tmp"
  printf 'mine\n' > "$TMP/u24-rules"
