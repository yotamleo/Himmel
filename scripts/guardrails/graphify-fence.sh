...
#                        claude-cli ONLY, any CLAUDE_CODE_USE_* reroute selector
#                          SET (HIMMEL-1070; see _CLAUDE_REROUTE_VARS) ->
#                          anthropic-rerouted -> HARD DENY on every corpus. These
#                          reroute the claude CLI to Bedrock/Vertex/Foundry/a
#                          gateway without touching ANTHROPIC_BASE_URL, so they
#                          are checked FIRST; none is a ratified matrix provider.
#                          ONLY unset/empty is off - '0'/'false' are SET and deny.
#                          NOT applied to `claude`: that backend is a plain HTTP
#                          call and never execs the CLI, so the vars are inert
#                          for it (the endpoint check below still covers both)
...
#
# NOW HANDLED (HIMMEL-2615):
#   - `setsid` (flag-consuming, same transparent shape as `nohup`) and the
#     repo's own quiet runner `scripts/quiet-run.sh <label> -- <cmd...>`, whose
#     `--` tail is re-classified as the clause - reached both when executed
#     directly and when launched as `bash scripts/quiet-run.sh ...`. A
#     quiet-run tail whose label cannot be aligned (a quoted label containing
#     whitespace, which this file's word-split tokenisation cannot rejoin) is
#     DENIED if it mentions graphify anywhere, the same fail-closed treatment
#     as xargs / find -exec.
#
...
# the Anthropic API, none of which touch ANTHROPIC_BASE_URL (HIMMEL-1070). The
# list is explicit, not a `CLAUDE_CODE_USE_*` wildcard, because the CLI ships
# same-prefixed vars that are ordinary FEATURE flags and must NOT deny
# (CLAUDE_CODE_USE_COWORK_PLUGINS / _NATIVE_FILE_SEARCH / _POWERSHELL_TOOL);
# enumerated from the installed CLI's own selector strings + the Bedrock/Vertex
# setup docs. A newly-added selector is the maintenance cost of that precision -
# accepted: a wrong wildcard deny is worse than a known list to extend.
_CLAUDE_REROUTE_VARS="CLAUDE_CODE_USE_BEDROCK CLAUDE_CODE_USE_VERTEX CLAUDE_CODE_USE_FOUNDRY CLAUDE_CODE_USE_GATEWAY CLAUDE_CODE_USE_MANTLE CLAUDE_CODE_USE_ANTHROPIC_AWS"

# _claude_reroute_var -> echoes the name of the FIRST reroute selector that is
# SET (any non-empty value), or "" when none is.
#
# SET, not "truthy" (CodeRabbit-major on HIMMEL-1070): an earlier cut treated
# `0` / `false` as OFF, which assumed a boolean PARSE the docs never promise.
# The docs only ever show `=1` and never define an off-value; Claude Code is a
# Node app, where a plain `Boolean(process.env.X)` check reads the STRING "0" as
# TRUE - so `CLAUDE_CODE_USE_BEDROCK=0` would very plausibly ENABLE Bedrock while
# a bash-minded fence read it as disabled. That is the exact fail-open this whole
...
_map_anthropic_endpoint() {
    local backend_name="${1:-}" u host
    # REROUTE SELECTORS FIRST (HIMMEL-1070, public-CR-critical): these reroute
    # the `claude` CLI away from Anthropic WITHOUT touching ANTHROPIC_BASE_URL,
    # so the endpoint check below is blind to them - unset base-url +
    # CLAUDE_CODE_USE_BEDROCK=1 returned `anthropic` and egressed corpus content
    # to AWS classified as Anthropic. Same bypass class as the Z.ai hole above,
    # different selector. himmel's own scripts/claude-codex treats these as
    # session-rerouting vars and explicitly CLEARS them. None of these targets is
    # a ratified matrix provider, so any of them set maps to a fail-closed
    # sentinel -> hard-denied before the matrix eval (a sentinel distinct from
    # anthropic-custom purely so the deny message can name the RIGHT variable;
    # both are equally hard-denied).
    #
    # claude-cli ONLY (CodeRabbit-major on HIMMEL-1070): these are Claude CODE
    # env vars, so they bind only to the backend that actually SHELLS that CLI.
    # Verified in graphify's own llm.py: `claude-cli` dispatches _call_claude_cli
    # (:1484) and inherits the env, while `claude` is a plain HTTP call to
    # base_url/ANTHROPIC_API_KEY (:60-67) that never execs the CLI and cannot see
    # them. Denying `claude` on a set selector was a FALSE deny - it would block
    # the legitimate config of an operator whose interactive Claude Code runs on
    # Bedrock while graphify extracts over the API. Same precision principle as
    # the explicit-list-not-wildcard choice above; the ENDPOINT check below still
    # applies to BOTH, because the `claude` backend genuinely does read
    # ANTHROPIC_BASE_URL. (graphify also has a separate first-class `bedrock`
    # backend, which maps to the literal `bedrock` -> undeclared -> default deny.)
    if [ "$backend_name" = "claude-cli" ] && [ -n "$(_claude_reroute_var)" ]; then
        echo anthropic-rerouted; return
    fi
...
apply_verdict() {
    ...
    # Same fail-closed treatment for the reroute sentinel (HIMMEL-1070): a
    # CLAUDE_CODE_USE_* selector sends the claude CLI's traffic to AWS Bedrock /
    # Google Vertex / a Foundry/gateway endpoint, none of them ratified matrix
    # providers - so they must not reach the matrix and land on the
    # `himmel-code x * x *` wildcard either. Named separately from
    # anthropic-custom only so the message names the variable the operator must
    # actually clear (the value is NOT echoed - same reasoning as below).
    if [ "$provider" = "anthropic-rerouted" ]; then
        deny "claude backend is rerouted away from Anthropic ($(_claude_reroute_var) is set); its target (AWS Bedrock / Google Vertex / Foundry / a gateway) is not a ratified egress-matrix provider, so corpus content must not reach it classified as anthropic; refusing on every corpus (fail-closed). NOTE: only UNSET/EMPTY counts as off - '0' and 'false' are still set, and the CLI may well read them as ON. Fix: unset that variable in the launching shell, or pick a backend that does not read it (e.g. --backend ollama for local extraction)"
    fi
    ...
    while [ "$i" -lt "$n" ]; do
        ...
        case "$(_wrapper_base "$s")" in
            ...
            setsid)
                ...
                # consume the leading flags and keep walking. It was simply
                # missing from the wrapper list, and `setsid graphify ...`
                # walked past the fence at rc=0 while `nohup graphify ...`
                # (identical target, identical flags) denied at rc=2.
                i=$((i+1))
                while [ "$i" -lt "$n" ]; do
                ...
                continue ;;
            quiet-run.sh)
                # HIMMEL-2615: scripts/quiet-run.sh <label> -- <cmd...> execs its
                # own `--` tail, so that tail IS the clause. The runner's grammar
                # is strict - label at $1, a LITERAL `--` at $2, usage/exit 2
                # otherwise - so mirroring its own parse (skip exactly one token,
                # require `--`) is exact rather than a text match. Reached here
                # for a direct `./scripts/quiet-run.sh ...` or any path-qualified
                # form (via _wrapper_base); `bash <path>/quiet-run.sh ...` and
                # `bash -c "... quiet-run.sh ..."` arrive through the bash/sh
                # unwrap below, which restarts this walk on the script token.
                # Guard against a false alignment: a quoted label with an
                # embedded `--` (e.g. `'a -- b'`) word-splits into its OWN
                # `--`-shaped token, which would otherwise satisfy the check
                # below at the wrong position and silently drop the real
                # separator (and the graphify tail after it) from view.
                #
                # History (do not re-attempt a cleverer version of this):
                # CR round 1 tried a per-quote-type PARITY count (odd `'` OR
                # odd `"`, counted separately, OR any backslash -> ambiguous).
                # CR round 2 broke it with adjacent quoted segments in one
                # fragment, e.g. label `'"'"a -- b"` (real argv label is
                # `"a -- b`, one word): fragment 1 is `'"'"a` - two `'` and
                # two `"`, EVEN on both types, so parity called it balanced.
                # It is not: the first `"` there is literal DATA (it sits
                # INSIDE a pair of single quotes), the second `"` is the
                # OPERATOR that opens a quote closed only in a later fragment
                # - and a character count cannot tell data from operator
                # apart. No parity rule can (a third and a fourth quote
                # character just move the same ambiguity one token further
                # out). This file is explicitly NOT a shell parser (see file
                # header) - tracking real quoting state would be the one
                # place in it pretending otherwise, so we do not.
                #
                # THE RULE: the alignment check below is trusted ONLY for a
                # label token that carries NO shell quoting or escaping at
                # all - no `'`, no `"`, no `\`, anywhere in the raw token. A
                # bare word cannot span whitespace, so its fragment IS the
                # whole label, unambiguously. Anything else - one quote char,
                # a mismatched pair, backslash, adjacent quoted segments, any
                # combination - we decline to guess and fall through to the
                # fail-closed scan below.
                #
                # Cost, stated honestly: a LEGITIMATE quoted single-word label
                # (`'lbl'`) now also fails this test and DENIES instead of
                # classifying - a real false positive, accepted deliberately.
                # It fails in the safe direction, the deny message already
                # names the remedy (invoke graphify directly), and it only
                # fires on a quiet-run tail that actually contains a graphify
                # token - an ordinary quoted label with no graphify in its
                # tail is untouched, because the scan below finds nothing and
                # returns 0.
                #
                # Bounds FIRST, under `set -u`: `[ $((i+3)) -lt "$n" ]` is the
                # only thing that proves toks[i+1] and toks[i+2] are in range,
                # so it has to short-circuit before either index is read - a
                # clause whose last token IS the runner (`./quiet-run.sh` with
                # no label at all) would otherwise blow up on an unset toks[i+1]
                # instead of reaching the fail-closed scan below.
                if [ $((i+3)) -lt "$n" ]; then
                    lbl_raw="${toks[$((i+1))]}"
                    lbl_special="${lbl_raw//[^\'\"\\]/}"
                    if [ -z "$lbl_special" ] && [ "$(_strip_cmd "${toks[$((i+2))]}")" = "--" ]; then
                        i=$((i+3)); continue               # runner + label + `--` consumed
                    fi
                fi
                # Shape did not line up. For a call the runner itself ACCEPTS
                # that means a quoted label containing whitespace, which this
                # file's word-split tokenisation cannot rejoin - the tail's
                # argument boundaries are then unknowable. Fail closed exactly
                # like xargs / find -exec: deny if graphify appears anywhere in
                # it, leave every other tail alone. (A tail the runner REJECTS
        ...
        bash|sh|*/bash|*/sh)
            ...
            while [ "$j" -lt "$n" ]; do
                ...
                case "$fs" in
                    ...
                    -*) j=$((j+1)); continue ;;
                    *)
                        # HIMMEL-2615: `bash <script> <args...>` - re-resolve the
                        # clause from the script token, so a script the wrapper
                        # walk above knows (scripts/quiet-run.sh) is still
                        # recognised when it is launched through an explicit
                        # interpreter rather than executed directly. For any
                        # other script the restarted walk stops on that very
                        # token and returns 0, exactly as before.
                        classify_clause "${toks[@]:$j}"
                        return
                        ;;
                esac
            done
