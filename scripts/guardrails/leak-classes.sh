...
ALLOW_HOME_NAMES=' you user <user> $user ${user} me <me> example <you> %username% $env:username %s ada jarrod alice bob diane jose somebody claude yotamleo leaktestuser leaker testop testuser test osboxes nulleak name <name> yourname wineonlyuser shouldnotleak realop quarantoken priorleak posixuser profileonlyuser unixonlyuser fakeuser myvault op ops otheruser fakeop runneradmin fake-sensitive-scriptpath-marker ... '
...
check_home_path() {
    ...
    # Bounded the same way the no-space form already was: the class excludes '/' and '\', so a
    # space only ever extends the match within the SAME path segment, up to
    # the next separator -- it can never swallow unrelated prose that isn't
    # itself immediately followed by a literal '/' or '\'.
    ...
    while [[ "$remaining" =~ $re ]]; do
        ...
        if ! home_name_allowed "$name"; then
            MATCHES+=("${BASH_REMATCH[2]}${BASH_REMATCH[3]}${BASH_REMATCH[5]}")
        fi
